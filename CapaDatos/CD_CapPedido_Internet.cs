...
namespace CapaDatos
{
    public class CD_CapPedido_Internet
    {
        ...
        public void ConsultarPedidos(string Conexion, ref IList<Pedido_Internet> pedidosLista, Pedido_Internet pedido)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    p.Id_Ped = Convert.ToInt32(dr["idPedido"]);
                    p.tipoPedido = Convert.ToInt32(dr["tipoPedido"]);
                    p.Url = dr.IsDBNull(dr.GetOrdinal("UrlPedido")) ? "N/A" : Convert.ToString(dr["UrlPedido"]);
                    //p.tipoPedido = 0;
                    pedidosLista.Add(p);
                }
        ...
        public void ConsultarPedido_Datos(string Conexion, ref Pedido_Internet pedido, ref ClienteDirEntrega dirEntrega, int num_Pedido, int tipoPedido)
        {
            try
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    p.id_Acs = Convert.ToInt32(dr["id_Acs"]);
                    ...
                    p.Estatus_Id = dr["Estatus_Id"].ToString();
                    p.OrdenCompra = Convert.ToBoolean(dr["OrDenCompra"]);
                    p.OrdenCompraSTR = Convert.ToString(dr["OrdenCompraSTR"]);
                    p.Id_PedExt = Convert.ToInt32(dr["id_pedidoExterno"]);
                    p.UsoCFDI = dr["Cte_UsoCFDI"].ToString();
        ...
        public void ConsultaPedido_Detalle(string Conexion, Int32 num_pedido, int tipoPedido, ref DataTable dt)
        {
            try
            {
                CapaDatos.CD_Datos CapaDatos = new CapaDatos.CD_Datos(Conexion);
                ...
                while (dr.Read())
                {
                    dt.Rows.Add(new object[] {
                            ...
                            0,
                            0,
                            dr.GetValue(dr.GetOrdinal("PrecioLista"))
        ...
        public void ConsultaPedido_DetalleAdmin(string Conexion, Int32 num_pedido, int tipoPedido, ref DataTable dt)
        {
            try
            {
                CapaDatos.CD_Datos CapaDatos = new CapaDatos.CD_Datos(Conexion);
