    ...
    public class CD_CatProveedores
    {
        ...
        public List<CapaEntidad.CatProveedorLocal> spProveedores_ComboCompraLocal(int Id1, int Id2, string Conexion)
        {
            ...
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spProveedores_ComboCompraLocal", ref dr, Parametros, Valores);

                while (dr.Read())
                {
                    CapaEntidad.CatProveedorLocal obj = new CapaEntidad.CatProveedorLocal();
