...
namespace CapaDatos
{
    public class CD_CatUen
    {
        ...
        public List<Uen> SelCatUen(int Id_Emp, int Id_Uen, string Conexion)
        {
            ...
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRM_SelCatUEN", ref dr, Parametros, Valores);

                Uen uen;
                while (dr.Read())
                {
                    uen = new Uen();
                    uen.Id_Emp = (int)dr.GetValue(dr.GetOrdinal("Id_Emp"));
                    uen.Id_Uen = (int)dr.GetValue(dr.GetOrdinal("Id_Uen"));
