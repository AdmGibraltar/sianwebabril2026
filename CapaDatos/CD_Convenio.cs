...
namespace CapaDatos
{
    public class CD_Convenio
    {
        public void ConsultaListaConvenios(Convenio conv, ref List<Convenio> ListUtil, ref List<Convenio> ListNoUtil, string Conexion)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    c.PCD_Gral = Convert.ToBoolean(dr["PCD_Gral"]);
        ...
        public void InsertarConvenio(Convenio conv, List<ConvenioDet> List, ref int Verificador, ref int Id_PC, string Conexion)
        {
            ...
            try
            {
                ...
                object[] Valores = {
                                      ...
                                      conv.Id_PCAnterior,
                                      conv.Id_U,
                                      conv.PC_NombreArchivoPDF   ,
                                      conv.PC_ArchivoPDF  ,
                                      conv.PC_FechaInicio  ,
                                      conv.PC_NoConvenioFuturo  ,
                                      conv.PC_DesplegarMatrizCN    
                                  };
                ...
                if (Verificador > 0)
                {
                    ...
                    string[] Parametros2 = { "@Id_Pc",
                                            ...
                                            "@PCD_CatDesp" ,
                                            "@PCD_Referencia",
                                            "@PCD_PrecioPAAAEspProv" ,
                                            "@PCD_PrecioVentaConvenio" ,
                                            "@PCD_Margen" };
                    foreach (ConvenioDet c in List)
                    {
                        ...
                        object[] Valores2 = {
                                           ...
                                           c.PCD_CatDesp,
                                           c.PCD_Referencia ,
                                           c.PCD_PrecioPAAAEspProv == (double?)null ? (object) null: c.PCD_PrecioPAAAEspProv,
                                           c.PCD_PrecioVentaConvenio == (double?)null ? (object) null: c.PCD_PrecioVentaConvenio,
                                           c.PCD_Margen == (double?)null ? (object) null: c.PCD_Margen  };
        ...
        public void ConsultaConvenio(int Id_PC, ref Convenio conv, string Conexion)
        {
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    conv.PC_NombreArchivoPDF = dr["PC_NombreArchivoPDF"].ToString();
                    conv.PC_ArchivoPDF = dr["PC_ArchivoPDF"].ToString();
                    conv.PC_FechaInicio = dr["PC_FechaInicio"].ToString() == "" ? (DateTime?)null : Convert.ToDateTime(dr["PC_FechaInicio"]);
                    conv.PC_FechaCreo = Convert.ToDateTime(dr["PC_FechaCreo"]);
                    conv.PC_NoConvenioFuturo = dr["PC_NoConvenioFuturo"].ToString();
                    conv.PC_DesplegarMatrizCN = dr["PC_DesplegarMatrizCN"].ToString() == "" ? 0 : Convert.ToInt32(dr["PC_DesplegarMatrizCN"]);
        ...
        public void ConsultaConvenioDet(int Id_PC, ref List<ConvenioDet> List, string Conexion)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    c.PCD_PrecioVtaMin = dr["PCD_PrecioVtaMin"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioVtaMin"]);
                    c.PCD_PrecioVtaMax = dr["PCD_PrecioVtaMax"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioVtaMax"]);
                    ...
                    // JFCV 15 abr 2019 la fecha fin ver ya no mostrará como antes de que si fecha fin era nula mostrara la finc o finA 
                    // ahora siempre mostrará la fechafin como fechafinver 

                    //c.PCD_FechaFinVer = dr["PCD_FechaFinVer"].ToString() == "" ? (DateTime?)null : Convert.ToDateTime(dr["PCD_FechaFinVer"]);
                    c.PCD_FechaFinVer = dr["PCD_FechaFin"].ToString() == "" ? (DateTime?)null : Convert.ToDateTime(dr["PCD_FechaFin"]);


                    c.PCD_Referencia = dr["PCD_Referencia"].ToString();

                    //JFCV Convenio
                    c.PCD_PrecioPAAAEspProv = dr["PCD_PrecioPAAAEspProv"].ToString() == "0" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioPAAAEspProv"]);
                    c.PCD_PrecioVentaConvenio = dr["PCD_PrecioVentaConvenio"].ToString() == "0" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioVentaConvenio"]);
                    c.PCD_Margen = dr["PCD_Margen"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_Margen"]);
        ...
        public void ConsultaConvenioHis(int Id_PC, ref ConvenioHis conv, ref List<ConvenioHis> List, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecConvenioHist_ConsultaLista", ref dr, Parametros, Valores);
                ...
                while (dr.Read())
                {
                    ...
                    c.Id_PCH = Convert.ToInt32(dr["Id_PCH"]);
                    c.Id_PC = Convert.ToInt32(dr["Id_PC"]);
                    c.PC_NoConvenio = dr["PC_NoConvenio"].ToString();
                    c.PC_Nombre = dr["Pc_Nombre"].ToString();
                    c.Id_Cat = Convert.ToInt32(dr["Id_Cat"]);
                    c.Cat_DescCorta = dr["Cat_DescCorta"].ToString();
                    c.Id_ULider = Convert.ToInt32(dr["Id_ULider"]);
                    c.Id_UEjecutivo = Convert.ToInt32(dr["Id_UEjecutivo"]);
                    c.PC_Margen = Convert.ToDouble(dr["Pc_Margen"]);
                    c.PC_Notas = dr["PC_Notas"].ToString();
                    c.PC_FechaMod = Convert.ToDateTime(dr["PC_FechaMod"]);

                    c.PC_NombreArchivoPDF = dr["PC_NombreArchivoPDF"].ToString();
                    c.PC_ArchivoPDF = dr["PC_ArchivoPDF"].ToString();
                    c.PC_FechaInicio = dr["PC_FechaInicio"].ToString() == "" ? DateTime.Now : Convert.ToDateTime(dr["PC_FechaInicio"]);
                    c.PC_NoConvenioFuturo = dr["PC_NoConvenioFuturo"].ToString();
                    c.PC_DesplegarMatrizCN = dr["PC_DesplegarMatrizCN"].ToString() == "" ? 0 : Convert.ToInt32(dr["PC_DesplegarMatrizCN"]);
                    c.PC_Nombreusuario = dr["PC_Nombreusuario"].ToString();
        ...
        public void ConsultaConvenioDetHis(Convenio conv, ref DataTable dt, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConvHis_ConsultaListaDet", ref ds, Parametros, Valores);
                ...
                //    //JFCV Convenio
                //    c.PCD_PrecioPAAAEspProv = dr["PCD_PrecioPAAAEspProv"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioPAAAEspProv"]);
                //    c.PCD_PrecioVentaConvenio = dr["PCD_PrecioVentaConvenio"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_PrecioVentaConvenio"]);
                //    c.PCD_Margen = dr["PCD_Margen"].ToString() == "" ? (Double?)null : Convert.ToDouble(dr["PCD_Margen"]);
                //    c.Id_PC = Convert.ToInt32(dr["Id_PC"]);
                //    c.Id_PCH = Convert.ToInt32(dr["Id_PCH"]);
                //    c.PCD_FechaMod = dr["Pc_FechaMod"].ToString() == "" ? (DateTime?)null : Convert.ToDateTime(dr["Pc_FechaMod"]);
                //    c.PCD_UsuarioResponsable = dr["UsuarioMod"].ToString();
        ...
        public void ConsultaProConvSucursal(int Id_PC, ref List<Convenio> List, string Conexion)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    conv.PCD_Gral = Convert.ToBoolean(dr["PCD_Gral"]);
                    List.Add(conv);
                }
            ...
            try
            {
                ...
                foreach (SolConvenioDet s in List)
                {
                    ...
                    object[] Valores = {
                                            ...
                                            sol.Id_PC ,
                                            s.Id_Cd,
                                            Convert.ToInt32(s.Id_Cte.ToString()),
                                            s.Sol_CteNombre ,
                                            s.Sol_UsuFinal,
                                            s.Id_Ter ,
                                            s.SolTer_Nombre
        ...
        public void ConvenioSolicitud_Consulta(int Id_Sol, ref SolConvenio sol, string Conexion)
        {
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    sol.Id_PC = Convert.ToInt32(dr["Id_PC"].ToString());
        ...
        public void ConvenioSolicitud_ConsultaAt(string Sol_Unique, ref SolConvenio sol, string Conexion)
        {
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    sol.PC_Nombre = dr["PC_Nombre"].ToString();
                    sol.Cat_DescCorta = dr["Cat_DescCorta"].ToString().Trim();
                    sol.Id_PC = Convert.ToInt32(dr["Id_PC"]);
                    sol.Id_UAtendio = Convert.ToInt32(dr["Id_UAtendio"]);
                    sol.Sol_NombreAtendio = dr["Sol_NombreAtendio"].ToString();
                    sol.Sol_FechaAtendio = Convert.ToDateTime(dr["Sol_FechaAtendio"]);
                    sol.OrigenSolicitud = Convert.ToInt32(dr["OrigenSolicitud"]);
        ...
        public void ConvenioSolicitudMatriz_ConsultaDetAt(int Id_Sol, ref  List<SolConvenioDet> List, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConv_SolicitudConsultaMatrizDetAt", ref dr, Parametros, Valores);
        ...
        public void ConvenioSolicitud_Atender(int Id_Sol, List<SolConvenioDet> List, int id_Uatendio, string sol_NombreAtendio, ref int Verificador, string Conexion)
        {
            try
            {
                ...
                if (Verificador == -1)
                {
                    string[] Parametros2 = { "@Id_Sol", "@Id_UAtendio", "@Sol_NombreAtendio" };
                    object[] Valores2 = { Id_Sol, id_Uatendio, sol_NombreAtendio };
        ...
        public void Convenio_ConsultaVinculados(int Id_PC, int Id_CD, ref List<SolConvenioDet> List, string Conexion)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    s.Id_UAtendio = Convert.ToInt32(dr["Id_UAtendio"]);
                    s.Sol_NombreAtendio = dr["Sol_NombreAtendio"].ToString();
                    s.OrigenSolicitud = Convert.ToInt32(dr["OrigenSolicitud"]);
                    s.OrigenSolicitudString = dr["OrigenSolicitudString"].ToString();
                    List.Add(s);
                }
        ...
        public void Convenio_Concesionarioshabilitados(Convenio conv, ref DataTable dt, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConvenioConcesionario_Consulta", ref ds, Parametros, Valores);
                dt = ds.Tables[0];
        ...
        public void Convenio_ConsultaClienteVinculado(ConvenioDet conv, ref ConvenioDet convdet, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConv_ConsultaClienteEsViculado", ref dr, Parametros, Valores);
                ...
                while (dr.Read())
                {
                    ...
                    convdet.PC_NoConvenio = dr["PC_NoConvenio"].ToString();
                    convdet.PC_Nombre = dr["PC_Nombre"].ToString();
                    convdet.Prd_Descripcion = dr["CDI"].ToString();
        ...
        public void ConvenioSolicitud_ValidaNoExistaClienteDet(SolConvenio sol, List<SolConvenioDet> List, ref int Verificador, ref string mensajeerror, string Conexion)
        {
            try
            {
                mensajeerror = "";
                ...
                foreach (SolConvenioDet s in List)
                {
                    ...
                    sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConv_Solicitud_ValidaInsertarDet", ref Verificador, Parametros, Valores);
                    cd_datos.LimpiarSqlcommand(ref sqlcmd);

                    if (Verificador > 0)
                    {
                        mensajeerror = mensajeerror + "<BR> No. de cliente: <b>" + s.Id_Cte.ToString() + "</b>";
                        mensajeerror = mensajeerror + "<BR> Nombre de cliente: <b>" + s.Sol_CteNombre + "</b>";
        ...
        public void ConsultarInformacionConvenio(int Id_emp, int Gestion_Tipo, ref  List<CapGestionPrecioInformacion> List, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                string[] Parametros = { "@Id_Emp", "@Gestion_Tipo" };
                object[] Valores = { Id_emp, Gestion_Tipo };

                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConvenioInformacion_Consultar", ref dr, Parametros, Valores);

                CapGestionPrecioInformacion sol;
                while (dr.Read())
                {
                    sol = new CapGestionPrecioInformacion();

                    if (dr["Gestion_NombreArchivo"] != null)
                    {
                        sol.Gestion_NombreArchivo = dr["Gestion_NombreArchivo"].ToString();
                        sol.Gestion_RutaArchivo = dr["Gestion_RutaArchivo"].ToString();
                        sol.Gestion_Descripcion = dr["Gestion_Descripcion"].ToString();
                    }
                    sol.Gestion_Tipo = Convert.ToInt32(dr["Gestion_Tipo"]);
                    sol.Id_Emp = 1;//Convert.ToInt32(dr["Id_Emp"]);
                    List.Add(sol);
                }
        ...
        public void ConsultarConvenioCorreos(int id_pc, int Gestion_Tipo, ref  List<ConvenioCorreos> List, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                string[] Parametros = { "@Id_PC", "@Gestion_Tipo" };
                object[] Valores = { id_pc, Gestion_Tipo };

                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConvenioDestinatarios_Consultar", ref dr, Parametros, Valores);

                ConvenioCorreos sol;
                while (dr.Read())
                {
                    sol = new ConvenioCorreos();

                    if (dr["U_Nombre"] != null)
                    {
                        sol.Id_Cd = Convert.ToInt32(dr["Id_Cd"]);
                        sol.Id_rik = Convert.ToInt32(dr["Id_rik"]);
                        sol.Id_U = Convert.ToInt32(dr["Id_U"]);
                        sol.Id_Ter = Convert.ToInt32(dr["Id_Ter"]);

                        sol.U_Nombre = dr["U_Nombre"].ToString();
                        sol.U_correo = dr["U_correo"].ToString();
                        sol.Cd_Nombre = dr["Cd_Nombre"].ToString();


                        sol.NombreTerritorio = dr["Nombre_Territorio"].ToString();
                    }
                    sol.Id_Emp = 1;//Convert.ToInt32(dr["Id_Emp"]);
                    List.Add(sol);
        ...
        public void ConsultaConvenioBonificacion(ConvenioBonificacion conv, ref DataTable dt, string Conexion)
        {
            try
            {
                CD_Datos cd_datos = new CD_Datos(Conexion);
                ...
                string[] Parametros = { "@FechaIni", "@FechaFin", "@Id_Categoria", "@Version", "@Id_CdiInicial", "@Id_CdiFinal" };
                object[] Valores = { conv.Filtro_FechaInicial, conv.Filtro_FechaFinal, conv.Filtro_Id_Cat, conv.Filtro_Version, conv.Filtro_Id_Cdinicial, conv.Filtro_Id_Cdfinal };

                SqlCommand sqlcmd = cd_datos.GenerarSqlCommand("spProPrecioConv_ConsultaBonificacionCentral", ref ds, Parametros, Valores);
