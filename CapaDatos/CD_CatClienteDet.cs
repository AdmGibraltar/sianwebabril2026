    ...
    public class CD_CatClienteDet
    {
        ...
        public CatClienteDet InsertarBasico(int idEmp, int idCd, int idCte, int idRik, int idTer, int idSeg, double vpo, string cadenaDeConexionEF)
        {
            ...
            using (sianwebmty_gEntities ctx = new sianwebmty_gEntities(cadenaDeConexionEF))
            {
                ...
                catClienteDet.Cte_Garantia = null;
                catClienteDet.Cte_Activo = true;
                catClienteDet.TerritorioPadre = true;
        ...
        public List<eTerritoriosCte> ObtenerTerritoriosPorCte(int Id_Emp, int Id_Cd, int Id_Cte, string Conexion)
        {
            ...
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCatClienteDet_TerrsByCte", ref dr, Parametros, Valores);

                while (dr.Read())
                {
                    eTerritoriosCte obj = new eTerritoriosCte();
                    obj.Id_Terr = dr.GetValue(dr.GetOrdinal("Id_Ter")).ToString();
                    obj.Cte_Potencial = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Cte_Potencial")));
        ...
        // 16 Ene  2018 
        public List<eTerritoriosCte> ObtenerTerritoriosPorCte_UnionRik(int Id_Emp, int Id_Cd, int Id_Cte, int Id_Rik, string Conexion)
        {
            List<eTerritoriosCte> lst = new List<eTerritoriosCte>();
            ...
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCatClienteDet_Terrs_CteUnionRik", ref dr, Parametros, Valores);
                ...
                while (dr.Read())
                {
                    ...
                    obj.Id_Terr = dr.GetValue(dr.GetOrdinal("Id_Ter")).ToString();
                    obj.Ter_Nombre = dr.GetValue(dr.GetOrdinal("Ter_Nombre")).ToString();
                    obj.Id_Rik = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Cte_Potencial")));
                    obj.Id_Seg = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Seg")));
                    obj.Ter_Activo = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Ter_Activo")));
                    obj.Ter_Asociado = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Ter_Asociado")));
                    obj.TerDeRik = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("TerDeRik")));
                    obj.Cte_Potencial = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Cte_Potencial")));
