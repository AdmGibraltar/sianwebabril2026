...
namespace CapaDatos
{
    public class CD_CrmValuacionOportunidades
    {
        ...
        public List<CrmValuacionOportunidade> ConsultarPorValuacion_Ver2(int idEmp, int idCd, int idCte, int idRik, int idValuacion, string conexionEF)
        {
            List<CrmValuacionOportunidade> result = null;
            using (sianwebmty_gEntities ctx = new sianwebmty_gEntities(conexionEF))
            {
                var proyectos = (from vp in ctx.CrmValuacionOportunidades
                                 where vp.Id_Emp == idEmp && vp.Id_Cd == idCd && vp.Id_Cte == idCte
                                  && vp.Id_Rik == idRik
                                 && vp.Id_Val == idValuacion
                                 select vp).ToList();
                result = proyectos;
            }
            return result;
