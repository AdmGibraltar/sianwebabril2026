...
namespace CapaDatos
{
    public class CD_CapNotaCargo
    {
        public void ConsultaNotaCargo(ref NotaCargo notaCargo, string Conexion)
        {
            try
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    notaCargo.Nca_FolioFiscal = dr.IsDBNull(dr.GetOrdinal("nca_foliofiscal")) ? string.Empty : dr.GetValue(dr.GetOrdinal("nca_foliofiscal")).ToString();
                ...
                while (dr.Read())
                {
                    ...
                    notaCargoDet.Id_Prd = Convert.ToInt64(dr.GetValue(dr.GetOrdinal("Id_Prd")));
        ...
        public List<NotaCargo> ConsultaProductosNotaCargo(ref NotaCargo notaCargo, string Conexion)
        {
            try
            {
                ...
                {//Id_Clp, c.Id_Prd, c.Clp_descripcion
                    notaCargo = new NotaCargo();
                    notaCargo.Id_Prd = Convert.ToInt64(dr.GetValue(dr.GetOrdinal("Id_Prd")));
                    listaNotaCargo.Add(notaCargo);
                }
                CapaDatos.LimpiarSqlcommand(ref sqlcmd);
        ...
        public void ConsultaNotaCargoMin(ref NotaCargo entNotaCargo, string Conexion)
        {
            try
            {
                SqlDataReader dr = null;
                ...
                string[] Parametros = { "@Id_Emp", "@Id_Cd", "@Id_Nca" };
                object[] Valores = { entNotaCargo.Id_Emp, entNotaCargo.Id_Cd, entNotaCargo.Id_Nca };

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCapNotaCargo_Consultar_min", ref dr, Parametros, Valores);
                if (dr.Read())
                {
                    entNotaCargo.Id_Emp = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Emp")));
                    entNotaCargo.Id_Cd = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cd")));
                    entNotaCargo.Id_Nca = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Nca")));

                    entNotaCargo.Nca_Subtotal = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Nca_Subtotal")));
                    entNotaCargo.Nca_Iva = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Nca_Iva")));
                    entNotaCargo.Nca_FPago = dr.GetValue(dr.GetOrdinal("Nca_FPago")).ToString();
                    entNotaCargo.Nca_CteRfc = dr.GetValue(dr.GetOrdinal("Cte_FacRfc")).ToString();
                    if (Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Id_NcaSerie"))))
                        entNotaCargo.Id_NcaSerie = string.Empty;
                    else
                        entNotaCargo.Id_NcaSerie = dr.GetValue(dr.GetOrdinal("Id_NcaSerie")).ToString();
                    if (Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Nca_XML"))))
                    {
                        entNotaCargo.Nca_Xml = null;
                    }
                    else
                    {
                        entNotaCargo.Nca_Xml = (object)dr.GetValue(dr.GetOrdinal("Nca_XML"));
                    }
                }
