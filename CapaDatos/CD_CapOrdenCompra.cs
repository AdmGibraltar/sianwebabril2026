...
namespace CapaDatos
{
    public class CD_CapOrdenCompra
    {
        ...
        public List<CapaEntidad.eListaGenerica> spCompraLocalPedidosProducto_Lista(int Solicitud, Int64 Id_Producto, string Conexion)
        {
            ...
            try
            {
                ...
                object[] Valores = { Solicitud, Id_Producto };
                ...
                while (dr.Read())
                {
                    ...
                    obj.Id = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Pedido")));
                    obj.Descripcion = dr.GetValue(dr.GetOrdinal("Ord_Fecha")).ToString();
        ...
        public void InsertarOrdenCompra(ref OrdenCompra ordenCompra, string Conexion, string ValidaParametrosOC, ref int verificador)
        {
            CapaDatos.CD_Datos CapaDatos = new CapaDatos.CD_Datos(Conexion);
            try
            {
                ...
                if (ValidaParametrosOC == "S")
                {
                    // Validador y Spliter de la sOC
                    string HuboSplit = "";
                    string strverificador = "";
                    string[] ParametrosVal = {
                                        "@Id_Emp",
                                        "@Id_Cd",
                                        "@Id_Ord"
                                      };
                    ...
                    sqlcmd = CapaDatos.GenerarSqlCommand("spCapOrdCompra_ValidaCandados", ref strverificador, ParametrosVal, ValoresVal);
                    HuboSplit = strverificador;
        ...
        public void ValidarOrdenCompraExtraordinaria(ref OrdenCompra ordenCompra, string Conexion, ref int verificador, Sesion session, string strURL)
        {
            CapaDatos.CD_Datos CapaDatos = new CapaDatos.CD_Datos(Conexion);
            try
            {
                StringBuilder strHTMLProductos = new StringBuilder();
                double dlMontoOrden = 0;
                double dlMonto = 0;
                strHTMLProductos.Append("<div align='center'>");
                strHTMLProductos.Append("<table>");
                strHTMLProductos.Append("<tr><td colspan='2'><b><font face='Tahoma' size='2'>");
                strHTMLProductos.Append("Se ha colocado una solicitud de autorización para la orden de compra @@OrdenCompra por el monto de: $ @@MontoOrden");
                strHTMLProductos.Append("</td></tr><tr><td><b><font face='Tahoma' size='2'>");
                strHTMLProductos.Append("Centro de distribución:  " + session.Id_Cd_Ver + " - " + session.Cd_Nombre);
                strHTMLProductos.Append("</td></tr><tr><td><b><font face='Tahoma' size='2'>");
                strHTMLProductos.Append("Solicitó : " + session.Id_U + " - " + session.U_Nombre);
                strHTMLProductos.Append("</td></tr><tr><td colspan='2'>");

                strHTMLProductos.Append("<center><br>");
                strHTMLProductos.Append("<a href='" + strURL + "ProOrdenCompra_Autorizacion.aspx?Id=@@OrdenCompra&Accion=2&email=1&PermisoGuardar=true&PermisoModificar=true&PermisoEliminar=true&PermisoImprimir=true'" + ">");
                strHTMLProductos.Append("Solicitud de autorización de ordenes de compra</a></font></center>");

                strHTMLProductos.Append("</td></tr><tr><td colspan='2'><br>");
                strHTMLProductos.Append("<center><br>Productos Incluidos en la Orden<br></center>");
                strHTMLProductos.Append("</td></tr><tr><td colspan='2'><table>");
                strHTMLProductos.Append("<tr><td><center><b>Producto</b></center></td><td><center><b>Cantidad</b></center></td><td><center><b>Total</b></center></td></tr>");
                foreach (OrdenCompraDet ordenCompraDet in ordenCompra.ListOrdenCompra)
                {
                    dlMonto = ordenCompraDet.Ord_Cantidad * ordenCompraDet.ProductoPrecio.Prd_Pesos;
                    strHTMLProductos.Append("<tr><td>" + ordenCompraDet.Id_Prd.ToString() + " " + ordenCompraDet.Producto.Prd_Descripcion);
                    strHTMLProductos.Append("&nbsp;&nbsp;&nbsp;</td><td><center>" + ordenCompraDet.Ord_Cantidad + " unidades");
                    strHTMLProductos.Append("</center></td><td> &nbsp;&nbsp;&nbsp;$" + string.Format("{0:#.00}", Convert.ToDecimal(dlMonto.ToString())));
                    strHTMLProductos.Append("</td></tr>");
                    dlMontoOrden = dlMontoOrden + dlMonto;
                    dlMonto = 0;
                }
                strHTMLProductos.Append("</table></td></tr></table></div>");

                string body = strHTMLProductos.ToString();
                ...
                SqlCommand sqlcmd = new SqlCommand();
                int iRetorno = 0;
                sqlcmd = CapaDatos.GenerarSqlCommand("spSendaMailPruebaOC", ref iRetorno, Parametros, Valores);
                verificador = iRetorno;
