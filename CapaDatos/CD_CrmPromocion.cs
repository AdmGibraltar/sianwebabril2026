...
namespace CapaDatos
{
    public class CD_CrmPromocion
    {
         ...
         * 
         * 26JUL-2019 RFH
         * 13ENE-2020 RFH - UPDATE - Se agrega RIK 
         * 
         * */
        public List<CrmPromociones> ConsultaCatPromocion_Ver2(int Id_Emp, int Id_Cd, int Id_Cte, int Id_Op, int Id_Rik, string Conexion, bool vTerritorio)
        {
            int validador;
            validador = 0;
            List<CrmPromociones> lst = new List<CrmPromociones>();
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand(vTerritorio ? "spCatCrmPromocion_Consulta_Ver2VTerritorio" : "spCatCrmPromocion_Consulta_Ver2", ref dr, Parametros.ToArray(), Valores.ToArray());

                while (dr.Read())
                {
                    CrmPromociones obj = new CrmPromociones();
                    obj.Ids = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Op")));
                    obj.Id = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Op")));
                    obj.Id_Cte = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cte")));
                    obj.Cds = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cd")));
                    obj.Id_Cte = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cte")));
                    obj.Representante = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Rik")));
                    obj.NombreCte = dr.GetValue(dr.GetOrdinal("Cte_NomComercial")).ToString();
                    obj.Id_Ter = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Ter")));
                    obj.Segmento = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Seg")));
                    obj.Cli_VPObservado = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Cli_VPObservado")));
                    obj.Descripcion = dr.GetValue(dr.GetOrdinal("Descripcion")).ToString();

                    obj.Analisis = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Analisis"))) ? "" : Convert.ToDateTime(dr.GetValue(dr.GetOrdinal("Analisis"))).ToString("dd/MM/yyyy");
                    obj.Presentacion = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Presentacion"))) ? "" : Convert.ToDateTime(dr.GetValue(dr.GetOrdinal("Presentacion"))).ToString("dd/MM/yyyy");
                    obj.Negociacion = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Negociacion"))) ? "" : Convert.ToDateTime(dr.GetValue(dr.GetOrdinal("Negociacion"))).ToString("dd/MM/yyyy");
                    obj.Cierre = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Cierre"))) ? "" : Convert.ToDateTime(dr.GetValue(dr.GetOrdinal("Cierre"))).ToString("dd/MM/yyyy");
                    obj.Cancelacion = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Cancelacion"))) ? " " : (string)dr.GetValue(dr.GetOrdinal("Cancelacion"));
                    obj.FechaCancelacion = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("FechaCancelacion"))) ? " " : Convert.ToDateTime(dr.GetValue(dr.GetOrdinal("FechaCancelacion"))).ToString("dd/MM/yyyy");

                    obj.Avances = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Avances")));
                    obj.Estatus = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Estatus")));
                    obj.Uen_Descrip = dr.GetValue(dr.GetOrdinal("Uen_Descrip")).ToString();
            ...
            SqlCommand sqlcmd = CD_Datos.GenerarSqlCommand("spCatCrmPromocion_Consulta_Ver2", ref dr, Parametros, Valores, icdCtx);
            int Avances;
            CrmPromociones catPromociones;
            while (dr.Read())
            {
