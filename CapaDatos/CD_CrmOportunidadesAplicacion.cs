...
namespace CapaDatos
{
    public class CD_CrmOportunidadesAplicacion
    {
        ...
        public void Insertar(Sesion sesion, int idEmp, int idCd, int idOp, int idApl, decimal vpo, ref int validador, ICD_Contexto idcCtx)
        {
            ...
            try
            {
                ...
                sqlcmd = transaction.Connection.CreateCommand() as SqlCommand;
                sqlcmd.Transaction = (System.Data.SqlClient.SqlTransaction)transaction;
                //                                      spCatCrmPromocion_Insertar
                sqlcmd = CD_Datos.GenerarSqlCommand("spCrmOprotunidadApl_Insertar", ref validador, Parametros, Valores, transaction.Connection, sqlcmd);
                sqlcmd.Dispose();
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public List<eCrmOportunidadesAplicacion> ConsultarPorOportunidad_Ver2(int Id_Emp, int Id_Cd, int Id_Rik, int Id_Op, string conexion)
        {
            List<eCrmOportunidadesAplicacion> lst = new List<eCrmOportunidadesAplicacion>();
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCrmOportunidadesAplicacion_Consulta", ref dr, Parametros.ToArray(), Valores.ToArray());
                while (dr.Read())
                {
                    eCrmOportunidadesAplicacion obj = new eCrmOportunidadesAplicacion();
