        ...
        public void Lista(Rastreo rastreo, List<Rastreo> list, string Conexion, int tipoBusqueda)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    rastreo.Id_Emp2 = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Id_Emp"))) ? -1 : Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Emp")));
                    rastreo.Id_Cd2 = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Id_Cd"))) ? -1 : Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cd")));
                    rastreo.Id_Cte = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Id_Cte"))) ? -1 : Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Cte")));
                    rastreo.Id_Det = Convert.IsDBNull(dr.GetValue(dr.GetOrdinal("Id_Det"))) ? -1 : Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Det")));
                    list.Add(rastreo);
                }
