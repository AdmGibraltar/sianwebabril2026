...
namespace CapaDatos
{
    public class CD_CatAplicacion
    {
        ...
        // 27 JUL 2021 RFH
        public List<Aplicacion> spObtenerTodasLasAplicacionesDisponibles
            (int Id_Emp, int Id_Cd, int Id_Uen, int Id_Seg, int Id_Area, int Id_Sol, int Id_Op, int Id_Cte, string Conexion)
        {
            ...
            try
            {
                ...
                string[] Parametros = { "@Id_Emp", "@Id_Cd", "@Id_Uen", "@Id_Seg", "@Id_Area", "@Id_Sol", "@Id_Op", "@Id_Cte" };
                object[] Valores = { Id_Emp, Id_Cd, Id_Uen, Id_Seg, Id_Area, Id_Sol, Id_Op, Id_Cte };

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spObtenerTodasLasAplicacionesDisponibles", ref dr, Parametros, Valores);

                while (dr.Read())
                {
                    Aplicacion A = new Aplicacion();
                    A.Id_Apl = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Apl")));
                    A.Id_Sol = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Sol")));
                    A.Id_Area = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Area")));
                    A.Apl_Descripcion = Convert.ToString(dr.GetValue(dr.GetOrdinal("Apl_Descripcion")));
                    A.Id_Seg = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Seg")));
                    A.Id_Uen = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("Id_Uen")));
                    A.Apl_Potencial = Convert.ToDouble(dr.GetValue(dr.GetOrdinal("Apl_Potencial")));
                    A.Apl_Limpieza = Convert.ToBoolean(dr.GetValue(dr.GetOrdinal("Apl_Limpieza")));
                    A.Estatus = Convert.ToBoolean(dr.GetValue(dr.GetOrdinal("Apl_Activo")));
                    A.OA_Id_Apl = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("OA_Id_Apl")));
