...
// 1 Mar 2019 - RFC - Se agrega funcion EliminarFisicamente
namespace CapaDatos
{
    public class CD_CrmOportunidad
    {
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
        public int AjustaPME(int IdEmp, int IdCDI, int IdCte, int IdOp, int IdTer, int IdSeg, float valor, string Conexion)
        {
            int iResult = 0;
            try
            {
                ...
                Parametro("@Id_Cte", IdCte);
                Parametro("@Id_Op", IdOp);
                Parametro("@Id_Ter", IdTer);
                Parametro("@Id_Seg", IdSeg);
                Parametro("@Valor", valor);


                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRMClientedetalle_PMEModificar", ref dr, Parametros.ToArray(), Valores.ToArray());

                if (dr.HasRows)
                {
                    dr.Read();
                    iResult = Convert.ToInt32(dr.GetOrdinal("ESTADO"));
                }
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
        public int EliminarFisicamente(int Id_Emp, int Id_Cd, int Id_Op, string Conexion)
        {
            int iResult = 0;
            try
            {
                ...
                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRMProyecto_Eliminar", ref dr, Parametros.ToArray(), Valores.ToArray());
        ...
        public IEnumerable<CrmOportunidade> ConsultarSoloCRMIIPorRik_Ver2(int idEmp, int idCd, int idRik, ICD_Contexto idcCtx)
        {
            IEnumerable<CrmOportunidade> result = null;
            sianwebmty_gEntities ctx = ((ICD_Contexto<sianwebmty_gEntities>)idcCtx).Contexto;
            var oportunidades = from o in ctx.CrmOportunidades
                                join p in ctx.CrmProspectoes
                                on o.Id_Cte equals p.Id_Cte
                                where o.Id_Usu == idRik && p.Id_Rik == idRik && o.Id_Emp == idEmp && o.Id_Cd == idCd && (o.CrmOp_OrigenCRMII != null && o.CrmOp_OrigenCRMII == true && o.Id_Ter > 0 && o.Id_Seg > 0 && o.ID_Area > 0)
                                select o;
            result = oportunidades;
            result = result.ToList();
            return result;
        ...
        /*
         * 
         *  JUN5 - 2019 RFH - Actualiza El Estatus del Proyecto en la Entidad.
         * 
         * */
        public int CrmOportunidades_UpdateEstatus(int Id_Emp, int Id_Cd, int Id_Val, int Id_Op, int Estatus, string conexion)
        {
            int iAfectedRows = 0;
            eResultadoValuacion eRV = new eResultadoValuacion();
            ...
            try
            {
                ...
                Parametro("@Id_Emp", Id_Emp);
                Parametro("@Id_Cd", Id_Cd);
                Parametro("@Id_Val", Id_Val);
                Parametro("@Id_Op", Id_Op);
                Parametro("@Estatus", Estatus);

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRM_CrmOportunidades_UpdateEstatus", ref dr, Parametros.ToArray(), Valores.ToArray());

                if (dr.HasRows)
                {
                    dr.Read();
                    iAfectedRows = Convert.ToInt32(dr.GetValue(dr.GetOrdinal("AfecteRows"))); ;
                }
        ...
        /// <summary>
        /// Ejecuta SP para cancelacion de proyecto CRM  
        /// 5AGO-2019 RFH
        /// </summary>
        /// <param name="Id_Emp"></param>
        /// <param name="Id_Cd"></param>
        /// <param name="Id_Op"></param>
        /// <param name="Id_Causa"></param>
        /// <param name="conexion"></param>
        /// <returns></returns>
        public int CrmOportunidades_Cancelar(int Id_Emp, int Id_Cd, int Id_Op, int Id_Cte, int Id_Causa, string conexion)
        {
            int iAfectedRows = 0;
            eResultadoValuacion eRV = new eResultadoValuacion();
            ...
            try
            {
                ...
                Parametro("@Id_Causa", Id_Causa);

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRM_CrmOportunidades_Cancelar", ref dr, Parametros.ToArray(), Valores.ToArray());
        ...
        /// <summary>
        /// ACTUALIZA A PROYECTO ACEPTADO EN CRM 
        /// 5AGO-2019 RFH - Creacion 
        /// </summary>
        /// <param name="Id_Emp"></param>
        /// <param name="Id_Cd"></param>
        ...
        /// <param name="conexion"></param>
        /// <returns></returns>
        public int CrmOportunidades_UpdateEstatus(
            int Id_Emp, int Id_Cd, int Id_Val, int Id_Op, int Id_Cte,
            int Id_Rik, string Vap_Estatus, int Vap_Estatus2, int Estatus, string conexion)
        {
            int iAfectedRows = 0;
            eResultadoValuacion eRV = new eResultadoValuacion();
            ...
            try
            {
                ...
                Parametro("@Vap_Estatus", Vap_Estatus);
                Parametro("@Vap_Estatus2", Vap_Estatus2);
                Parametro("@Estatus", Estatus);

                SqlCommand sqlcmd = CapaDatos.GenerarSqlCommand("spCRM_CapValProyecto_Update", ref dr, Parametros.ToArray(), Valores.ToArray());
        ...
        // 2 Oct 2018 RFH 
        // 
        public CrmOportunidade ConsultarPorId_Context(int idEmp, int idCd, int idOp, ICD_Contexto icdCtx)
        {
            CrmOportunidade resultado = null;
            try
            {
