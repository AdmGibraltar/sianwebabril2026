...
namespace CapaNegocios
{
    public class CN_CrmOportunidad
    {
        ...
        {
            CD_CrmOportunidad CO = new CD_CrmOportunidad();
            return CO.CrmOportunidades_UpdateEstatus(Id_Emp, Id_Cd, Id_Val, Id_Op, Estatus, conexion);
        }
        ...
        /// <returns></returns>
        public int CrmOportunidades_Cancelar(
            int Id_Emp, int Id_Cd, int Id_Op, int Id_Cte, int Id_Causa, string conexion)
        {
            CD_CrmOportunidad CO = new CD_CrmOportunidad();
            return CO.CrmOportunidades_Cancelar(Id_Emp, Id_Cd, Id_Op, Id_Cte, Id_Causa, conexion);
        }

        /// <summary>
        ///  Actualiza el proyecto a Aceptado en CRM 
        ...
        /// <param name="Vap_Estatus"></param>
        /// <param name="Vap_Estatus2"></param>
        ...
        {
            CD_CrmOportunidad CO = new CD_CrmOportunidad();
            return CO.CrmOportunidades_UpdateEstatus(
                Id_Emp, Id_Cd, Id_Val, Id_Op, Id_Cte,
                Id_Rik, Vap_Estatus, Vap_Estatus2, Estatus, conexion);
        }
        ...
        public IEnumerable<CrmOportunidade> ObtenerSoloCRMIIPorRik_Ver2(Sesion sesion, IBusinessTransaction ibt)
        {
            CD_CrmOportunidad cdCrmOportunidad = new CD_CrmOportunidad();
            return cdCrmOportunidad.ConsultarSoloCRMIIPorRik_Ver2(sesion.Id_Emp, sesion.Id_Cd, sesion.Id_Rik, ibt.DataContext);
        }
        ...
        // 2 Oct 2018 RFH
        //
        public CrmOportunidade ObtenerPorId_Context(int Id_Emp, int Id_Cd, int idOp, ICD_Contexto icdCtx)
        {
            CD_CrmOportunidad cdCrmOportunidad = new CD_CrmOportunidad();
            return cdCrmOportunidad.ConsultarPorId_Context(Id_Emp, Id_Cd, idOp, icdCtx);
        }
        ...
        // 1 Oct 2018 RFH
        // 
        public CapaEntidad.eCapValProyecto Consulta_ResultadoValuacion(
            int Cte, int Id_Op, int Val, Sesion sesion, IBusinessTransaction ibt)
        {
            ...
            Obj = cnPE.spCRMCapValProyecto(sesion.Id_Emp, sesion.Id_Cd, Cte, sesion.Id_Rik, Id_Op, Val, sesion);
            return Obj;
        }

        public CapaEntidad.eCapValProyecto Consulta_ResultadoValuacion_Ver2(
            int Cte, int Id_Op, int Val, Sesion sesion, IBusinessTransaction ibt)
        {
            CapaEntidad.eCapValProyecto Obj = new CapaEntidad.eCapValProyecto();
            CN_CrmPropuestaEconomica cnPE = new CN_CrmPropuestaEconomica();
        ...
        public int AjustaPME(int IdEmp, int IdCDI, int IdCte, int IdOp, int IdTer, int IdSeg, float valor, Sesion sesion)
        {
            CD_CrmOportunidad CD = new CD_CrmOportunidad();
            return CD.AjustaPME(IdEmp, IdCDI, IdCte, IdOp, IdTer, IdSeg, valor, sesion.Emp_Cnx);
        }

        // 1 Mar 2019
        public int EliminarFisicamente(int Id_Emp, int Id_Cd, int Id_Op, string ClaveConfirmacion, Sesion sesion)
        {
            CD_CrmOportunidad CD = new CD_CrmOportunidad();
            return CD.EliminarFisicamente(Id_Emp, Id_Cd, Id_Op, sesion.Emp_Cnx);
        }
        ...
        */
        ...
        public ResultadosValuacion CalcularResultadoValuacion(CrmPromociones proyecto, Sesion sesion, IBusinessTransaction ibt)
        {
                ...
                /*.Select(p =>
            {
                ...
                if (parametros2 == null)
                {
                    int IdVal1 = proyectoValuaciones.Id_Val;
                    throw new CapValProyecto_ParametrosIndefinidosException();
                }

                if (parametros == null)
                {
                    int IdVal2 = proyectoValuaciones.Id_Val;
