...
namespace CapaNegocios
{
    public class CN_CatTerritorios
    {
        ...
        // NOV14-2019 RFH
        public void ConsultaTerritorios_Ver2(int PageNumber, int PageSize,
            Territorios territorio, string TextoBuscar, int TipoRepresentante, int Ter_Activo,
            string Conexion, ref List<Territorios> List, bool vTerritorio)
        {
            CD_CatTerritorios claseCapaDatos = new CD_CatTerritorios();
            claseCapaDatos.ConsultaTerritorios_Ver2(PageNumber, PageSize,
                territorio, TextoBuscar, TipoRepresentante, Ter_Activo,
                Conexion, ref List, vTerritorio);
        }

        // NOV20-2019 RFH
        public Territorios ConsultaTerritorio(
            int Id_Emp, int Id_Cd, string Id_Ter, string Conexion)
        {
            CD_CatTerritorios claseCapaDatos = new CD_CatTerritorios();
            return claseCapaDatos.ConsultaTerritorios_ById(Id_Emp, Id_Cd, Id_Ter, Conexion);
        }
        ...
        {
            CD_CatTerritorios claseCapaDatos = new CD_CatTerritorios();
            return claseCapaDatos.ConsultaTerritoriosCliente_ById(Id_Emp, Id_Cd, Id_Ter, ParametroBusqueda, Conexion);
        }
        ...
        public void ConsultaTerritorios(ref Territorios catterr, string Conexion)
        {
            try
            {
                ...
                claseCapaDatos.ConsultaTerritorios(ref catterr, Conexion, sesion.VersionTerritorio);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        /// <summary>
        /// Version 2 del metodo arriba con SPs
        /// 22AGO-2019
        /// </summary>
        /// <param name="s"></param>
        /// <param name="idCrmProspecto"></param>
        /// <returns></returns>
        public List<eTerritorio> ObtenerTerritoriosPorProspecto_Ver2(Sesion s, int idCrmProspecto)
        {
            CD_CatTerritorios cdCatTerritorios = new CD_CatTerritorios();
            CN_CrmProspecto cnCrmProspecto = new CN_CrmProspecto();
            var prospecto = cnCrmProspecto.ObtenerProspecto_Ver2(
                s.Id_Emp,
                s.Id_Cd,
                s.Id_Rik,
                idCrmProspecto,
            ...
            return cdCatTerritorios.ConsultarTerritoriosPorClienteYRIK_Ver2(s.Id_Emp, s.Id_Cd, prospecto.Id_Cte, s.Id_Rik, s.Emp_Cnx);
        }
        ...
        //version para SP  
        public List<eTerritorio> spCatTerritorio_PorRik_Ver2(int Anio, int Mes, int idEmp, int idCd, int idRik, Sesion sesion)
        {
            CD_CatTerritorios cdCatTerritorios = new CD_CatTerritorios();
            return cdCatTerritorios.spCatTerritorio_PorRik_Ver2(Anio, Mes, idEmp, idCd, idRik, sesion.Emp_Cnx);
        }

        // 3MAY-2021
        public List<eTerritorio> spGetTerritoriosAll(int idEmp, int idCd, int IdRik, Sesion sesion)
        {
            CD_CatTerritorios cdCatTerritorios = new CD_CatTerritorios();
            return cdCatTerritorios.spGetTerritoriosAll(idEmp, idCd, IdRik, sesion.Emp_Cnx);
        }
        ...
        {
            CD_CatTerritorios cdCatTerritorios = new CD_CatTerritorios();
            return cdCatTerritorios.SelCatTipoTerritorio(Id_Cd, Movimiento, Conexion);
        }
    }
}