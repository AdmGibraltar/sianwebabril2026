...
namespace CapaNegocios
{
    public class CN_CatSegmentos
    {
        ...
        {
            CD_CatSegmentos CD = new CD_CatSegmentos();
            return CD.SelCatSegmento(Id_Emp, Id_Uen, Conexion);
        }

        // 24SEP-2021 RFH
        public List<eSegmento> ConsultaSegmentos(int Id_Emp, int Id_Cd, int Id_Rik, string Conexion)
        {
            CD_CatSegmentos claseCapaDatos = new CD_CatSegmentos();
            return claseCapaDatos.Consultar_SegmentoByRik(Id_Emp, Id_Cd, Id_Rik, Conexion);
        }
