...
namespace CapaNegocios.FlujosDeEstado.CRM
{
        ...
        : BaseStateMachine<ProyectoState, ProyectoTransition>
    {
        ...
        protected void BuildGraph()
        {
            ...
            // GENERA PROPUESTA RECNO-ECONOMICA
            ...
            negociacionState.OnEnter = () =>
            {
                ...
                try
                {
                    ...
                    {
                        // 19 Sep 2018
                        // Se elimina metodo por EF y se utiliza sp 
                        // Genera propuesta tecnica apartir de valuacion por medio de un SP
                        //cnCrmPropuestaTecnica.GenerarAPartirDeValuacion(Sesion, valuacion.Id_Val, Transaction);
                        cnCrmPropuestaTecnica.Sp_GenerarAPartirDeValuacion(Sesion, valuacion.Id_Val);
                    }
                }
                catch (Exception ex)
                {
            ...
            cierreState.OnEnter = () =>
            {
                ...
                try
                {
                    ...
                    //bool bTieneACYS = cnCapAcys.ClieneTieneACYS(Sesion, valuacion.Id_Cte, valuacion.Id_Ter.Value);

                    int Acys_Id_Acs = 0;
                    int Acys_Id_AcsVersion = 0;
                    int Acys_Id_Cte = valuacion.Id_Cte;
                    int Acys_Id_Ter = valuacion.Id_Ter.Value;
                    // (8)
                    cnCapAcys.ClieneTieneACYS_ByCteTer(Sesion, Sesion.Id_Emp, Sesion.Id_Cd, valuacion.Id_Cte, valuacion.Id_Ter.Value,
                    ...
                    if (Acys_Id_Acs > 0)
                    {
                        ...
                        //Actualizamos el ACYS // RFH 22 01 2018
                        //acys.Id_Acs = acysDeCliente.Id_Acs;
                        ...
                        lstProductosExistentes = cnCapAcysDet.Consulta_ProductosDeACYS(
                            Sesion.Id_Emp
                            , Sesion.Id_Cd
                            , acysDeCliente.Id_Cte.Value
                            , acysDeCliente.Id_Acs
                            , acysDeCliente.Id_Ter.Value, Sesion
                            );
                        */
                        ...
                        // 2 Abr 2019 RFH
                        // el listado lstProductosExistentes puede regresar null y al hacer ejectar "Last()"
                        // Lanza una exepcion se agrega el try                        
                        ...
                        int iMax = 0;
                        foreach (var E in productosExistentes)
                        {
                            if (E.Id_AcsDet > iMax)
                            {
                                iMax = E.Id_AcsDet;
                        ...
                        */
                        /*
                        for (var i = 0; i < productosExistentes.Count(); i++)
                        {
                            if (productosExistentes[i].Id_AcsDet > max)
                            {
                                iMax = productosExistentes[i].Id_AcsDet;
                            }
                        }
                        */
                        //maxId = iMax;
                        ...
                        */
                        ...
                        for (int pId = siguienteId, idx = 0; pId < ultimoId; pId++, idx++)
                        {
                            productosDeACYS.Add(new CapAcysDet()
                            {
                                ...
                                //Id_Ter = acysDeCliente.Id_Ter //RFH 22 Ene 2018
                                Id_Ter = Acys_Id_Ter  //RFH 22 Ene 2018
                    ...
                    // May07-2019 RFH
                    // Obsoleto ya no aplica relacionar Valuacion->ACyS
                    //
            ...
            Transition_PresentacionANegociacion.Evaluator = () =>
            {
                ...
                if (valuacion != null)
                {
                    ...
                    /*TODO: debe preguntar si el resultado de la valuación es aceptable; por ahora se deja en 2, para que 
                     * la valuación pueda ser aceptada, y la utilidad remanente pueda modificarse desde la base de datos.
