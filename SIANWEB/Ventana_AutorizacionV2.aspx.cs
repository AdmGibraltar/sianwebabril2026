...
namespace SIANWEB
{
    public partial class Ventana_AutorizacionV2 : System.Web.UI.Page
    {
                ...
                if (id == 1)
                {
                    ...
                    if (user.Cu_Estatus)
                    {
                        ...
                        else
                        {
                            ...
                            if (!HttpContext.Current.Session["Sesion" + HttpContext.Current.Session.SessionID + "Id_Tu"].ToString().Contains("," + user.Id_TU.ToString() + ",") && CDI != 0) 
                            {
                                result.Status = "El usuario no cuenta con la autorización necesaria";
                            }
                            else
                            {
                                HttpContext.Current.Session["autorizo!"] = true;
                            }
                        }
                    }
