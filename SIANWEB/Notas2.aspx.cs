...
namespace SIANWEB
{
    public partial class Notas2 : System.Web.UI.Page
    {
        #region Variables
            ...
            try
            {
                ...
                else
                {
                    if (!Page.IsPostBack)
                    {
                        ...
                        this.Inicializar(Id_Emp, Id_Cd, Id_Nca, notaModificable);
        ...
        protected void cmbProductoDet_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    new CN_CatProducto().ConsultaProducto(ref prd, session.Emp_Cnx, session.Id_Emp, session.Id_Cd_Ver, session.Id_Cd_Ver, Convert.ToInt32(txtProducto.Value.HasValue ? (sender as RadNumericTextBox).Value : -1),0);
                }
                catch (Exception ex)
                {
        ...
        protected void rgNotaCargoDet_ItemDataBound(object sender, GridItemEventArgs e)
        {
            try
            {
                if ((e.Item is GridEditableItem) && (e.Item.IsInEditMode))
                {
                    ...
                    if (updatebtn != null)
                    {
                        long Id_Prd = Convert.ToInt32(editItem.OwnerTableView.DataKeyValues[editItem.ItemIndex]["Id_Prd"]);
        ...
        protected void rgNotaCargoDet_InsertCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    valor_adenda = (insertedItem[det.Id_AdeDet.ToString()].Controls[0] as TextBox).Text;
                    if (valor_adenda == "")
                    {
                        falta_adenda = true;
                        break;
                ...
                if (falta_adenda)
                {
                    Alerta("Todos los datos de la adenda son requeridos");
                    e.Canceled = true;
                }
                else
                {
        ...
        private void LLenarFormNotaCargo(int Id_Emp, int Id_Cd, int Id_Nca)
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            ...
            new CN_CapNotaCargo().ConsultarAdenda(sesion.Id_Emp, sesion.Id_Cd_Ver, Id_Nca, notaCargo.Id_NcaSerie, "3", "4", ref listCabT, ref listDetT, sesion.Emp_Cnx);
            ...
            CargarEspecial(Id_Nca, sesion, notaCargo);
        ...
        private void CargarEspecial(int Id_Nca, Sesion sesion, NotaCargo notaCargo)
        {
            //- Especial
        ...
        private void Inicializar(int Id_Emp, int Id_Cd, int Id_Nca, string notaModificable)
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            ...
            if (Id_Emp > 0 && Id_Cd > 0 && Id_Nca > 0)
            {
                this.panelBanco.Style.Add("display", "none");
                this.panelCuentaContable.Style.Add("display", "none");
                this.LLenarFormNotaCargo(Id_Emp, Id_Cd, Id_Nca); //EDICION de factura
        ...
        private NotaCargo LlenarObjetoNotaCargo()
        {
            ...
            notaCargo.Nca_Referencia = Convert.ToInt32(txtReferencia.Text);
                    ...
                    { //ADENDA
                        ...
                        if (Session["ListaProductosNotaCargoEspecial" + Session.SessionID] != null)
                        {
                            if (Session["NcargoEspecialGuardada" + Session.SessionID].ToString() == "1")
                            {
                                ...
                                if (Math.Round(totalEspecial, 2) != Math.Round(txtSubtotal2.Value.Value))
                                {
                                    Alerta("El total del documento especial no es igual al total del documento original");
                                    return;
                                }
                            }
                        }
        ...
        protected void ListaProductosNotaCargo_EliminarProducto(long id_Prd)
        {
            try
            {
                DataRow[] Ar_dr;
