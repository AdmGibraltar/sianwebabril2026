...
namespace SIANWEB
{
    public partial class CapEntSalAutorizacion : System.Web.UI.Page
    {
        ...
        protected void rgEntradaSalida_ItemCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    Int64 prd_ = Int64.Parse((rgEntradaSalida.MasterTableView.Items[e.Item.ItemIndex]["Id_Prd"].FindControl("ProdLabel") as Label).Text);
        ...
        private void Autorizar(string esolunique)
        {
            try
            {
                Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
                ...
                string solunique = Request.QueryString["id"].ToString();
                EntSalSolicitud esvalidar = new EntSalSolicitud();

                if (esolunique != "-1")
                {
                    solunique = esolunique;
                }
                cn_esol.CapEntSalSolicitud_Consulta(solunique, ref esvalidar, sesion.Emp_Cnx);


                if (esolunique == "-1")
                {

                    if (esvalidar.Id_UEnviar != sesion.Id_U && esvalidar.Id_UCC != sesion.Id_U)
                    {
                ...
                if (esvalidar.ESol_Estatus != "P")
                {
                    AlertaCerrar("Solo se pueden atender solicitudes con estatus <b> Pendiente</b>");
                    return;
                }
                //poner aqui el cambio de fecha 
                if (es.Es_Fecha < sesion.CalendarioIni || es.Es_Fecha > sesion.CalendarioFin)
                {
        ...
        private void EnviarCorreoAtendio(int Tipo, int Id_ESol)
        {
            try
            {
                ...
                MailMessage m = new MailMessage();
                m.From = new MailAddress(configuracion.Mail_Remitente);
                if (es.Id_Tm != 66 && es.Id_Tm != 67 && es.Id_Tm != 68 && es.Id_Tm != 69)
                {
                    m.To.Add(new MailAddress(es.ESol_CorreoDest));
                }
                //m.To.Add(new MailAddress("jmartinez@axsistec.com"));
