...
namespace SIANWEB
{
    public partial class Rep_SerImproductivo : System.Web.UI.Page
    {
        #region Variables
        ...
        private void Imprimir(bool a_pantalla)
        {
            try
            {
                ...
                if (error == 0)
                {
                    ...
                    ALValorParametrosInternos.Add(this.txtRelacion.Text == "" ? "Todos" : this.txtRelacion.Text); // OPCION DE SELECCION POR gRUPO
                    ALValorParametrosInternos.Add(this.txtCliente.Text == "" ? "Todos" : this.txtCliente.Text); // OPCION DE SELECCION POR CLIENTE
                    ALValorParametrosInternos.Add(this.cmbRepresentante.SelectedValue == "-1" ? "Todos" : cmbRepresentante.Text); // OPCION DE SELECCION POR CLIENTE
                    ALValorParametrosInternos.Add(this.chkControl.Checked ? chkControl.Text : ""); // 
                    ...
                    ALValorParametrosInternos.Add(this.txtCliente.Text == "" ? null : this.txtCliente.Text); // OPCION DE SELECCION POR gRUPO
                    ALValorParametrosInternos.Add(this.txtRelacion.Text == "" ? null : this.txtRelacion.Text); // OPCION DE SELECCION POR gRUPO
                    ALValorParametrosInternos.Add(cmbVentas.SelectedValue == "3" ? "0" : null); // TERCER MES SELECCIONADO
                    ALValorParametrosInternos.Add(cmbPor.SelectedValue); // POR 1: GENERAL, 3: COMODATO
                    ALValorParametrosInternos.Add(cmbRepresentante.SelectedValue); // RIK
                    ALValorParametrosInternos.Add(chkControl.Checked); // 
                    ALValorParametrosInternos.Add(false); // 
                    ALValorParametrosInternos.Add(sesion.Emp_Cnx); // CADENA DE CONEXION A LA BASE DE DATOS

                    Type instance = null;

                    if (chkControl0.Checked)
                    {
                        if (a_pantalla)
                        {
                            instance = typeof(LibreriaReportes.Rep_SerImproductivos);
                        }
                        else
                        {
                            instance = typeof(LibreriaReportes.ExpRep_SerImproductivos);
                        }
                    }
                    else
                    {
                        if (a_pantalla)
                        {
                            instance = typeof(LibreriaReportes.Rep_SerImproductivos2);
                        }
                        else
                        {
                            instance = typeof(LibreriaReportes.ExpRep_SerImproductivos2);
                        }
                    }
        ...
        protected void txtCliente_TextChanged(object sender, EventArgs e)
        {
            try
            {
                bool isNumeric = int.TryParse((sender as RadTextBox).Text, out int n);
                if (isNumeric)
                {
                    ErrorManager();
                    ...
                    if (new CN_CatCliente().ConsultaClienteDirecto(cliente, gSession.Emp_Cnx))
                    {
                        Alerta($"El cliente {cliente.Id_Cte} pertenece a clientes directos y es administrado desde central");
