...
namespace SIANWEB
{
    public partial class ProPrecioEspecial_Autorizacion : System.Web.UI.Page
    {
        #region Variables
        ...
        private List<VentanaPrecioEspecialCte> GetListCte()
        {
            try
            {
                ...
                ape.Id_Cd = session.Id_Cd_Ver;
                ape.Ape_Unique = GUID;
                cn_ape.ConsultaVentanaPrecioEspecialCte(ape, session.Emp_Cnx, ref List);
                return List;
            }
            catch (Exception ex)
            {
        ...
        private void EnviaEmail(int Id_Ape)
        {
            try
            {
                ...
                CN_PrecioEspecial cn_pe = new CN_PrecioEspecial();
                PrecioEspecial pe = new PrecioEspecial();

                cn_pe.ConsultaDatosEmail(ref pe, Id_Ape, session);
                ...
                cuerpo_correo.Append("La solicitud #" + lblFolio.Text + " ha sido "+ pe.Ape_Estatus + "." );
                cuerpo_correo.Append("</td></tr>");
                cuerpo_correo.Append("<tr>");
                cuerpo_correo.Append("<td colspan='2'><b><font face= 'Tahoma' size = '2'>");
                cuerpo_correo.Append(pe.Ape_NotaResp);
                cuerpo_correo.Append("</td>");
                cuerpo_correo.Append("</tr>");
                cuerpo_correo.Append("<tr><td colspan='2'>");
                string[] url = Request.Url.ToString().Split(new char[] { '/' });
                cuerpo_correo.Append("<center><br><a href='" + Request.Url.ToString().Replace(url[url.Length - 1], "") + "ProPrecioEspecial_Admin.aspx'>Solicitud de autorización de precios especiales</a></font></center>");
