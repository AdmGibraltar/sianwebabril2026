...
namespace SIANWEB
{
    public partial class ProEntradaVirtual_Admin : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void CmbCentro_SelectedIndexChanged(object o, Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                if (Sesion.CalendarioIni >= dpFechaIni.MinDate && Sesion.CalendarioIni <= dpFechaFin.MaxDate)
                {
                    dpFechaIni.DbSelectedDate = Sesion.CalendarioIni;
                }
        ...
        protected void rg1_ItemCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                switch (e.CommandName.ToLower())
                {
                    ...
                    case "autorizar":
                        if (gi.Cells[4].Text.ToString().ToLower() == "s" || gi.Cells[4].Text.ToString().ToLower() == "a")
                        {
                            autorizar(gi);
                        }
                        else
        ...
        private void eliminar(GridItem gi)
        {
            ...
            if (gi.Cells[4].Text.ToString().ToLower() == "s" || gi.Cells[4].Text.ToString().ToLower() == "c")
            {
                ...
                EntradaVirtual p = new EntradaVirtual();
                p.Id_Env = Convert.ToInt32(gi.Cells[2].Text);
                p.Id_Emp = sesion.Id_Emp;
                p.Id_Cd = sesion.Id_Cd_Ver;
                int verificador = 0;
                CN_EntradaVirtual.Eliminar(p, sesion.Emp_Cnx, ref verificador);
        ...
        protected void rg1_ItemDataBound(object sender, GridItemEventArgs e)
        {
            try
            {
                if (e.Item is GridDataItem)
                {
                    ...
                    Button.Attributes["onclick"] = clickHandler.Replace("[[ID]]", item.GetDataKeyValue("Id_Env").ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void ValidarPermisos()
        {
            try
            {
                ...
                //Comentado MPS 13-02-2016//
                /*if (ConsultarAutorizacionPrecio() == "True")
                {
                    this.rg1.Columns[10].Visible = true ;
                }
                else
                {
                    this.rg1.Columns[10].Visible = false;
        ...
        private DataTable GetList()
        {
            try
            {
                List<AutEntradaVirtual> List = new List<AutEntradaVirtual>();
                CN_EntradaVirtual clsCN_CapProAutEntradaVirtual = new CN_EntradaVirtual();
                Sesion Sesion = (Sesion)Session["Sesion" + Session.SessionID];

                AutEntradaVirtual AutEntradaVirtual = new AutEntradaVirtual();
                AutEntradaVirtual.Id_Emp = Sesion.Id_Emp;
                AutEntradaVirtual.Id_Cd = Sesion.Id_Cd_Ver;
                if (txtFolioIni.Text != "") AutEntradaVirtual.Folio1 = Convert.ToInt32(txtFolioIni.Text);
                if (txtFolioFin.Text != "") AutEntradaVirtual.Folio2 = Convert.ToInt32(txtFolioFin.Text);
                if (dpFechaIni.SelectedDate != null) AutEntradaVirtual.Fecha1 = dpFechaIni.SelectedDate;
                if (dpFechaFin.SelectedDate != null) AutEntradaVirtual.Fecha2 = Convert.ToDateTime(dpFechaFin.SelectedDate.Value.ToString("dd/MM/yyyy")).AddDays(1).AddSeconds(-1);
                AutEntradaVirtual.Estatus = cmbEstatus.SelectedValue;
                if (txtClienteIni.Text != "") AutEntradaVirtual.Id_CteFiltro1 = Convert.ToInt32(txtClienteIni.Text);
                if (txtClienteFin.Text != "") AutEntradaVirtual.Id_CteFiltro2 = Convert.ToInt32(txtClienteFin.Text);
                // if (string.IsNullOrEmpty(txtSolicitud.Text)) AutEntradaVirtual.Solicitud = Convert.ToInt32(txtSolicitud.Text);

                clsCN_CapProAutEntradaVirtual.ConsultaProAutEntradaVirtual_Lista(AutEntradaVirtual, Sesion.Emp_Cnx, ref List);
                dt = new DataTable();
                dt.Columns.Add("Env_Estatus", System.Type.GetType("System.String"));
                dt.Columns.Add("Env_EstatusStr", System.Type.GetType("System.String"));
                dt.Columns.Add("Env_Fecha", System.Type.GetType("System.DateTime"));
                dt.Columns.Add("Env_CteNomComercial", System.Type.GetType("System.String"));
                dt.Columns.Add("Id_Env", System.Type.GetType("System.Int32"));
                ...
                dt.Columns.Add("Id_Es", System.Type.GetType("System.Int32"));
                dt.Clear();

                foreach (AutEntradaVirtual EntradaVirtual in List)
                {
                    DataRow row = dt.NewRow();
                    row["Env_Estatus"] = EntradaVirtual.Env_Estatus;
                    row["Env_EstatusStr"] = EntradaVirtual.Env_EstatusStr;
                    row["Env_Fecha"] = EntradaVirtual.Env_Fecha;
                    row["Env_CteNomComercial"] = EntradaVirtual.Cte_NomComercial;
                    row["Id_Env"] = EntradaVirtual.Id_Env;
                    row["Id_Cte"] = EntradaVirtual.Id_Cte;
                    row["Id_Cd"] = Sesion.Id_Cd;
                    row["Id_Emp"] = Sesion.Id_Emp;
                    row["Id_Es"] = EntradaVirtual.Id_Es;
        ...
        private void EnviaEmail(int Id_Env)
        {
            try
            {
                Sesion Sesion = (Sesion)Session["Sesion" + Session.SessionID];
                ...
                CN_EntradaVirtual CN_EntradaVirtual = new CN_EntradaVirtual();
                CN_EntradaVirtual.ConsultaEnvio(ref pe, Sesion.Emp_Cnx, ref verificador);

                if (CambiarEstatus(Id_Env, "S") != 1)
                {
                ...
                cuerpo_correo.Append("Se ha colocado una solicitud de autorización de EntradaVirtual " + Id_Env);
                ...
                // JFCV
                Boolean permisoGuardar = true;
                Boolean permisoModificar = true;
                Boolean permisoEliminar = true;
                Boolean permisoImprimir = true;

                cuerpo_correo.Append("<center><br>");
                cuerpo_correo.Append("<a href='" + Request.Url.ToString().Replace(url[url.Length - 1], "") + "ProEntradaVirtual.aspx?Id_Folio=" + Id_Env + "&accion=" + 5 + "&PermisoGuardar=" +
                ...
                m.Subject = "Solicitud de autorización de entrada virtual #" + Id_Env + " del centro " + Sesion.Id_Cd_Ver;
                m.IsBodyHtml = true;
        ...
        private void autorizar(GridItem gi)
        {
            try
            {
                ...
                RAM1.ResponseScripts.Add("return AbrirVentana_EntradaVirtual('" + gi.Cells[2].Text + "', 5)");
        ...
        private string ConsultarAutorizacionPrecio()
        {
            try
            {
                ...
                u.Id_U = Sesion.Id_U;
                string Autorizacion = "";
                cn_catusuario.ConsultaAutorizacionPrecio(u, Sesion.Emp_Cnx, ref Autorizacion);
                return Autorizacion;
