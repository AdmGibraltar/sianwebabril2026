...
namespace SIANWEB
{
    public partial class CapFisico : System.Web.UI.Page
    {
                    ...
                    switch (btn.CommandName)
                    {
                        ...
                        case "DwExcel":
                            ...
                            if (File.Exists(ruta))
                            {
                                ...
                                RAM1.ResponseScripts.Add("window.location.href = 'Reportes/Fisico.xls';");

                            }
                            break;
                        case "UpExcel":
                            RAM1.ResponseScripts.Add("upexcel1();");
                            break;
                    }
                }
            }
        ...
        protected void txtProducto_TextChanged(object sender, EventArgs e)
        {
            ...
            cn_producto.ConsultaProducto(ref prd, Sesion.Emp_Cnx, Sesion.Id_Emp, Sesion.Id_Cd_Ver, (Int64)txtProducto.Value.Value, 0);
        ...
        protected void cmbProductosLista_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                if (e.Value != string.Empty && e.Value != "-1")
                {
                    ...
                    new CN_CatProducto().ConsultaProducto(ref p, Sesion.Emp_Cnx, Sesion.Id_Emp, Sesion.Id_Cd_Ver, Convert.ToInt32(e.Value), 0);
        ...
        private void CargarProductos()
        {
            try
            {
                ...
                {
                    CapaNegocios.CN__Comun CN_Comun = new CapaNegocios.CN__Comun();
                    CN_Comun.LlenaCombo64(1, Sesion.Id_Emp, Sesion.Id_Cd_Ver, Sesion.Emp_Cnx, "spCatProducto_Combo", ref cmbProductosLista);
                }
            }
            catch (Exception ex)
            {
        ...
        private Fisico LlenarFisico()
        {
            ...
            Fisico.Id_Prd = Convert.ToInt64(txtProducto.Text);
            Fisico.Fis_Fecha = DateTime.Now;
            Fisico.Fis_Fisico = Convert.ToInt32(txtFisico.Text);
            return Fisico;
        }

        private List<FisicoConsignado> GetList(long Id_Prd)
        {
            try
            {
                List<FisicoConsignado> List = new List<FisicoConsignado>();
