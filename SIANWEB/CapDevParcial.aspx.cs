    ...
    public partial class CapDevParcial : System.Web.UI.Page
    {
        ...
        protected void txtClave_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cn_capfactura.RastreoDev(ref fac, Sesion.Emp_Cnx);

                    double saldoRastreo =
                      (double)(fac.Fac_ImporteIva ?? 0) +
                      (double)(fac.Fac_SubTotal ?? 0) -
                      (double)(fac.Fac_Pagado ?? 0);

                    if (saldoRastreo <= 0.0001)
                    {
                        txtFactura.Text = "";
                        Alerta("La factura no tiene saldo para devolver");
                        LimpiarDatos();
                        return;
                    }
        ...
        protected void ckDevuelto_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
                    int i = Convert.ToInt32(((sender as CheckBox).Parent.FindControl("Label9") as Label).Text) - 1;
                    int territorio = Convert.ToInt32((rgDevParcial.MasterTableView.Items[i]["Id_Ter"].Text));
                    Int64 prd = Convert.ToInt64((rgDevParcial.MasterTableView.Items[i]["Id_Prod"].Text));
                    ...
                    if (agrupador != "-1")
                    {
                        for (int j = 0; j < rgDevParcial.MasterTableView.Items.Count; j++)
                        {
                            if (agrupador == rgDevParcial.MasterTableView.Items[j]["Prd_Agrupador"].Text && (rgDevParcial.MasterTableView.Items[j].FindControl("ckDevuelto") as CheckBox).Checked)
                                if (prd == Convert.ToInt64(rgDevParcial.MasterTableView.Items[j]["Id_Prod"].Text))
        ...
        private void Inicializar()
        {
            try
            {
                ...
                if (txtCliente.Text != "" && txtFactura.Text != "")
                {
                    cargarDatosCuentaNacional(txtCliente.Text);
                }
            }
        ...
        private void baja(ref GridCommandEventArgs e, ref List<string> statusPosibles, ref GridItem item)
        {
            ...
            devparcial.IdProd = Convert.ToInt64(rgDevParcial.MasterTableView.Items[e.Item.ItemIndex]["Id_Prod"].Text);
        ...
        private void CargarCFDIUsoParaNC()
        {
            Sesion Sesion = new Sesion();
            Sesion = (Sesion)Session["Sesion" + Session.SessionID];
        ...
        private void CargarFacturasPantalla(List<DevParcial_Detalle> devParcialDetalle)
        {
            try
            {
                ...
                try
                {
                    ...
                    cmbNCUsoCFDI.SelectedIndex = cmbNCUsoCFDI.FindItemIndexByValue(devParcialDetalle[0].Ncr_UsoCFDI.ToString());
                    cmbNCUsoCFDI.Text = cmbNCUsoCFDI.FindItemByValue(devParcialDetalle[0].Ncr_UsoCFDI.ToString()).Text;
                    cmbNCFormaPago.SelectedIndex = cmbNCFormaPago.FindItemIndexByValue(devParcialDetalle[0].Ncr_FormaPago.ToString());
                    cmbNCFormaPago.Text = cmbNCFormaPago.FindItemByValue(devParcialDetalle[0].Ncr_FormaPago.ToString()).Text;
                    cmbNCMetodoPago.SelectedIndex = cmbNCMetodoPago.FindItemIndexByValue(devParcialDetalle[0].Ncr_MetodoPago.ToString());
                    cmbNCMetodoPago.Text = cmbNCMetodoPago.FindItemByValue(devParcialDetalle[0].Ncr_MetodoPago.ToString()).Text;
                }
                catch
                {
                    try
                    {
                        ...
                        cte.Id_Cte = Convert.ToInt32(devParcialDetalle[0].Cliente1.ToString());
                        ...
                        //cmbNCFormaPago.SelectedIndex = cmbNCFormaPago.FindItemIndexByValue(cte.Cte_NCMetodoPago.ToString());
                        //cmbNCFormaPago.Text = cmbNCFormaPago.FindItemByValue(cte.Cte_NCMetodoPago.ToString()).Text;
                        //cmbNCMetodoPago.SelectedIndex = cmbNCMetodoPago.FindItemIndexByValue(cte.Cte_NCFormaPago.ToString());
                        //cmbNCMetodoPago.Text = cmbNCMetodoPago.FindItemByValue(cte.Cte_NCFormaPago.ToString()).Text;
        ...
        private void Totales()
        {
            ...
            factura.Id_Cd = sesion.Id_Cd;
            factura.Id_Fac = !string.IsNullOrEmpty(txtFactura.Value.ToString()) ? Convert.ToInt32(txtFactura.Value.Value) : 0;
            // AOG Debe traer el mismo IVA de la factura
            //new CN_CatCentroDistribucion().ConsultarIva(sesion.Id_Emp, sesion.Id_Cd_Ver, ref iva_cd, sesion.Emp_Cnx);
            new CN_CapFactura().ConsultaFactura(ref factura, sesion.Emp_Cnx);
            iva_cd = Convert.ToDouble(factura.Fac_ImporteIva / factura.Fac_Importe) * 100;
        ...
        private void Guardar()
        {
            ...
            if (cmbNCUsoCFDI.SelectedIndex == cmbNCUsoCFDI.FindItemIndexByValue("-1"))
            {
                Alerta("Seleccione un Uso CFDI valido");
                return;
            }
            if (cmbNCFormaPago.SelectedIndex == cmbNCFormaPago.FindItemIndexByValue("-1"))
            {
                Alerta("Seleccione una forma de pago valida");
                return;
            }
            if (cmbNCMetodoPago.SelectedIndex == cmbNCMetodoPago.FindItemIndexByValue("-1"))
            {
                Alerta("Seleccione un metodo de pago valido");
                return;
            }

            if (cmbNCMetodoPago.SelectedIndex == cmbNCMetodoPago.FindItemIndexByValue("PPD") && cmbNCFormaPago.SelectedIndex != cmbNCFormaPago.FindItemIndexByValue("99"))
            {
                Alerta("Si el método de pago es PPD (En parcialidades o diferido), la forma de pago debe ser 99 Por Definir");
                return;
            }

            if (cmbNCMetodoPago.SelectedIndex != cmbNCMetodoPago.FindItemIndexByValue("PPD") && cmbNCFormaPago.SelectedIndex == cmbNCFormaPago.FindItemIndexByValue("99"))
            {
                Alerta("Si el método de pago es PUE (Pago en una sola exhibición), la forma de pago no debe ser 99 Por Definir");
                return;
            }

            if (cmbNCFormaPago.SelectedIndex == cmbNCFormaPago.FindItemIndexByValue("31"))
            {
                Alerta("La forma de pago Intermediario Financienro es invalida");
                return;
            }
            this.Totales();
            ...
            devparcial.Desc = txtDesc.Text;
            devparcial.Desc2 = txtDesc2.Text;
            devparcial.Ncr_FormaPago = cmbNCFormaPago.SelectedValue;
            devparcial.Ncr_MetodoPago = cmbNCMetodoPago.SelectedValue;
            devparcial.Ncr_UsoCFDI = cmbNCUsoCFDI.SelectedValue;
            int verificador = 0;

            bool nuevaNotaCredito = devparcial.Nota == 0 ? true : false;
            ...
            if (RadTabStrip1.Tabs[2].Visible)
            {
                ...
                facturaNacional.Id_Ncr = verificador;
                int verificador2 = 0;
                new CN_CapNotaCredito().InsertarNotaCreditoNacional(ref facturaNacional, sesion.Emp_Cnx, ref verificador2);
            ...
            {//valido            
                ...
                // JorgeRF 23/09/22 agregar nota de credito
                CapNotaCredito_Lista RefCapNotaCredito = new CapNotaCredito_Lista();

                if (nuevaNotaCredito)
                {
                    try
                    {
                        RefCapNotaCredito.ImprimirNotaCreditoAutomatica(sesion.Id_Emp, sesion.Id_Cd, verificador, ref strMsjError, ref strScript);
                        ...
                        {
                            if (strScript.Contains("AbrirNotaCreditoPDF"))
                            {
                                RAM1.ResponseScripts.Add(string.Concat(@"CloseWindow('", "Devolución grabada correctamente, " +
                                    "consulte la Nota de Crédito en el menú: Procesos Operativos > Crédito y Cobranza > Capturas > Notas de crédito.", "')"));
                            }
        ...
        private void guardarGrid(ref List<DevParcial_Detalle> devparcialList)
        {
            ...
            for (int i = 0; i < this.rgDevParcial.Items.Count; i++)
            {
                ...
                devparcial.Desc = txtDesc.Text;
                devparcial.Desc2 = txtDesc2.Text;
                devparcial.IdProd = !string.IsNullOrEmpty(rgDevParcial.MasterTableView.Items[i]["Id_Prod"].Text) ? Convert.ToInt64(rgDevParcial.MasterTableView.Items[i]["Id_Prod"].Text) : 0;
        ...
        public void cargarDatosCuentaNacional(string idCliente)
        {
            ...
            RadTabStrip1.Tabs[2].Visible = FacturarCuentaNacional;
        }
