            ...
            try
            {
                if (e.CommandName.ToString() == "Autorizar")
                {
                    ...
                    if (item >= 0)
                    {
                        ...
                        factura.Id_Rem = Convert.ToInt32(rgRemisiones.Items[item]["Id_Rem"].Text);
                        try
                        {
                            factura.Fac_PedNum = Convert.ToInt32(rgRemisiones.Items[item]["Pedido"].Text);
                        }
                        catch
                        {
                        ...
                        remision.Id_Rem = Convert.ToInt32(rgRemisiones.Items[item]["Id_Rem"].Text);
                        remision.Id_Cte = Convert.ToInt32(rgRemisiones.Items[item]["Num_Cliente"].Text);

                        try
                        {
                            remision.Fac_PedNum = Convert.ToInt32(rgRemisiones.Items[item]["Pedido"].Text);
                        }
                        catch
                        {
                        ...
                        CNProFactura_Embarque.ValidarPedidoFacturaEcommerce(factura, session.Emp_Cnx, ref lista);
                        string estatus = "";
                        string mensaje = "";
                        if (lista.Count > 0)
                        {
                            ...
                            APis.EnvioPedidRemision(remision, session.Emp_Cnx, ref estatus, ref mensaje);
