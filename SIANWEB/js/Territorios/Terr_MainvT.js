//JUN10-2020 RFH Actualizacion
//MAY28-2020 RFH Actualizacion
//MAY9-2022 RFH Actualizacion Cambia la funcionaldiad para tipo de territorio 6

var Result_List = [];
function Establece_TipoRepresentante(TT, CALLBACK_Success) {
    console.log('Tipo Representante:' + TT)
    TT = parseInt(TT);
    if (isNaN(TT)) {
        TT = 0;
    }
    switch (TT) {
        ...
        case 2:
            ...
            $('#dev_tipoCliente').removeClass('tbl-Oculto')
            $('#dev_tipoCliente').addClass('tbl-Visible')
            break;
        case 3:
        case 6:
        case 5:
        ...
        case 4: //RIK
            ...
            $('#dev_tipoCliente').removeClass('tbl-Visible')
            $('#dev_tipoCliente').addClass('tbl-Oculto')
            break;
    }
    if (CALLBACK_Success) {
        CALLBACK_Success();
        ...
        */
        if (lst[i].IdAutorizacion > 0) {
            row.append($('<td class="ColEstatus">').append(
                '<span class="badge badge-success" style="margin-top:4px; background-color:#dbebfa; color:#000;">0' + lst[i].Id_Ter + '</span>'
            ));
        } else {
            row.append($('<td class="ColEstatus">').append(
                '0' + lst[i].Id_Ter
...
function Territorio_Editar(obj) {
    ...
    Cargar_TiposTerritorios(2, function () { });
    Inicializa_Modal();
    ...
    Territorios_CargarById(Id_Ter, function (Rec) {
        ...
        if (Rec.Territorializacion == "True") {
            if (Id_Ter == 51 || Id_Ter == 61) {
                $('#btnCrearSolicitudDeCambio').css('display', 'none');
                $('#btnTerritorio_Cancelar').prop('disabled', true);
                $('#btnTerritorio_Guardar').prop('disabled', true);
            } else if (Id_Ter > 10000000 || Id_Ter == 48) {
                $('#btnCrearSolicitudDeCambio').css('display', 'none');
                $('#btnTerritorio_Cancelar').prop('disabled', false);
                $('#btnTerritorio_Guardar').prop('disabled', false);
            }
        ...
        if (Rec.Estatus) {
            ...
            //JUN08-2020 RFH
            $('#ddlSolicitudCambio_Activo').val(1);
        } else {
            $('#btnTerrAut_Activo').css('display', 'none');
            $('#btnTerrAut_Inactivo').css('display', 'block');
            //JUN08-2020 RFH
            $('#ddlSolicitudCambio_Activo').val(0);
        }
        //console.log('Rec.Estatus:' + Rec.Estatus);
        Modal_Estado(false);
...
function Modal_Territorio_Nuevo() {
    $('#spinner_modalTerritorio').css('display', 'block');
    Inicializa_Modal();
    Cargar_TiposTerritorios(1, function () { });
...
function Calcular_Territorio() {
    ...
    if (TipoRepresentante == '1' || TipoRepresentante == '2') {
        $('#tbUEN').val('');
        $('#tbSegmento').val('');
    }
    var UEN = $('#tbUEN').val();
    ...
    var Territorio_Prefijo = TipoRepresentante
    if (TipoRepresentante < 3) {
        Territorio_Prefijo += TipoCliente
    }
    ...
    Segmento_1 = parseInt(Segmento_1);
    if (isNaN(Segmento_1)) {
        Segmento_1 = 0;
    }
    console.log('UEN: ' + UEN);
    console.log('Segmento_1: ' + Segmento_1);
    console.log('Territorio_Prefijo: ' + Territorio_Prefijo);

    Territorio_Maximo(Territorio_Prefijo, TipoRepresentante, TipoCliente, UEN, Segmento_1, function (TerritorioNuevo) {
        $('#tbClaveTerritorio').val('0' + Territorio_Prefijo + TerritorioNuevo);
    }, function () {
...
function Guardar_Territorio() {
    ...
    if ((Id_TipoRepresentante == 1 || Id_TipoRepresentante == 2) && tbTipoCliente.length <= 0) {
    ...
    var tbTerritorio = $('#ddlRepresentante').val();
    if (tbTerritorio == -1) {
        alertify.alert('El valor en el campo "Territorio autorizado -> Representante" es obligatorio.');
        return;
    }
    ...
    if (CrearSolicitud == true) {
        ...
        if (IdInsertUpdate == 1) {
            ...
            //return;
            Id_Rik = $('#tbSolicitudCambio_Representante').val();;
            Ter_Activo = $('#ddlSolicitudCambio_Activo').val();
            Ter_Nombre = $('#tbSolicitudCambio_Terr').val();
        }
        else {
            Ter_Activo = 1;
            Ter_Nombre = $('#tbTerritorio').val();
        ...
        //var Id_Rik = $('#ddlSolicitudCambio_Representante').val();
        ...
        Territorio_Crear(DataTer,
            function () {
                ...
                $('#modalTerritorio').modal('hide');
                if (IdInsertUpdate == 0) {
                    alertify.success('Se ha creado el registro de territorio ' + Id_Ter + ' exitosamente.');
                } else {
                    alertify.success('Se ha actualizado exitosamente el territorio: ' + Id_Ter + '.');
                }
...
function Pre_Territorios_CargarIndice() {
    ...
    var txtSearch = $('#tbParametroBusqueda').val()
    if (txtSearch.indexOf('0', 0) >= 0) txtSearch = txtSearch.substring(1)
    ...
    $('#btnCrearSolicitudDeCambio').click(function () {
        ...
        TerrUsuarios_CargarCombos(Id_Uen, Id_TipoRepresentante, $('#ddlSolicitudCambio_Representante'), 0,
            function (lst) {
                ...
                $('#tbSolicitudCambio_Representante').val($('#ddlSolicitudCambio_Representante').val());
            }, function () {
            });
        ...
        //MAY12-2020 RFH
        $('#tbIdLocal').prop('disabled', false);
        $('#tbIdLocal').prop('readonly', false);
