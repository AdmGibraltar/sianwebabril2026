...
function TerritoriosModal_AplicarPermisos(P, Id_Ter) {
...
function Territorio_Editar(obj) {
    ...
    Territorios_CargarById(Id_Ter, function (Rec) {
        ...
        Cargar_PermisosUsuario(Territorios_Pagina, function (Permiso) {
            TerritoriosModal_AplicarPermisos(Permiso, Id_Ter);
        }, function () {
        });
...
function CargarTipoTerritorio(obj, CALLBACK_Success) {

        obj.prop('disabled', true);

    SelAll_CatTipoTerritorio(0, 0,function (lst) {

            console.log('Listado TipoTerritorio');
            console.log(lst);

            obj.empty();
            for (var i = 0; i < lst.length; i++) {
                obj.append('<option value=' + lst[i].Id_TipoTer + '>' + lst[i].Id_TipoTer + ' .-' + lst[i].Descripcion + '</option>');
            }
