/*
    ...
    MAR26 - 2020 RFH 
    - Actialización de reporte para estatus Gerente y JO.
...
function ExportExcel_BajarReporteExcel(Data) {
    ...
    for (var i=0; i<Data.length; i++) {             
        // Imprime registro Normal
        excel.set(0, 0, line, Data[i].CNDescripcion, formatCell);
        excel.set(0,1,line,Data[i].Id_Acs, formatCell_C);                    
        excel.set(0,2,line,Data[i].Id_AcsVersion, formatCell);                    

        var Acs_ReqAutGerente = Data[i].Acs_ReqAutGerente;
        Acs_ReqAutGerente = parseInt(Acs_ReqAutGerente);
        if (isNaN(Acs_ReqAutGerente)) {
            Acs_ReqAutGerente = 0
        }
        var Acs_ReqAutJefeOp = Data[i].Acs_ReqAutJefeOp;
        Acs_ReqAutJefeOp = parseInt(Acs_ReqAutJefeOp);
        if (isNaN(Acs_ReqAutJefeOp)) {
            Acs_ReqAutJefeOp = 0;
        }

        switch (Data[i].Acs_Estatus) {
                case 'B':
                    AcsEstatus = 'Cancelado';
                    break;
                case 'C':
                    AcsEstatus = 'Capturado';
                    break;
                case 'S':
                    var AUTJO = "-";
                    var AUTG = "-";
                    ...
                    AcsEstatus = 'Solicitado('+ AUTG+'/'+AUTJO +')';
                    break;
                case 'A':
                    AcsEstatus = 'Autorizado';
                    break;
                case 'R':
                    AcsEstatus = 'Rechazado';
                    break;
                default:
                    AcsEstatus = 'Err.';
                    break;
        ...
        excel.set(0,3,line,AcsEstatus, formatCell);                    
        excel.set(0,4,line,Data[i].Id_Cte, formatCell);                    
        excel.set(0,5,line,Data[i].Cte_NomComercial, formatCell);                    
        excel.set(0,6,line,Data[i].Id_Ter, formatCell);                    
        excel.set(0,7,line,Data[i].Id_Rik, formatCell);                    
