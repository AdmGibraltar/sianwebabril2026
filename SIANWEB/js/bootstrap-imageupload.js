...
(function($) {
    ...
    function getImageThumbnailHtml(src) {
        return '<img src="' + src + '" alt="Image preview" class="img-thumbnail" style="max-width: ' + options.maxWidth + 'px; max-height: ' + options.maxHeight + 'px">';
    }
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'El archivo es demasiado grande (m&aacute;x ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Archivo de imagen no v&aacute;lido.');
        }
        else {
            callback(false, 'Tipo de archivo no v&aacute;lido.');
        }
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('Cargar ');
        $fileTab.find('.btn:eq(1)').hide();
        $fileTab.find('input').val('');
        $fileTab.prepend(getImageThumbnailHtml(imgDefault));
    }
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').text('Cargar ');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $browseFileButton.find('span').text('Editar ');
                    $removeFileButton.css('display', 'inline-block');
                };

                fileReader.onerror = function() {
                    $fileTab.prepend(getAlertHtml('Error al cargar la imagen.'));
                    $fileInput.val('');
                };
