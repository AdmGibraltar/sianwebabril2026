...
Prospectos2.js 
Key Soluciones 
06AGO-2020 Ajuste a Campos Obligatorios
JUN02-2020 RFH Campos de Prospecto Obligatorios
01Ene 2017 Angel Segura
01Jun 2018 RFH Correcciones 
13Dic 2018 .- Columna de No de Cliente en listado de prospectos. 
*/
...
var _datosGeneralesGuardados = false;
var _territorioAsociado = false
...
function territorioAsociadoConExito(response){
    ...
    '<td style="text-align: center">' +
        '<button '+
            'type="button" '+
            'onclick="btnRetirarTerritorio$click(this)" '+
...
function cargarTerritoriosAsociadosAProspectoSucceeded(response) {
    ...
    //console.log('DEBUG (function cargarTerritoriosAsociadosAProspectoSucceeded)');
...
function cargarSoluciones($, jqElement, idArea, onSuccess, onFailure) {
    if (typeof (idArea) == null || idArea == null) {
        alertify.success('Debe seleccionar el Area');
        return;
    }
...
const verificarNombre = (nombre) => {
    let resultado = []
    $.ajax({
        url: _ApplicationUrl + '/api/CrmProspecto/VerificarNombreProspecto?nombre=' + nombre,
        type: 'GET',
        cache: false,
        async: false,
        contentType: 'application/json'
...
function verificarN() {
    window.VerificaNombre = true;
    $('#modalAlerta').modal('hide')
    $('#btnDvModalNuevoProspectoGuardar').trigger('click')
}
...
function crearProspecto(sender) {
    ...
    var n = verificarNombre(Nombre)
    if (n.length && !window.VerificaNombre) {
        var html = '<strong>Se encontraron coincidencias con el mismo nombre:</strong><ul>';
        n.forEach(function (item) {
            // Usa la propiedad relevante, por ejemplo, Cte_NomComercial
            html += '<li>' + item.Nombre + ' | Id Prospecto: ' + item.Id_CrmProspecto + ' | Id Cliente: ' + item.Id_Cte + '</li>';
        });
        html += '</ul>';
        html += '<br>'
        html += '<button type="button" class="btn" onclick="verificarN()">Confirmar continuar registro</button>'
        $('#modalAlertaError').html(html)
    ...
    */
    ...
    $.ajax({
        ...
        statusCode: {
            401: function (jqXHR, textStatus, errorThrown) {
                $('#dvDialogoInicioSesion').modal();
                //    JFCV                _onLoginSuccessful = $.proxy(crearProspecto, sender);
            }
        }
    }).done(function (response, textStatus, jqXHR) {
        ...
        */
        ...
        //JFCV Leads asociar prospecto a Leads 
        ...
        //JFCV Leads Inicializar pantalla 2 dic 2020 
        location.reload();
...
function cancelarLead($) {
    ...
    $.ajax({
        url: _ApplicationUrl + '/api/CrmLeads/?IdLead=' + leadActualCancelar + '&tipocancelacion=' + tipodecancelacion + '&motivocancelacion=' + leadMotivoCancelar,
        type: 'DELETE',
        cache: false,
...
function crearProyecto() {
    ...
    if (cantidad == "" || cantidad <= 0) {
        alertify.error('El campo cantidad es obligatorio y tiene que ser mayor a 0');
...
/**
 * INICIO DAVID BUSTAMANTE
 * */
function obtenerUENS() {
    $.ajax({
        url: _ApplicationUrl + '/api/CatUEN/?Id_Emp=' + _EntidadSesion_Id_Emp + '&Id_Uen=0',
        cache: false,
        type: 'GET'
    }).done(function (response, textStatus, jqXHR) {

        var elm = $('#dg_uen')
        elm.append('<option value="-1">-- Seleccione --</option>')
        $.each(response.Datos, function (i, v) {
            elm.append('<option value="' + v.Id_Uen + '">' + v.Descripcion + '</option>')
        })

    })
}

function dg_uenOnChange(elm) {
    var idUen = $(elm).val()
    cargarSegmentos(idUen, null)
}

function cargarSegmentos(idUen, segmento) {
    //mostrar el indicador de operación en proceso
    $('#imgProcesandoSegmentoDvModalNuevoProyecto').fadeIn();
    $.ajax({
        url: _ApplicationUrl + '/api/CatSegmento/SelCatSegmento/?Id_Emp=' + _EntidadSesion_Id_Emp + '&Id_Uen=' + idUen,
        cache: false,
        type: 'GET',
        statusCode: {
            401: function (jqXHR, textStatus, errorThrown) {
        ...
        elm.append('<option value="-1">-- Seleccione --</option>')
        $.each(response.Datos, function (i, v) {
            elm.append('<option value="' + v.Id_Seg + '">' + v.Seg_Descripcion + '</option>')
        })

        if (segmento != null) $('#dg_segmento').val(segmento)
...
function obtenerTipoClientes() {
    $.ajax({
        url: _ApplicationUrl + '/api/Helper/?diff1=1',
        cache: false,
        type: 'GET'
    }).done(function (response, textStatus, jqXHR) {

        var elm = $('#dg_tipoClientes')
        $.each(response.Datos, function (i, v) {
            elm.append('<option value="' + v.Id + '">' + v.Descripcion + '</option>')
        })
    })
}

function dg_tipoClientesOnChange(elm) {

}

function obtenerCuentasCorporativas() {
    $.ajax({
        url: _ApplicationUrl + '/api/Helper/?diff2=2',
        cache: false,
        type: 'GET'
    }).done(function (response, textStatus, jqXHR) {

        var elm = $('#dg_cuentasCorporativas')
        $.each(response.Datos, function (i, v) {
            elm.append('<option value="' + v.Id + '">' + v.Descripcion + '</option>')
        })
    })
...
function dg_guardar() {
    ...
    if ($('#dg_uen').val() <= 0) {
        sucss = false
        $('#dg_uenError').show()
    } else $('#dg_uenError').hide()

    if ($('#dg_segmento').val() <= 0) {
        sucss = false
        $('#dg_segError').show()
    } else $('#dg_segError').hide()

    if ($('#dg_tipoClientes').val() <= 0) {
        sucss = false
        $('#dg_tipoClienteError').show()
    } else $('#dg_tipoClienteError').hide()

    if (sucss) {
        $('#dg_loader').show()

        var data = {
            IdCliente: _clienteSeleccionado.Id_Cte,
            Uen: $('#dg_uen').val(),
            Segmento: $('#dg_segmento').val(),
            TipoCliente: $('#dg_tipoClientes').val(),
            Cte_NomComercial: _clienteSeleccionado.Cte_NomComercial,
            Cte_Email: _clienteSeleccionado.Cte_Email
        }

        $.ajax({
            url: _ApplicationUrl + '/api/CatCliente/',
            type: 'POST',
            cache: false,
            contentType: 'application/json',
            data: JSON.stringify(data),
        ...
        var error = 'Se debe seleccionar las variables de: UEN, Segmento y Tipo de cliente en la sección de "Seguimiento" en el apartado de "Datos generales", para poder crear un nuevo proyecto para un prospecto o cliente nuevo en el CRM';

        $('#modalAlertaError').text(error)
        $('#modalAlerta').modal('show')
    }
}

/**
 * FIN DAVID BUSTAMANTE
 * */
...
function NuevoProyecto(obj) {
    ...
    var openModalProyecto = true
    var errorEnDatosGenerales = false

    if ((data.Id_Uen <= 0 || data.Id_Seg <= 0 || data.Id_TCte <= 0) && !_datosGeneralesGuardados) {
        openModalProyecto = false
        errorEnDatosGenerales = true
        if (data.Id_Uen <= 0) $('#dg_uenError').show()
        if (data.Id_Seg <= 0) $('#dg_segError').show()
        if (data.Id_TCte <= 0) $('#dg_tipoClienteError').show()
    ...
    CONSULTAR_TERRITORIOS_XCTE(data.Id_Cte, null, function (res) {
        if (res.some(x => x.Ter_Asociado == 1)) {
            $('#errorTerritorioAsociado').hide()
        } else {
            if (_territorioAsociadosAProspecto.length == 0 || parseFloat($('#txtDetallesAsociarTerritorio_Potencial').val()) <= 0) {
                openModalProyecto = false
                if (!errorEnDatosGenerales) $('#tabSeguimiento a[href="#dvSeccionTerritorios"]').tab('show');
                $('#errorTerritorioAsociado').show()
            } else {
                $('#errorTerritorioAsociado').hide()
            }
        }

        if (openModalProyecto) IniciaProyecto();
        else {
            var error = ''
            if (errorEnDatosGenerales) error = 'Se debe seleccionar las variables de: UEN, Segmento y Tipo de cliente en la sección de "Seguimiento" en el apartado de "Datos generales", para poder crear un nuevo proyecto para un prospecto o cliente nuevo en el CRM';
            else error = 'Se debe asignar territorio y valor potencial observado';
            $('#modalAlertaError').text(error)
            $('#modalAlerta').modal('show')
        }
    })
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
function selTerritorio_on_change(rowId) {
    var datosProspecto = $('#tblProspectos').DataTable().row(rowId).data();
    ...
    cargarSelUEN_Ver2($selUEN, datosProspecto.Id_Uen, datosProspecto.Uen_Descripcion);
    cargarSegmento_Ver2($selSegmento, datosProspecto, objTerritorio);
    selSegmento$on_change();
}
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
function cargarSegmento_Ver2(jqelement, dg, territorio) {
    jqelement.append('<option value="' + dg.Id_Seg + '">' + dg.Seg_Descripcion + '</option>');
    jqelement.selectpicker('val', 0);
    jqelement.selectpicker('refresh');

    $('#dvModalNuevoProyecto #hdnDim_Id_Uen').val(dg.Id_Uen);
    $('#dvModalNuevoProyecto #hdnDim_Id_Seg').val(dg.Id_Seg);
    if (territorio) {
        $('#dvModalNuevoProyecto #txtDimension').val(territorio.Seg_Unidades);
        $('#dvModalNuevoProyecto #txtPrecioUnidad').val(territorio.Seg_ValUniDim);

        _valorUnidadDimension = territorio.Seg_ValUniDim;
        var cantidad = $('#dvModalEditarProyecto #txtCantidad').val();
        cantidad = isNaN(cantidad) ? 0 : cantidad;
        $('#dvModalNuevoProyecto #txtVPM').val(cantidad * territorio.Seg_ValUniDim);
    }
}
...
function EditarProspecto(obj) {
   ...
   */
    ...
    var P = _datosProspectoAActualizar;

    cargarCamposDialogoEditarProspecto(idcrmprospecto);
    cargarDescripcion(rowidx);

    $('#dvModalEditarProspecto #txtRFC').val(P.Cte_Rfc);
    $('#dvModalEditarProspecto #txtNombre').val(P.Cte_NomComercial);
    $('#dvModalEditarProspecto #txtContacto').val(P.Cte_Contacto);

    $('#dvModalEditarProspecto #txtEmail').val(P.Cte_Email);
    $('#dvModalEditarProspecto #txtCalle').val(P.Cte_Calle);
    $('#dvModalEditarProspecto #txtTelefono').val(P.Cte_Telefono);
    $('#dvModalEditarProspecto #tbNoCte').val(P.Id_Cte);
    $('#dvModalEditarProspecto #hdnId_Cte').val(P.Id_Cte);
...
$(document).ready(function () {
    ...
    _tablaProspectos = $('#tblProspectos').DataTable({ /*"sDom": "<'dataTables_header' <'row' <'col-md-9' f i r> <'col-md-2' <'#tblProspectosToolbar'> > > >" +
        ...
        "columns": [
             ...
             },*/
            {
                ...
                'render': function (data, type, full, meta) {
                    return '<a ' +
                        'id="hrefCargarDescripcion_' + meta.row + '" ' +
                        'data-id_crmprospecto="' + data.Id_CrmProspecto + '" ' +
                        'href="javascript:cargarDescripcion(' + meta.row + ')">' +
                        full.Cte_NomComercial +
                        '</a>';
                }
            },
            // 1 Oct 2018 RFH
            {
                'data': 'Id_Cte',
                'render': function (data, type, full, meta) {
                    return '<a row=' + meta.row + '>' + full.Id_Cte + '</a>';
                }
            },
            {
                'data': null,
    ...
    */
    ...
    cargarUENs($, $selUEN);
    obtenerUENS()
    $('#dg_segmento').append('<option value="-1">-- Seleccione -- </option>')
    obtenerTipoClientes()
   ...
   */
    ...
    // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
    ///cargo la información de la tabla de LEads ( ejemplo ) 
    ...
    var table = $('#tblLeads').DataTable({
        ...
        'ajax': {
            'url': _ApplicationUrl + '/api/CrmLeads/?idEmp=' + _EntidadSesion_Id_Emp + '&idCd=' + _EntidadSesion_Id_Cd + '&idRik=' + _EntidadSesion_Id_Rik + '&idPantalla=' + 2,
            'dataSrc': ''
        },


        //            "ajax": "../archivodatos.txt",
        "columns": [

            { "data": "Acs_Fecha" },
            { "data": "Acs_Contacto3" },
            { "data": "Acs_Contacto2" },
            { "data": "Acs_NomComercial" },
            { "data": "Acs_Notas" },
            {
    ...
    function format(d) {
        ...
        return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
            // return '<table  class="datatable table table-striped table-bordered table-hover display">' +
            '<tr>' +
            ...
            //                '<td>' +  '<a  href="javascript:cargarDatosLeads(' + d.Id_Acs + ')">' +
            //                     d.Id_Acs + '</a>' + '</td>' +
