...
function ActualizarLeads(obj) {
    ...
    $.ajax({
        ...
        if (response = 1) {
            ...
            debugger;
            alertify.error('Cambios no realizados con éxito');
        }
        debugger;
        //            onSuccess(response);
        ////////        }
        ...
        switch (jqXHR.status) {
            ...
            default:
                ...
                if (Messge == 'An error has occurred.') {
                    Messge = '"Por favor seleccione un Rik o una sucursal no puede dejarlos en blanco o seleccionar los dos.';
                }
...
function cancelarLead($) {
    ...
    if (tipodecancelacion != 3) {
        ...
        if (leadMotivoCancelar.trim().length == 0) {
            $('#toastSuccess #toastSuccessMessage').html('Ingrese el motivo de cancelacíon');
            $('#toastSuccess').fadeIn();
            setTimeout(function () {
                $('#toastSuccess').fadeOut();
            }, 4000);
...
$(document).ready(function () {
    ...
    //MODIFICAR ATRINUTO DE ALIGN PARA QUE SE CENTREn LOS ICONOS
    ...
    var table = $('#tblLeads').DataTable({
        ...
        'ajax': {
            'url': _ApplicationUrl + '/api/CrmLeads/?idEmp=' + _EntidadSesion_Id_Emp + '&idCd=' + _EntidadSesion_Id_Cd + '&idRik=' + _EntidadSesion_Id_Rik + '&idPantalla=' + 1,
            'dataSrc': ''
        },
        ...
        "columns": [
                        ...
                        if (1 == 1) {
                            ...
                            if (row.Id_Rik != null) {
                                ...
                                if (row.Id_Rik == v.id) {  //use == instead of ===
                                    ...
                                    //JFCVcombos 16feb2021 5Mayo2021
                                    if (row.Id_Rik == -1) {
                    ...
                    $.each(sucoption, function (k, v) {
                        if (1 == 1) {
                            ...
                            if (row.Id_Cd == v.id) {  //use == instead of ===
                                ...
                                //JFCVcombos 16feb2021 JFCV 5MAYO2021 puse false en los dos 
                                if (row.Id_Cd == -1) {
            ...
            },
            {
                ...
                'data': 'Id_Acs',
                'render': function (data, type, full, meta) {
                    return '<button type="button" class="btn btn-primary" style="background-color:transparent; box-shadow: none; font-size:10px; vertical-align: middle;"   onclick="ActualizarLeads(this)" data-terid="' + full.Id_Acs + '" data-nombrecontacto="' + full.Acs_Contacto3 + '" ' + 'data-rik="' + full.Id_Rik + '" ' + 'data-idcd="' + full.Id_Cd + '" class="btn btn-primary">' +
                        '<i class="fa fa-check" aria-hidden="true" align="center"></i>' + '</button>';
                }
            },
            {
                "className": 'cancelar',
                'data': 'Id_Acs',
                'render': function (data, type, full, meta) {
                    return '<button type="button" class="btn btn-primary" style="background-color:transparent; box-shadow: none; font-size: 10px; vertical-align: middle;"    onclick="javascript:inicializarModalCancelarLead(' + full.Id_Acs + ')" data-terid="' + full.Id_Acs + '" data-nombrecontacto="' + full.Acs_Contacto3 + '" ' + 'data-rik="' + full.Id_Rik + '" ' + 'data-idcd="' + full.Id_Cd + '" class="btn btn-primary">' +
                        '<i class="fa fa-times" aria-hidden="true" align="center" ></i>' + '</button>';
    ...
    function format(d) {
        ...
        return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
            ...
            '</tr>' +
            '</table>' +
            //agregar historico en pantalla gerente JFCV 2 mzo 2021

            '<div class="row">' +

            '<div class="col-sm-4 bg-primary text-white">Logs</div>' +
            '<div class="col-sm-2 bg-primary text-white">Fecha</div>' +
            '<div class="row">' + ' </div >' +
            d.Acs_Contacto5 +
            ' </div >';
        ...
        //'<tr>' +
        //'<th  class="col-sm-7">Logs</th>' +
        //'<td  class="col-sm-3">12-02-2021</td>' +
