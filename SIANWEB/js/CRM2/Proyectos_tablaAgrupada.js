/*
    Key Quimica 2019
    28 feb 2019 - Se agrega function para eliminar fisicamente proyecto.
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
// ESTE ES EL BUENO 
function servicioCancelarProyecto_Ver2(idCte, idOp, idCausa, onSuccess, onFailure, always) {
       
    $.ajax({
        //url: _ApplicationUrl + '/api/CancelarProyecto',
        url: _ApplicationUrl + '/api/CancelarProyecto?Id_Op='+idOp+'&Id_Cte='+idCte+'&Id_Causa='+idCausa,
        type: 'GET',
        cache: false,        
        contentType: 'application/json',
        /*data: JSON.stringify({
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/* se movio a CRM_Productos.js
function $aceptarEditarCantidad($C, idPrd) {
    
    console.log('Proyecos_tablaAgrupada.js: function $aceptarEditarCantidad');
...
function cargarInfoSeccionGeneral(datos){            
    ...
    $('#hf_Id_Op').val(datos.Id_Op);

    $('#hf_Id_EmpGen').val(datos.Id_Emp);
    $('#hf_Id_CdGen').val(datos.Id_Cd);
    $('#hf_Id_CteGen').val(datos.Id_Cte);
    $('#hf_Id_CteDetGen').val(datos.Id_CteDet);
    $('#hf_Id_TerGen').val(datos.Id_Ter);
    $('#hf_Id_SegGen').val(datos.Id_Seg);

    var VPT = 0;
    VPT = parseFloat(datos.ValorPotencialTeorico);
    if (isNaN(VPT)) {
        VPT = 0;
    } /* else {
        VPT = 0;
    ...
    */
    $('#ddVPT').text(numeral(VPT).format('$ 0,0.00'));

    var PME = 0;
    PME = parseFloat(datos.VentaPromedioMensualEsperada);
    if (isNaN(PME)) {
        PME = 0;
    }
    /*
    ...
    */
    $('#ddVPME').text(numeral(PME).format('$ 0,0.00'));

    var PMEAC = 0;
    PMEAC = parseFloat(datos.VentaPromedioMensualEsperadaAntesCierre);
    if (isNaN(PMEAC)) {
        PMEAC = 0;
    }
    $('#ddVPMEAC').text(numeral(PMEAC).format('$ 0,0.00'));

    var btnEditaPME = document.getElementById("imgEdicionPME");
    var txtEditaPME = document.getElementById("txtVPME");

    txtEditaPME.value = PME;
    //  console.log(datos);
    btnEditaPME.style.visibility = "hidden";
    txtEditaPME.style.visibility = "hidden";
    if ((datos.Estatus < 4) && (datos.Valuacion == 0) ) {
        btnEditaPME.style.visibility = "visible";
        txtEditaPME.style.visibility = "visible";
    }
...
function cargarProductosDeProyecto(oportunidadSeleccionada, clienteDeOportunidad, datos) {
    var ESTATUS = $('#ESTATUS_'+oportunidadSeleccionada).val();
    var VAP_ESTATUS = $('#VAP_ESTATUS_'+oportunidadSeleccionada).val();
    var VAP_ESTATUS2 =  $('#VAP_ESTATUS2_'+oportunidadSeleccionada).val();
...
function $obtenerRutaDeOferta($C, idCte, idOp) {
    ...
    //console.log('Productos:'+idCte+' '+idOp+' '+$C.selector);
    ...
    $.ajax({
        ...
        alertify.error('Error: No se encontro información ('+$C.selector+')');
...
function obtenerRutaDeOferta(idCte, idOp) {
    ...
    $.ajax({
        ...
        alertify.error('Ocurrió un error al obtener la información del producto(8)');
        //establecer un tooltip en el comando para señalar el error asociado.
    }).always(function (jqXHR, textStatus, errorThrown) {
...
function $cargarInfoProductoAplicacion($C, idCte, idOp, idPrd) {
    ...
    $.ajax({
        ...
        alertify.error('Ocurrió un error al obtener la información del producto(9)');
    }).always(function (jqXHR, textStatus, errorThrown) {
...
function $cargarInfoProducto($C, id) {
    ...
    $.ajax({
        ...
        alertify.error('Ocurrió un error al obtener la información del producto(10)');
    }).always(function (jqXHR, textStatus, errorThrown) {
        $C.find('#imgBuscandoProducto').fadeOut();
    });
}
...
function cargarInfoProductoCatalogoUnico($C, id) {
    ...
    $.ajax({
        ...
        alertify.error('Ocurrió un error al obtener la información del producto(11)');
    }).always(function (jqXHR, textStatus, errorThrown) {
        $C.find('#imgBuscandoProducto').fadeOut();
    });
}
...
function cancelarProyecto(sender){                     
    ...
    if (!btnDiabled) {
        ...
        .cancelBtn("No")
        .confirm("<b>Cancelar Proyecto</b><br/>Est&aacute; a punto de cancelar el proyecto. "+
        "¿Est&aacute; seguro de que desea continuar?", function (ev) {                
            ev.preventDefault();                
...
function Proyecto_EliminarFisicamente(Id_Op) {

    var ClaveConfirmacion = '0000';

    $.ajax({
        url: _ApplicationUrl + '/api/CrmProyecto/EliminarFisicamente?Id_Op=' + Id_Op + '&Eliminar=1&ClaveConfirmacion='+ClaveConfirmacion,
        method: 'GET',
        cache: false,
        statusCode: {
            401: function (jqXHR, textStatus, errorThrown) {
        ...
        if (Estado == 1) {            
            $('#ProyectoRow_'+Id_Op).remove();        
            alertify.success('Se ha el eliminado fisicamente el proyecto '+Id_Op);
        } else {
            alertify.error('Ocurrio un error al intentar eliminar el proyecto '+Id_Op);
        }
...
// ELIMINAR /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
function EliminarProyecto(sender){
    ...
    if (!btnDiabled) {
        // Solo si esta habilitado
        alertify
        .okBtn("Si, ELIMINAR el Proyecto")
        .cancelBtn("No")
        .confirm("<b>ELIMINAR Proyecto</b><br/>Est&aacute; a punto de ELIMINAR el proyecto "+ Id_Op +
        ". ¿Est&aacute; seguro de que desea continuar? <br/><br/><br/>" + 
        '<span class="label label-danger">Ejecutar esta acción NO ES reversible</span>', function (ev) {                
            ev.preventDefault();                
            //$('#dvModalCancelarProyecto input[name="CausaCancelacion"]').attr('checked', false); 
            //$('#dvModalCancelarProyecto').modal('show');
            Proyecto_EliminarFisicamente(Id_Op);
...
function GuardaNuevoPME(sender) {
    var btnDiabled = $(sender).hasClass('disabled');
    var Id_Op = $('#hf_Id_Op').val();

    var txtEditaPME = document.getElementById("txtVPME");
    var vvPMEE = txtEditaPME.value;

    var oIdEmp = document.getElementById("hf_Id_EmpGen");
    var oIdCdi = document.getElementById("hf_Id_CdGen");
    var oIdCte = document.getElementById("hf_Id_CteGen");
    var oIdCteDet = document.getElementById("hf_Id_CteDetGen");
    var oIdTer = document.getElementById("hf_Id_TerGen");
    var oIdSeg = document.getElementById("hf_Id_SegGen");

    AjustarPME(oIdEmp.value, oIdCdi.value, oIdCte.value, Id_Op, oIdTer.value, oIdSeg.value, vvPMEE);

}


// ELIMINAR FISICAMENTE /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
function AjustarPME(IdEmp, IdCDI, IdCte, IdOp, IdTer, IdSeg, vaaalor) {

    var vURL = _ApplicationUrl + '/api/CrmProyecto/AjustaPME?IdEmp=' + IdEmp + '&IdCDI=' + IdCDI + '&IdCte=' + IdCte + '&IdOp=' + IdOp + '&IdTer=' + IdTer + '&IdSeg=' + IdSeg + '&valor=' + vaaalor
    //      console.log(vURL);
    $.ajax({
        url: vURL,
        method: 'GET',
        ...
        Datos = res.Datos;
        if (Estado == 1) {
            alertify.success('Se ha modificado el PME !!');
            //  element.VentaPromedioMensualEsperada = vaaalor;
            $('#ddVPME').text(numeral(vaaalor).format('$ 0,0.00'));
            /// console.log(element);
        } else {
            alertify.error('Ocurrio un error al intentar modificar el PME.');
        }
