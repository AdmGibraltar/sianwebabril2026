...
namespace SIANWEB
{
    public partial class ObtenerProducto_ES : System.Web.UI.Page
    {
        private List<EntradaSalidaDetalle> list_Es
        {
            get { return (List<EntradaSalidaDetalle>)Session["ListEs" + Session.SessionID + HF_ClvPag]; }
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        try
                        {
                            cn_pro.ConsultaProducto(ref producto, sesion.Emp_Cnx, sesion.Id_Emp, sesion.Id_Cd_Ver, sesion.Id_Cd_Ver, Convert.ToInt64(prd_), ValidaInv);
                            ...
                            {
                                valor_retorno = Producto_Cantidad(sesion, valor_retorno, nat_, gpo_, Convert.ToInt64(prd_), ref_, Convert.ToInt32(es_), Convert.ToInt32(ter_), can_, mov_, cte_, producto);
                            }
                            else if (acc_ == "cos") //COSTO
                            {
                                ...
                                if (valor_retorno == "1")
                                {
                                    ...
                                    if (ConfigurationManager.AppSettings["AlertaCostoDifEnt"].ToString() == "S")
                                    {
                                        float PrecioAAA = obtenerPrecioAAA(prd_);

                                        if (((Convert.ToDouble(pre_)- PrecioAAA)>= 0.001) || ((Convert.ToDouble(pre_) - PrecioAAA)<=-0.001))
                                        {
                                            valor_retorno = "-1@@" + "El Costo registrado es diferente al AAA del sistema :" + PrecioAAA.ToString();
                                        }
                                    }
        ...
        private string Producto_Cantidad(Sesion sesion, string valor_retorno, string nat_, string gpo_, Int64 id_prd, string ref_, int es_, int ter_, int can_, string mov_, string cte_, Producto producto)
        {
            try
            {
                ...
                if (gpo_ == "4" || gpo_ == "2")
                {
                    ...
                    int cantidadEnDt = 0;
                    //Con los codigos 999 ya no es necesario
        ...
        private string Producto_Valido(Sesion sesion, string valor_retorno, string gpo_, string ref_, string ter_, string prd_, Producto producto)
        {
            try
            {
                ...
                if (gpo_ == "2")
                {
                    ...
                    else if (Convert.ToInt64(prd_) > 999000000)
                    {
                        valor_retorno = "-1@@" + "El código de producto no puede ser Usado";
                    }
                }
                else if (gpo_ == "3")
                {
                ...
                if (valor_retorno == "")
                {
                    ...
                    valor_retorno = "@@" + precio.ToString() + "@@" + producto.Prd_Presentacion + "@@" + producto.Prd_Descripcion + "@@" + producto.Prd_Unico;
        ...
        private float obtenerPrecioAAA(string Id_Prd)
        {
            try
            {
                ...
                int Id_Pre = 0;
                cn_proprec.ConsultaListaProductoPrecioAAA(ref precio, Sesion, Convert.ToInt64(Id_Prd), ref Id_Pre);
                return precio;
            }
            catch (Exception ex)
            {
