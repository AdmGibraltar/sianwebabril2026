...
namespace SIANWEB
{
    public partial class CapOrdCompra : System.Web.UI.Page
    {
        ...
        protected float ObtenerPrecioAAA(object oc)
        {
            if (oc is OrdenCompraDet)
            {
                float salida = ((OrdenCompraDet)oc).ProductoPrecio.Prd_Pesos;
                return salida;
            }
            else
            {
        ...
        protected float ObtenerTotal(object oc)
        {
            if (oc is OrdenCompraDet)
            {
                float salida = (((OrdenCompraDet)oc).ProductoPrecio.Prd_Pesos) * (((OrdenCompraDet)oc).Ord_Cantidad);
                return salida;
            }
            else
            {
        ...
        protected string ObtenerPromVtaMens(object oc)
        {
            if (oc is OrdenCompraDet) { return ((OrdenCompraDet)oc).Producto.VtaCPromedio; } else { return string.Empty; }
        }

        protected string ObtenerPorcPromVtaMensUnidades(object oc)
        {
            if (oc is OrdenCompraDet) { return ((OrdenCompraDet)oc).Producto.VtaUPromedio.ToString(); } else { return string.Empty; }
        }

        protected string ObtenerPorcPromVtaMens(object oc)
        {

            if (oc is OrdenCompraDet)
            {
                var UnVsProm = ((OrdenCompraDet)oc).Producto.VtaUPromedio == 0 ? 0 : Math.Round(Convert.ToDecimal(((((OrdenCompraDet)oc).Ord_Cantidad) / Convert.ToDouble(((OrdenCompraDet)oc).Producto.VtaUPromedio)) * 100), 2);

                return UnVsProm.ToString() + " %"; ;
            }
            else
            {
                return string.Empty; ;
        ...
        protected string ObtenerNotas(object oc)
        {
            if (oc is OrdenCompraDet) { return ((OrdenCompraDet)oc).Producto.Prd_Notas; } else { return string.Empty; }
        }
        ...
        protected void ListaProductosOrdenCompra_EliminarProducto(long id_Prd)
        {
            List<OrdenCompraDet> lista = this.ListaProductosOrdenCompra;
        ...
        protected void rgOrdCompra_InsertCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                ordenCompraDet.Producto.VtaCPromedio = (insertedItem["Prd_PromVtaMens"].FindControl("lblEditPrd_PromVtaMens") as Label).Text;
                ordenCompraDet.Producto.VtaUPromedio = Convert.ToInt32((insertedItem["Prd_ProcPromVtaMens"].FindControl("HD_Prd_ProcPromVtaMens") as HiddenField).Value);
                ordenCompraDet.Producto.Prd_Notas = (insertedItem["Prd_Notas"].FindControl("lblEditPrd_Notas") as Label).Text;
        ...
        protected void rgOrdCompra_UpdateCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                float Prd_Precio = Convert.ToSingle(((HiddenField)editedItem.FindControl("HD_Prd_PrecioAAA")).Value);
                ordenCompraDet.ProductoPrecio.Prd_Pesos = Prd_Precio;

                ordenCompraDet.Producto.VtaCPromedio = (editedItem["Prd_PromVtaMens"].FindControl("HD_Prd_PromVtaMens") as HiddenField).Value;
                ordenCompraDet.Producto.VtaUPromedio = Convert.ToInt32((editedItem["Prd_ProcPromVtaMens"].FindControl("HD_Prd_ProcPromVtaMensUnidades") as HiddenField).Value);
                ordenCompraDet.Producto.Prd_Notas = (editedItem["Prd_Notas"].FindControl("HD_Prd_Notas") as HiddenField).Value;
        ...
        protected void txtProducto_TextChanged(object sender, EventArgs e)//protected void cmbProducto_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                if (txtId_Prd.Value.ToString() != string.Empty && txtId_Prd.Value.ToString() != "-1")
                {
                    ...
                    clsProducto.ConsultaProducto(ref producto, sesion.Emp_Cnx, sesion.Id_Emp, sesion.Id_Cd_Ver, sesion.Id_Cd_Ver, Convert.ToInt32(txtId_Prd.Value.ToString()), 3);
                }
                ...
                else
                {
                    ...
                    ((Label)tabla.FindControl("lblPrd_PrecioAAA")).Text = producto == null ? string.Empty : string.Format("{0:$#,##0.00;($#,##0.00);''}", Convert.ToDecimal(producto.Prd_Precio.ToString()));

                    ((Label)tabla.FindControl("lblEditPrd_PromVtaMens")).Text = producto == null ? string.Empty : string.Format("{0:$#,##0.00;($#,##0.00);''}", Convert.ToDecimal(producto.VtaCPromedio.ToString()));

                    ((Label)tabla.FindControl("lblEditPrd_ProcPromVtaMens")).Text = ""; //// producto == null ? string.Empty : producto.VtaUPromedio.ToString();
                    ((HiddenField)tabla.FindControl("HD_Prd_ProcPromVtaMens")).Value = producto == null ? string.Empty : producto.VtaUPromedio.ToString();
                    ((HiddenField)tabla.FindControl("HD_Prd_ProcPromVtaMensUnidades")).Value = producto == null ? string.Empty : producto.VtaUPromedio.ToString();
                    ((Label)tabla.FindControl("lblEditPrd_Notas")).Text = producto == null ? string.Empty : producto.Prd_Notas;
            ...
            catch (Exception ex)
            {
                ...
                txtPrd_Descripcion.Text = string.Empty;
                txtPrd_Presentacion.Text = string.Empty;
                txtPrd_UniNe.Text = string.Empty;
                txtPrd_CantGen.Text = "0";
                ((HiddenField)tabla.FindControl("HD_Prd_PrecioAAA")).Value = string.Empty;
                ((Label)tabla.FindControl("lblPrd_PrecioAAA")).Text = string.Empty;

                //--------controles auxiliares--------
                //establecer unidades de empaque
                ((HiddenField)tabla.FindControl("HD_Prd_UniEmp")).Value = string.Empty;
        ...
        protected void txtCantidad_TextChanged(object sender, EventArgs e)//protected void cmbProducto_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                /// identifica los nuevos controles Label y les pone un valor dummy por mientras se consulta el dato directamente
                /// 

                HiddenField HDPrecioAAA = ((HiddenField)tabla.FindControl("HD_Prd_PrecioAAA"));
                double PrecioAAA = Convert.ToDouble(HDPrecioAAA.Value == "" ? "0" : HDPrecioAAA.Value);

                Label lblPrd_Total = ((Label)tabla.FindControl("lblEditPrd_Total"));
                lblPrd_Total.Text = string.Format("{0:$#,##0.00;($#,##0.00);''}", Convert.ToDecimal((Ctrl_txtOrd_Cantidad.Value * PrecioAAA).ToString()));

                Label lblPrd_PromVtaMens = ((Label)tabla.FindControl("lblEditPrd_PromVtaMens"));
                /// var UnVsProm = Convert.ToDecimal( (Ctrl_txtOrd_Cantidad.Value / Convert.ToDouble(lblPrd_PromVtaMens.Text) ) );
                //  lblPrd_PromVtaMens.Text = UnVsProm.ToString() + " %";

                HiddenField HddPrd_ProcPromVtaMens = ((HiddenField)tabla.FindControl("HD_Prd_ProcPromVtaMens"));
                HiddenField HD_Prd_ProcPromVtaMensUnidades = ((HiddenField)tabla.FindControl("HD_Prd_ProcPromVtaMensUnidades"));
                Label lblPrd_ProcPromVtaMens = ((Label)tabla.FindControl("lblEditPrd_ProcPromVtaMens"));

                Decimal UnVsProm = 0;

                if (Convert.ToDouble(HD_Prd_ProcPromVtaMensUnidades.Value) > 0)
                {
                    UnVsProm = Math.Round(Convert.ToDecimal((Ctrl_txtOrd_Cantidad.Value / Convert.ToDouble(HD_Prd_ProcPromVtaMensUnidades.Value)) * 100), 2);
                }


                lblPrd_ProcPromVtaMens.Text = UnVsProm.ToString() + " %";
                /*
                Label lblPrd_Notas = ((Label)tabla.FindControl("lblEditPrd_Notas"));
                lblPrd_Notas.Text = " [Sobre Pedido] <br><br> Codigo " + txtPrd_Descripcion.Text + " al ser un código sobre pedido, puede estar sujeto a ser rechazada una solicitud de modificación de pedidos";
                */
                /// this.ValidaAutorizacion(txtPrd_Descripcion.Text);
                ...
                if (hiddenId.Value == string.Empty)
                {
                    ...
                    foreach (OrdenCompraDet Partida in ordCompra.ListOrdenCompra)
                    {
                        ...
                        if (Prod.Count() > 0)
                        {
                            ...
                            {
                                long prod = Partida.Id_Prd - 999000000;
                            ...
                            {
                                long prod2 = Partida.Id_Prd + 999000000;
                            ...
                            foreach (DataRow Row in listaPartidas.Select("Id_Prd =" + Partida.Id_Prd))
                            {
                                OrdenCompraDet Partida2 = new OrdenCompraDet();
                                Partida2 = Partida;
                                //se valida que sea un sistema propietario
                                if (int.Parse(Row["Id_Ptp"].ToString()) == 1)
                                    listaPartidanoaceptado.Add(Partida2);
                                else
                                    listaPartida.Add(Partida2);
                            }
                    ...
                    if (listaPartida.Count > 0)
                    {
                        ...
                        String ValidaParametrosOC = "";

                        if (ConfigurationManager.AppSettings["ValidaParametrosOC"].ToString() == null)
                        {
                            ValidaParametrosOC = "N";
                        } else
                        {
                            if (ConfigurationManager.AppSettings["ValidaParametrosOC"].ToString() == "")
                            {
                                ValidaParametrosOC = "N";
                            } else
                            {
                                ValidaParametrosOC = ConfigurationManager.AppSettings["ValidaParametrosOC"].ToString();
                            }
                        }

                        

                        clsCapOrdCompra.InsertarOrdenCompra(ref ordCompra, session.Emp_Cnx, ValidaParametrosOC, ref verificador);
                    }
        ...
        private void ValidaAutorizacion(string Produucto)
        {
            string mmsg = "El monto del producto " + Produucto + " hace que la orden de compra que sea mas del 30% del promedio de la sucursal, se va a solicitar una autorización por parte del JOS.";
            Alerta(mmsg);
            ...
            try
            {
                ...
                cuerpo_correo.Append("</tr><tr>");
                cuerpo_correo.Append("<td colspan='2'><b><font face='Tahoma' size='2'>");
                cuerpo_correo.Append("Se ha colocado una solicitud de autorización de una orden de compra con el producto  " + Produucto);
                cuerpo_correo.Append("</td></tr><tr><td><b><font face='Tahoma' size='2'>");
                cuerpo_correo.Append("Centro de distribución:  " + session.Id_Cd_Ver + " - " + session.Cd_Nombre);
                cuerpo_correo.Append("</td></tr><tr><td><b><font face='Tahoma' size='2'>");
                cuerpo_correo.Append("Solicitó : " + session.Id_U + " - " + session.U_Nombre);
                ...
                * /
                ...
                m.From = new MailAddress(configuracion.Mail_Remitente);
                //  m.To.Add(new MailAddress(configuracion.Mail_OrdenCompra_sisprop));
                m.To.Add(new MailAddress("enrique.mendez.castillo@outlook.com"));
                m.Subject = "Solicitud de autorización de Orden de Compra del Producto " + Produucto + " del centro " + session.Id_Cd_Ver;
                m.IsBodyHtml = true;
                string body = cuerpo_correo.ToString();
                AlternateView vistaHtml = AlternateView.CreateAlternateViewFromString(body, null, MediaTypeNames.Text.Html);
                //Esto queda dentro de un try por si llegan a cambiar la imagen el correo como quiera se mande
