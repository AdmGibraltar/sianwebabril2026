...
namespace SIANWEB
{
    public partial class Login : System.Web.UI.Page
    {
                    ...
                    if (Session["Sesion" + Session.SessionID] != null)
                    {
                        ...
                        if (salir != 1 && salir != 2)
                        {
                            try
                            {
                                ...
                                {
                                    Response.Redirect("ProAuxiliarV2.aspx");
                                }
                            }
                            catch (Exception ex2)
                            {
        ...
        bool TieneVersionTerritorio(int id_cd)
        {
            using (var connection = new SqlConnection(ConfigurationManager.AppSettings["strConnectionCentral"]))
            {
                connection.Open();

                return connection.Query<bool>("select habilitado from VersionTerritorio where id_cd = @id_cd", new
                {
                    id_cd
                }).FirstOrDefault();
            }
        ...
        private void Entrar(Usuario usuario)
        {
            ...
            if (Id == 1)
            {
                ...
                if (usuario.Cu_Estatus == true)
                {
                    ...
                    sesion.AnioActual = usuario.AñoActual;

                    sesion.VersionTerritorio = TieneVersionTerritorio(usuario.Id_Cd);
                    ...
                    if (sesion.VersionTerritorio)
                    {
                        ...
                        {
                            //if (row["Sm_href"].ToString().ToLower().Contains("catclientes"))
                            //{
                            //    string value = row["Sm_href"].ToString().ToLower();
                            //    row.SetField("Sm_href", value.Replace("catclientes", "catclientesV3"));
                            //}

                            if (row["Sm_href"].ToString().ToLower().Contains("catterritorios_index"))
                            {
                                string value = row["Sm_href"].ToString().ToLower();
                                row.SetField("Sm_href", value.Replace("catterritorios_index", "catterritorios_indexvT"));
                            }

                            if (row["Sm_href"].ToString().ToLower().Contains("prospectosv2"))
                            {
                                string value = row["Sm_href"].ToString().ToLower();
                                row.SetField("Sm_href", value.Replace("prospectosv2", "prospectosv2vT"));
                            }
                        }
                    }
