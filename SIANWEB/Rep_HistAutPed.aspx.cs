...
namespace SIANWEB
{
    public partial class Rep_HistAutPed : System.Web.UI.Page
    {
        #region Variables
        ...
        {
            if (Session["RepHistPed"] != null)
            {
                List<PedidoVtaInst> Lista = (List<PedidoVtaInst>)Session["RepHistPed"];
                GrdHistAutPed.DataSource = Lista;
                GrdHistAutPed.DataBind();
            }
        }
        ...
        private void CargarGrid()
        {
            try
            {
                ...
                CN.ConsultarLogHistPedido(Datos, ref List, session2.Emp_Cnx);

                Session["RepHistPed"] = List;
                GrdHistAutPed.DataSource = List;
                GrdHistAutPed.DataBind();
            }
            catch (Exception ex)
            {
        ...
        {
            GrdHistAutPed.ExportXlsToResponse(new XlsExportOptionsEx { ExportType = ExportType.WYSIWYG });
        }
