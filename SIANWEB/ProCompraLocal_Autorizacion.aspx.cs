...
namespace SIANWEB
{
    public partial class ProAutCompraLocal : System.Web.UI.Page
    {
        ...
        protected void rtb1_ButtonClick(object sender, RadToolBarEventArgs e)
        {
            try
            {
                if (Page.IsValid)
                {
                    ...
                    {
                        //  Regresar()
                        Response.Redirect("ComprasLocales.aspx");
                    }
                }
            }
            catch (Exception ex)
                    ...
                    if (session2.Id_Cd_Ver.ToString() == IdCd)
                    {
                        if (IdSol != "")
                        {
                            ...
                            else
                            {
                                ...
                                hddCorreoSOlicitante.Value = cl.TipoSolicitud;
                                lblTipoSolicitud.Text = cl.IdTipoSolicitud.ToString();
                                hddTipoSolicitud.Value = cl.IdTipoSolicitud.ToString();

                                lblFechaVigencia.Text = cl.Vigencia.ToString();

                                lblComentarios.Text = cl.Comentarios.Replace("\r\n", "<br>");

                                if (lblFechaVigencia.Text == "01/01/1900")
                                {
                                    Label7.Visible = false;
                                    lnkModificar.Visible = false;
                                    lblFechaVigencia.Visible = false;
                                }
                                else
                                {
                                    rdpVigencia.SelectedDate = Convert.ToDateTime(cl.Vigencia);
                                }
                                if (lblComentarios.Text == string.Empty)
                                {
                                    Label8.Visible = false;
                                }
                                // SI es Tipo 1 se oculta de Costo CEDIS.
                                if (hddTipoSolicitud.Value == "1")
                                {
                                    rg1.Columns[6].Visible = false;
                                }

                                //  si es TIpo 3 se ocultan las columnas de CEDIS       6,7,8
                                if (hddTipoSolicitud.Value == "3")
                                {
                                    rg1.Columns[6].Visible = false;
                                    rg1.Columns[7].Visible = false;
                                    rg1.Columns[8].Visible = false;
                                }
                                else
                                {
                                    rg1.Columns[6].Visible = true;
                                    rg1.Columns[7].Visible = true;
                                    rg1.Columns[8].Visible = true;
                                }
                            }
                        }
                        else
        ...
        private void Guardar()
        {
            try
            {
                ...
                foreach (GridDataItem gi in rg1.Items)
                {
                    ...
                    {
                        CantidadAutorizados++;
                        if ((gi["Autoriza"].FindControl("chkAutoriza") as CheckBox).Checked )
                        {
                            EstatusSolicitu = "AUTORIZADA";
                        }
                        if ((gi["Rechaza"].FindControl("chkRechaza") as CheckBox).Checked)
                        {
                            EstatusSolicitu = "RECHAZADA";
                        }
                    }
                ...
                if (this.lblFechaVigencia.Text != "")
                {
                    CN_ComprasLocales comLoc = new CN_ComprasLocales();
                    FVigencia = this.rdpVigencia.ValidationDate.ToString();
                    

                    comLoc.GrabaVigencia(FVigencia, cl.Id_Comp, session.Emp_Cnx, ref grabo);
                }
        ...
        private void CorreosAutorizadorxMotivo(ref string Mails)
        {
            ...
            int Motivo2 = Convert.ToInt32(hddTipoSolicitud.Value);
        ...
        private void CorreosAutorizadorxMotivoxApp(ref string Mails)
        {
            Sesion Sesion = new Sesion();
            Sesion = (Sesion)Session["Sesion" + Session.SessionID];
            CN_ComprasLocales cn_Listadocompralocal = new CN_ComprasLocales();
            ...
            int aplica = 1;
            cn_Listadocompralocal.CorreosAutorizadorxMotivoxApp(ref Mails, 0, aplica, Sesion.Id_Emp, Sesion.Id_Cd, Sesion.Emp_Cnx);

        }

        private void EnviaEmail(int cambioVigencia, string FVigencia)
        {
            try
            {
                Sesion session = new Sesion();
                ...
                cuerpo_correo.Append("Se ha dado respuesta a la solicitud # " + lblFolio.Text + ".</td></tr>");
                ...
                foreach (GridDataItem gi in rg1.Items)
                {
                    idProd = gi["Id_Prd"].Text;
                    DeProducto = gi["Descripcion"].Text;

                    cuerpo_correo.Append("<tr><td colspan='2'> <font face= 'Tahoma' size = '2'> En la cual se incluye el producto " + idProd + " - " + DeProducto + " </font> </td></tr>");
                    
                }

                // evaluar si se cambio la vigencia para poder 

                cuerpo_correo.Append("<tr><td colspan='2'>");
                if (FVigencia != "")
                {
                    cuerpo_correo.Append("<tr><td colspan='2'> <font face= 'Tahoma' size = '2'> Se le modifico la vigencia a la fecha " + FVigencia + " </font> </td></tr>");
                }
                ...
                cuerpo_correo.Append("<center style='font-family: verdana; font-size: 16pt;'>" + EstatusSolicitu +"</center>");
                
                //  cuerpo_correo.Append("<center><br><a style='font-family: verdana; font-size: 8pt;' href='" + Request.Url.ToString().Replace(url[url.Length - 1], "") + "ComprasLocales.aspx?SolCompra=" + lblFolio.Text + "'>Solicitud de autorización de compra local</a></font></center>");
                ...
                if (hddCorreoSOlicitante.Value != "")
                {
                    m.To.Add(new MailAddress(hddCorreoSOlicitante.Value));
                }
                else
                {
                ...
                sm.Send(m);
                Alerta("Respuesta enviada correctamente");
            }
            catch (Exception)
            {
