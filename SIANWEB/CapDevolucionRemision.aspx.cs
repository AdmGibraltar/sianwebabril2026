    ...
    public partial class CapDevolucionRemision : PaginaBase
    {
        ...
        protected void BtnFacturar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (GridDataItem item in rgDevParcial.MasterTableView.Items)
                {
                    ...
                    {
                        if (Convert.ToInt64(item["Id_Prd"].Text) == compare.Id_Prd)
                        {
                ...
                gridItems.ToList().ForEach(gi =>
                {
                    ...
                    remision.Id_Prd = Convert.ToInt64(gi["Id_Prd"].Text);
        ...
        protected IEnumerable<RemisionesVencidas> ObtieneListaRemisionesProducto(IEnumerable<GridDataItem> grdItems)
        {
            foreach (GridDataItem item in grdItems)
            {
                ...
                rem.Id_Prd = Convert.ToInt64(item["Id_Prd"].Text);
        ...
        protected void BtnDetalleRemisiones_Click(object sender, EventArgs e)
        {
            ...
            idRemision = grdItems.Select(prods => prods["Id_Rem"].Text);
            dr.Id_Rem = string.Join(",", idRemision);
            ...
            lRemisionDet = lRemisionDet.Where(rem => rem.Rem_Estatus != "C")
                                       .Join(lRemVencidas, rem => new { Id_Ter = rem.Id_Ter.Value, Id_Prd = rem.Id_Prd, Id_Rem = rem.Id_Rem },
                                                           grd => new { Id_Ter = grd.Id_Ter, Id_Prd = grd.Id_Prd, Id_Rem = grd.Id_Rem }, (rem, grd) => new { rem, grd })
                                       .Select(s => s.rem).ToList();
        ...
        protected void BtnEntradaRemision_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                IEnumerable<string> enumRemision = gridItems.Select(s => s["Id_Rem"].Text);
                dR.Id_Rem = string.Join(",", enumRemision);
                ...
                foreach (GridDataItem item in rgDetalleRemision.MasterTableView.Items)
                {
                    ...
                    int cant = 0;
                    bool flag = false;
                    IdProd = Convert.ToInt64(item["Id_Prd"].Text);
                    ...
                    foreach (RemisionDet rd in lRemision.Where(x => x.Id_Prd == IdProd && x.Id_Rem == IdRem && x.Id_Ter == IdTer))
                    {
                        if (vCantidadT != 0)
                        {
                            ...
                            clsProducto.ConsultaProducto(ref producto, gSession.Emp_Cnx, gSession.Id_Emp, gSession.Id_Cd_Ver, gSession.Id_Cd_Ver, rd.Id_Prd, 4);
                    ...
                    else
                    {
                        if ((cantidad != "") && (cantidad != "0"))
                        {
                            if (RblRealizar.SelectedValue == "1")
                            {
                                ...
                                long Id_Prd = Int64.Parse(item["Id_Prd"].Text);
                                float precio = 1;
            ...
            try
            {
                ...
                foreach (GridDataItem item in rgDevParcial.MasterTableView.Items)
                {
                    ...
                    foreach (RemisionDet rd in lRemision)
                    {
                        if (CantidadT != 0)
                        {
                            if (Convert.ToInt64(item["Id_Prd"].Text) == rd.Id_Prd)
                            {
        ...
        protected void txtTipoId_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (val == 80)
                {
                    RblRealizar.Items[1].Enabled = false;
                    RblRealizar.SelectedIndex = 0;
                }
                else
                {
                    RblRealizar.Items[1].Enabled = true;
