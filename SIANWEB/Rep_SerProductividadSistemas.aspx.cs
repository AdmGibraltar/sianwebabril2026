    ...
    public partial class Rep_SerProductividadSistemas : System.Web.UI.Page
    {
        ...
        private void Imprimir(bool a_pantalla)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(cmbCategoria.SelectedValue))
                    this.ValidaRangosSintaxis(cmbCategoria.SelectedValue, ref error);

                if (!string.IsNullOrEmpty(cmbGrupo.SelectedValue))
                    this.ValidaRangosSintaxis(cmbGrupo.SelectedValue, ref error);
                ...
                ALValorParametrosInternos.Add(cmbCategoria.SelectedValue == string.Empty ? "Todos" : cmbCategoria.SelectedValue);
                ALValorParametrosInternos.Add(cmbCategoria.SelectedValue);

                ALValorParametrosInternos.Add(cmbGrupo.SelectedValue == string.Empty ? "Todos" : cmbGrupo.SelectedValue);
                ALValorParametrosInternos.Add(cmbGrupo.SelectedValue);
                ...
                // Se agrega opcion para seleccionar tipo de Cliente entre Nacional, Local o todos
                //RBM 2023
                //Inicio
                ALValorParametrosInternos.Add(rblTipoCliente.SelectedValue);
                //Fin
        ...
        protected void cmbCategoria_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                CargarGrupos(cmbCategoria.SelectedIndex);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void CargarCategorias()
        {
            try
            {
                ...
                CN_CatSistemasPropietarios CN = new CN_CatSistemasPropietarios();
                CN.LlenaCombo(Sesion.Emp_Cnx, "spCatCategoriasCombo", ref Lista);
                if (Lista.Count > 0)
                {
        ...
        private void CargarGrupos(int Id_Cat)
        {
            try
            {
                Sesion Sesion = new CapaEntidad.Sesion();
                ...
                System.Collections.Generic.List<Comun> Lista = new System.Collections.Generic.List<Comun>();
                CN_CatSistemasPropietarios CN = new CN_CatSistemasPropietarios();
                CN.LlenaCombo(Id_Cat, Sesion.Emp_Cnx, "spCatGrupoCombo", ref Lista);
                if (Lista.Count > 0)
                {
                    cmbGrupo.DataSource = Lista;
                    cmbGrupo.DataValueField = "Id";
                    cmbGrupo.DataTextField = "Descripcion";
                    cmbGrupo.DataBind();
                }
            }
            catch (Exception ex)
        ...
        protected void cmbCdi_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
