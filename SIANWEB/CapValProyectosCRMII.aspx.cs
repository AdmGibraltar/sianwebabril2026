...
namespace SIANWEB
{
    public partial class CapValProyectosCRMII : BaseServerPage
    {
        #region Variables
        ...
        // 950 = 0 .- NO pemite captura el precio MENOR a Precio de Venta 
        // 950 = 1 .- SI pemite captura el precio MENOR a Precio de Venta 
        public int GLOBAL_CRMValuacion_PrecioVentaMayor_AAA = 0;
        // 952 = 0 .- NO pemite captura el precio MENOR a MinimoRik (precio Objetivo) 
        // 952 = 1 .- SI pemite captura el precio MENOR a MinimoRik (precio Objetivo)
        public int GLOBAL_CRM_ValidaPrecioMinimoRik = 0;

        private int TipoValuacion = 0; // Tipo 0 Individual, 1 Global 
        private int Id_Vap = 0;
        private int Id_Cd = 0;
        private int Id_Ter_ = 0;
        private int Id_Cte_ = 0;
        private int Id_Op_ = 0;
        private int Id_Emp = 0;
        private int SoloLectura = 0;

        // Parametros Gerente
        private int Parametro_IdTU = 0;
        private int Parametro_IdRik = 0;
        private int CRM_Gerente_Id = 0;
        private int CRM_Gerente_Rik = 0;
        private int CRM_Usuario_Id = 0;
        private int CRM_Usuario_Rik = 0;
                ...
                else
                {
                    ...
                    { //obtener valores desde la URL
                      // 23AGO-2021 RFH
                        ...
                        CargarTerritoriosCliente();

                        /*
                        if (Id_Vap > 0)
                        {
                            SeleccionarTerritorioElegido(CapValProyecto);
                        }
                        */
                        ...
                        CargarProyectos();
                        /*
                        AgregarProductosDeProyecto_SP(Id_Op_, Id_Cte_);
                        ...
                        */
                        ...
                        // 0 .- Edicion , 1.- Solo Lectura
                        if (SoloLectura == 1)
                        {
                            // deshabilita grids.
                            this.rgDetalle.Enabled = false;
                            this.rgProyectos.Enabled = false;

                            RadToolBar1.FindItemByValue("save").Enabled = false;
                            RadToolBar1.FindItemByValue("save").Text = "Solo lectura";
                        }
                        else
                        {
                            // habilita grids.
                            this.rgDetalle.Enabled = true;
                            this.rgProyectos.Enabled = true;

                            RadToolBar1.FindItemByValue("save").Enabled = true;
                            RadToolBar1.FindItemByValue("save").Text = "Guardar Valuación";
                        }
                        Parametros_EstableceEstatus(SoloLectura);
        ...
        private void CargarConfiguracion()
        {
            ...
            try
            {
                ...
                SysC = CN.spSysConfiguracionById(Sesion.Id_Emp, Sesion.Id_Cd, 950, Sesion.Emp_Cnx);
                int iTmp = 0;
                int.TryParse(SysC.Conf_Valor, out iTmp);
                GLOBAL_CRMValuacion_PrecioVentaMayor_AAA = iTmp;
            }
            catch (Exception ex)
            {
                GLOBAL_CRMValuacion_PrecioVentaMayor_AAA = 0;
            }

            // 952 = 0 .- NO pemite captura el precio MENOR a Precio Minimo Rik ( Precio Objetivo) 
            // 952 = 1 .- SI pemite captura el precio MENOR a Precio Minimo Rik ( Precio Objetivo) 

            try
            {
                CN_SysConfigruacion CN = new CN_SysConfigruacion();
                SysC = CN.spSysConfiguracionById(Sesion.Id_Emp, Sesion.Id_Cd, 952, Sesion.Emp_Cnx);
                int iTmp = 0;
                int.TryParse(SysC.Conf_Valor, out iTmp);
                GLOBAL_CRM_ValidaPrecioMinimoRik = iTmp;
            }
            catch (Exception ex)
            {
                GLOBAL_CRM_ValidaPrecioMinimoRik = 0;
            }
        ...
        // 
        private void Parametros_EstableceEstatus(int SoloLectura)
        {
            // 0 .- Edicion , 1.- Solo Lectura
            if (SoloLectura == 1)
            {
                // deshabiltia
                txtCuentasPorCobrar.Enabled = false;
                txtFleteLocales.Enabled = false;
                txtInventario.Enabled = false;
                txtGastosServirCliente.Enabled = false;
                txtInversionactivosfijos.Enabled = false;
                txtIsr.Enabled = false;
                txtFinanciamientoproveedores.Enabled = false;
                txtCetes.Enabled = false;
                txtCostodecapital.Enabled = false;
            ...
            {
                //habilita
                txtCuentasPorCobrar.Enabled = true;
                txtFleteLocales.Enabled = true;
                txtInventario.Enabled = true;
                txtGastosServirCliente.Enabled = true;
                txtInversionactivosfijos.Enabled = true;
                txtIsr.Enabled = true;
                txtFinanciamientoproveedores.Enabled = true;
                txtCetes.Enabled = true;
                txtCostodecapital.Enabled = true;
        ...
        private void Parametros()
        {
            try
            {
                ...
                TipoValuacion = Convert.ToInt32(Page.Request.QueryString["Tipo"]); // 0 Val Individual , 1 Val Global
                Id_Vap = Convert.ToInt32(Page.Request.QueryString["Id_Vap"]);
                Id_Ter_ = Convert.ToInt32(Page.Request.QueryString["Id_Ter"]);
                Id_Cte_ = Convert.ToInt32(Page.Request.QueryString["Id_Cte"]);
                Id_Op_ = Convert.ToInt32(Page.Request.QueryString["Id_Op"]);
                TipoValuacion = Convert.ToInt32(Page.Request.QueryString["Tipo"]);

                SoloLectura = Convert.ToInt32(Page.Request.QueryString["SoloLectura"].ToString());

                Parametro_IdTU = Convert.ToInt32(Page.Request.QueryString["Parametro_IdTU"]);
                Parametro_IdRik = Convert.ToInt32(Page.Request.QueryString["Parametro_IdRik"]);
                CRM_Gerente_Id = Convert.ToInt32(Page.Request.QueryString["CRM_Gerente_Id"]);
                CRM_Gerente_Rik = Convert.ToInt32(Page.Request.QueryString["CRM_Gerente_Rik"]);
                CRM_Usuario_Id = Convert.ToInt32(Page.Request.QueryString["CRM_Usuario_Id"]);
                CRM_Usuario_Rik = Convert.ToInt32(Page.Request.QueryString["CRM_Usuario_Rik"]);

                hfId_Cte.Value = Id_Cte_.ToString();
                hfId_Op.Value = Id_Op_.ToString();
                hfTipoValuacion.Value = TipoValuacion.ToString();

                if (Page.Request.QueryString["Id_Cte"] != null)
                {
                    try
                    {
                        Id_Cte = Convert.ToInt32(Page.Request.QueryString["Id_Cte"]);
                        txtCliente.Value = Id_Cte;
                        txtCliente_TextChanged(txtCliente, new EventArgs());
                    }
                    catch (FormatException ex)
                    {
                        Alerta("El identificador del cliente tiene errores de formato");
                    }
                }
        ...
        protected void CargarProyectos()
        {
            ...
            //promocion.Territorio = Id_Ter.Value;
            promocion.Territorio = Id_Ter_;
            ...
            //promocion.Cliente = int.Parse(txtCliente.Text);            

            if (Id_Cte != null)
            {
                promocion.Cliente = (Int32)Id_Cte;
            }
            else
            {
                promocion.Cliente = int.Parse(txtCliente.Text);
            ...
            // Si llega el parametro rik se utiliza este
            if (CRM_Usuario_Rik > 0)
            {
                promocion.Id_Rik = CRM_Usuario_Rik.ToString();
            }
            else
            {
            ...
            if (Id_Vap != 0)
            {
                CNCrmPromocion.ConsultaCatPromocion(Sesion, promocion, ref List);
            }
            else
            {
                //
                //cls.ProyectosDisponiblesParaValuacion(Sesion, promocion, ref List, BusinessTransaction);
                // 14 sep 2018 RFH
                
                //CNCrmPromocion.ProyectosDisponiblesParaValuacion_2(Sesion, promocion, ref List, BusinessTransaction);
                
                if (Tipo_Valuacion == 1)
                {
                    CN_CrmPromocion CN_CrmProm = new CN_CrmPromocion();
                    List = CN_CrmProm.ConsultaCatPromocion_Ver2(Sesion.Id_Emp, Sesion.Id_Cd, Id_Cte_, Id_Op_, Sesion.Emp_Cnx); 
                }
                if (Tipo_Valuacion == 2)
                {
                    CN_CrmPromocion CN_CrmProm = new CN_CrmPromocion();
                    List = CN_CrmProm.ConsultaCatPromocion_Ver2(Sesion.Id_Emp, Sesion.Id_Cd, Id_Cte_, 0, Sesion.Emp_Cnx);
                }
                                
                /*
                CD_CrmPromocion CD_CrmProm = new CD_CrmPromocion();
                List = CD_CrmProm.ConsultaCatPromocion_Ver2(Sesion.Id_Emp, Sesion.Id_Cd, promocion.Id_Cte, Sesion.Emp_Cnx);
        ...
        */
            ...
            {
                CN_CrmPromocion CN_CrmProm = new CN_CrmPromocion();
                List = CN_CrmProm.ConsultaCatPromocion_Ver2(Sesion.Id_Emp, Sesion.Id_Cd, Id_Cte_, Id_Op_, Sesion.Id_Rik, Sesion.Emp_Cnx, Sesion.VersionTerritorio);
            }
            if (TipoValuacion == 1)
            {
                CN_CrmPromocion CN_CrmProm = new CN_CrmPromocion();
                List = CN_CrmProm.ConsultaCatPromocion_Ver2(Sesion.Id_Emp, Sesion.Id_Cd, Id_Cte_, 0, Sesion.Id_Rik, Sesion.Emp_Cnx, Sesion.VersionTerritorio);
            }

            // Se consulta el primer elemento de la lista de oportunidades para concocer el Estatus de la Valuacion o CrmOportunidades
            // 28 Sep 2018 RFH

            //if (List[0].Estatus == 4) // ACyS
            //{
            //    SoloLectura = 1;
            //}

            //if (List[0].Estatus == 5) // Cancelada
            //{
            //    SoloLectura = 1;    
            //}

            rgProyectos.DataSource = List;
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        /// <summary>
        /// Establece el valor del territorio elegido previamente en la valuación en la vista del listado de territorios.
        /// </summary>
        /// <param name="cvp">Entidad de datos capValProyecto</param>
        private void SeleccionarTerritorioElegido(CapaModelo.CapValProyecto cvp)
        {
            if (cvp != null)
            {
                if (cvp.Id_Ter != null)
                {
                    ddlTerritorio.SelectedValue = cvp.Id_Ter.Value.ToString();
                    Id_Ter = cvp.Id_Ter;
                }
                else
                {
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case "panel":
                        ...
                        break;
                    //  JFCV 24ene23  que grabe cuando regrese de cerrar pantalla de ventana_autorización
                    case "grabarVal":
                        Guardar_Ver2();
                        break;
            ...
            try
            {
                ...
                switch (btn.CommandName)
                {
                    case "edit":
                        ...
                        break;
                    case "excel":
                        System.IO.StreamWriter sw = new System.IO.StreamWriter("F:/APLICACIONES_IIS/sianwebmtycrm3rentabilidad/Vp.xls");
                        ...
                        sw.Close();
                        Response.Redirect("http://189.206.126.67/sianwebmtycrm3vprentabilidad/Vp.xls");
                        break;
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                switch (btn.CommandName)
                {
                    ...
                    case "save":
                        ...
                        //JFCV 16 Jun 2022 Validar que tengan precio mayor a Precio minimo Rik 
                        //agregue función que primero valida que los precios sean correctos 
                        if (Guardar_Valida() == true)
                        {
                            //this.Guardar_Ver2();
                            string accion = "grabarVal";
                            RAM1.ResponseScripts.Add(string.Concat(@"GrabaAlerta('", accion, "')"));
        ...
        // CLICK en CHECKBOX Listado de Proyectos 
        // Para agregar o remover un proyecto
        void chk_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (chk.Checked)
            {
                ...
                AgregarProductosDeProyecto_SP(idOp, idCte);
                rgDetalle.Rebind();
            }
            else
            {
                //RemoverProductosDeProyecto(idOp, idCte);
                RemoverProductosDeProyecto_ver2(idOp, idCte);
                rgDetalle.Rebind();
            }
        ...
        protected void AgregarProductosDeProyecto(int idOp, int idCte)
        {
            ...
            var detalle = (from p in productos
                           select new ValuacionProyectoDetalle()
                           {
                               ...
                               Vap_Costo = Math.Round(this.PartidasCalcularPrecioAAA(Sesion.Id_Emp, Sesion.Id_Cd_Ver, Convert.ToInt32(txtCliente.Value), p.Id_Prd, Sesion.Emp_Cnx), 2),
                               Vap_Precio = cnProductoPrecios.ConsultarPrecioAAA(Sesion, p.Id_Prd),
                               Vap_PrecioEspecial = cnProductoPrecios.ConsultarPrecioLista(Sesion, p.Id_Prd),
                               Id_Ter = Convert.ToInt32(Id_Ter_),
                           }).ToList();
        ...
        // 24 Sep 2018 RFH
        // Agrega los productos al detalle segun oportunidad
        protected void AgregarProductosDeProyecto_SP(int idOp, int idCte)
        {
            CN_CrmOportunidadesProductos cnOportunidadesProductos = new CN_CrmOportunidadesProductos();
            CN_ProductoPrecios cnProductoPrecios = new CN_ProductoPrecios();
            ...
            List<ValuacionProyectoDetalle> lstDetalle = new List<ValuacionProyectoDetalle>();

            CN_ValuacionProyectoDetalle CN_VPD = new CN_ValuacionProyectoDetalle();
            lstDetalle = CN_VPD.Detalle(Sesion.Id_Emp, Sesion.Id_Cd, idOp, Sesion.Emp_Cnx);

            //
            // PRIMERO QUITAR LOS QUE ESTAN 
            //
            ...
            {
                ValuacionProyectoDetalle VPD_A = lstDetalle[i];
                for (int y = 0; y < lista.Count; y++)
                {
                    ValuacionProyectoDetalle VPD_B = lista[y];

                    if (VPD_A.Id_Prd == VPD_B.Id_Prd)
                    {
                        lista.RemoveAt(y);
                        break;
                    }
            ...
            foreach (ValuacionProyectoDetalle p in lstDetalle)
            {
                p.Id_Op = idOp;
                p.Id_Vap = Convert.ToInt32(txtFolio.Text);
                p.Vap_Costo = Math.Round(this.PartidasCalcularPrecioAAA(Sesion.Id_Emp, Sesion.Id_Cd_Ver, Convert.ToInt32(txtCliente.Value), p.Id_Prd, Sesion.Emp_Cnx), 2);
                // Si no hay precio  
                if (p.Vap_Precio <= 0)
                {
                    p.Vap_Precio = cnProductoPrecios.ConsultarPrecioAAA(Sesion, p.Id_Prd);
                }
                p.Vap_PrecioEspecial = cnProductoPrecios.ConsultarPrecioLista(Sesion, p.Id_Prd);
            }

            ListaProductosValProyecto.AddRange(lstDetalle);
            //proyectoProductos.Productos.AddRange(detalle);
        }
        ...
        protected void RemoverProductosDeProyecto(int idOp, int idCte)
        {
            ...
            _proyectosSeleccionados.RemoveAll(pp => pp.Id_Op == idOp);

        }

        // VER 2 
        // 10JUL-2019 
        protected void RemoverProductosDeProyecto_ver2(int Id_Op, int idCte)
        {
            /*
            CN_CrmOportunidadesProductos CN_OP = new CN_CrmOportunidadesProductos();
            CN_ProductoPrecios cnProductoPrecios = new CN_ProductoPrecios();
            var productos = CN_OP.ObtenerProductosPorOportunidad_ver2
                (Sesion.Id_Emp, Sesion.Id_Cd, idOp, idCte,Sesion.Emp_Cnx);
            var detalle = (from p in productos
                           select p.Id_Prd).ToList();
            ...
            */
            ...
            /*
            foreach (ValuacionProyectoDetalle VPD in ListaProductosValProyecto)
            {
                if (VPD.Id_Op == Id_Op)
                {
                    ListaProductosValProyecto.Remove(VPD);
                }
            }
            */
            //ListaProductosValProyecto.RemoveAll(i => i.Id_Op == Id_Op);
            ...
            CN_ValuacionProyectoDetalle CN_VPD = new CN_ValuacionProyectoDetalle();
            lstDetalle = CN_VPD.Detalle(Sesion.Id_Emp, Sesion.Id_Cd, Id_Op, Sesion.Emp_Cnx);
        ...
        protected void rgDetalle_ItemDataBound(object sender, GridItemEventArgs e)
        {
            try
            {
                if ((e.Item is GridEditableItem) && (e.Item.IsInEditMode))
                {
                    ...
                    //JFCV 
                    string txtId_Ter = ((Label)editItem.FindControl("lblId_Ter")).ClientID.ToString();
                    ...
                    if (updatebtn != null)
                    {
                        ...
                        long Id_Prd = Convert.ToInt64(editItem.OwnerTableView.DataKeyValues[editItem.ItemIndex]["Id_Prd"]);
                        foreach (ValuacionProyectoDetalle vpd in this.ListaProductosValProyecto)
                        {
                            if (vpd.Id_Prd == Id_Prd)
                            {
                                ...
                                ((RadNumericTextBox)editItem.FindControl("txtVap_Costo")).Enabled = false;
                                if (vpd.Vap_Tipo == 2)
                                {
        ...
        protected void rgDetalle_InsertCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                valuacionProyectoDetalle.Id_Prd = Convert.ToInt64(comboProducto.Value);
                valuacionProyectoDetalle.Producto = new Producto();
                valuacionProyectoDetalle.Producto.Id_Prd = Convert.ToInt64(comboProducto.Value);
                ...
                valuacionProyectoDetalle.Id_Ter = Convert.ToInt32((insertedItem.FindControl("txtId_Ter") as RadNumericTextBox).Text);
                //agregar producto de nota de cargo a la lista
                List<ValuacionProyectoDetalle> lista = this.ListaProductosValProyecto;
        ...
        protected void rgDetalle_UpdateCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                var dataProyecto = ListaProductosValProyecto.Where(x => x.Id_Prd == Convert.ToInt64(comboProducto.Value)).FirstOrDefault();
                var idOp = 0;
                if (dataProyecto != null)
                {
                    idOp = dataProyecto.Id_Op;
                }
                valuacionProyectoDetalle.Id_Op = idOp;
                ...
                valuacionProyectoDetalle.Vap_Precio = Convert.ToDouble((insertedItem.FindControl("txtVap_Precio") as RadNumericTextBox).Text);
                valuacionProyectoDetalle.Vap_PrecioEspecial = Convert.ToDouble((insertedItem.FindControl("lblVap_ListaEdit") as Label).Text);//lblVap_PrecioEspecialEdit") as Label).Text);
                valuacionProyectoDetalle.Vap_PrecioObjetivo = Convert.ToDouble((insertedItem.FindControl("lblVap_PrecioObjetivoEdit") as Label).Text);
                ...
                //JFCV 
                valuacionProyectoDetalle.Id_Ter = Convert.ToInt32((insertedItem.FindControl("lblId_Ter") as Label).Text);
        ...
        protected void rgDetalle_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                long id_Prd = Convert.ToInt64(item.OwnerTableView.DataKeyValues[item.ItemIndex]["Id_Prd"]);
        ...
        {
            if (GLOBAL_CRM_ValidaPrecioMinimoRik == 0)
            {
                //ocultar columna de Precio Objetivo
                rgDetalle.MasterTableView.GetColumn("Vap_PrecioObjetivo").Visible = false;
            }
            else
            {
                //Mostrar columna de Precio Objetivo
                rgDetalle.MasterTableView.GetColumn("Vap_PrecioObjetivo").Visible = true;
            }
            //rgDetalle.Rebind();
        }
        ...
        protected void cmbTipo_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                if (rgDetalle.EditItems.Count == 0)
                {
                    ...
                    (rcb.Parent.Parent.FindControl("txtId_Ter") as RadNumericTextBox).Text = "";
        ...
        protected void cmbProducto_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Value != string.Empty && e.Value != "-1")
                {
                    ...
                    clsProducto.ConsultaProducto(ref producto, sesion.Emp_Cnx, sesion.Id_Emp, id_Cd_Prod, sesion.Id_Cd_Ver, Convert.ToInt64(e.Value), 1);
                }
                ...
                //new CN_ProductoPrecios().ConsultarPrecios(ref precioAAA, ref precioLista, sesion, Convert.ToInt32(txtCliente.Text), Convert.ToInt64(e.Value));
                //12enero que no traiga el precio especial, si no el precio AAA y precio de lista normal
                new CN_ProductoPrecios().ConsultarPrecios(ref precioAAA, ref precioLista, sesion, Convert.ToInt32(txtCliente.Text), Convert.ToInt64(e.Value));
                precioLista = new CN_ProductoPrecios().ConsultarPrecioLista(Sesion, Convert.ToInt64(e.Value));
        ...
        protected void cmbProductoDet_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    new CN_CatProducto().ConsultaProducto(ref prd, session.Emp_Cnx, session.Id_Emp, session.Id_Cd, Convert.ToInt64(txtProducto.Value.HasValue ? (sender as RadNumericTextBox).Value : -1), 1);
                }
                catch (Exception ex)
                {
                ...
                new CN_ProductoPrecios().ConsultaListaProductoPrecioAAA(ref precioAAA, session, Convert.ToInt64(txtProducto.Value), ref Id_Pre);
                new CN_ProductoPrecios().ConsultaListaProductoPrecioPUBLICO(ref precioLista, session, Convert.ToInt64(txtProducto.Value));

                txtVap_CostoEdit.Text = Math.Round(this.PartidasCalcularPrecioAAA(session.Id_Emp, session.Id_Cd_Ver, Convert.ToInt32(txtCliente.Value), Convert.ToInt64(txtProducto.Value), session.Emp_Cnx), 2).ToString();
        ...
        // 
        protected void ddlTerritorio_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            //1. Se interpreta el valor entero del valor elegido del listado de territorios
            //2. Se recarga el listado de proyectos, debido a que estos dependen del territorio elegido.

            //1
            DropDownList listadoTerritorio = sender as DropDownList;
            string idTerStr = listadoTerritorio.SelectedValue;
            try
            {
                Id_Ter = int.Parse(idTerStr);
            }
            catch (Exception ex)
            {
                //Esto no debería de pasar. Pasar a bitácora. Las posibles causas son: cambio de formato del valor del identificador de territorio.
            }
        ...
        /// <summary>
        /// Carga el conjunto de territorios asociados al RIK en la vista del listado de Territorios.
        /// </summary>
        private void CargarTerritoriosCliente()
        {
            CN_CatTerritorios cnCatTerritorios = new CN_CatTerritorios();

            //IEnumerable<CapaEntidad.CatTerritorio> territoriosRIK = new IEnumerable<CapaEntidad.CatTerritorio>();

            //var territoriosRIK = cnCatTerritorios.ObtenerTerritoriosPorCliente(Sesion, Id_Cte.Value);
            /*
            private int Parametro_IdTU = 0;
            private int Parametro_IdRik = 0;
            private int CRM_Gerente_Id = 0;
            ...
            */
            ...
            lst = cnCatTerritorios.ObtenerTerritorios_PorClienteYRik(Sesion.Id_Emp, Sesion.Id_Cd, Id_Cte_, CRM_Usuario_Rik, Sesion);

            var territoriosRIKLista = lst;
            //var territoriosRIKLista = IEnumerable<CatTerritorio>;
            //var territoriosRIKLista=territoriosRIK.ToList();

            territoriosRIKLista.Add(new CapaEntidad.eTerritorio() { Id_Ter = 0, Ter_Nombre = "--Seleccione--" });

            ddlTerritorio.DataSource = territoriosRIKLista;
            ddlTerritorio.DataTextField = "Ter_Nombre";
            ddlTerritorio.DataValueField = "Id_Ter";
            ddlTerritorio.DataBind();
            ddlTerritorio.Enabled = false;

            ddlTerritorio.SelectedValue = Id_Ter_.ToString();

            hfId_Ter.Value = Id_Ter_.ToString();
        ...
        private void Inicializar(int Id_Emp, int Id_Cd, int Id_Vap)
        {
            ...
            else //Nueva
            {
                ...
                if (Id_Vap == 0)
                {
                    ...
                    //
                    // RFH 6 Mar 2019
                    // No se puede asignar un folio al incio ya que al se utilizado por mutiples usuarios 
                    // entegara el mismo folio y al guardar provocaria error. Se inicia en cero.
                    // 
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        protected void ListaProductosValProyecto_EliminarProducto(long id_Prd)
        {
            List<ValuacionProyectoDetalle> lista = this.ListaProductosValProyecto;
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        private double PartidasCalcularPrecioLista(int Id_Emp, int Id_Cd, int Id_Cte, long Id_Prd, string Conexion)
        {
            ...
            //////JFCV 12 enero este sp ya no esta vigente la informacion qu etrae es del 2012
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        // Calculo de precio AAA
        // no utiliza EF 
        private double PartidasCalcularPrecioAAA(int Id_Emp, int Id_Cd, int Id_Cte, long Id_Prd, string Conexion)
        {
            double precioProductoAceptado = 0;
            //obtener precio especial del producto 
            //para el cliente actual de la factura
            ...
            VentanaPrecioEspecialPro precioEspecialPro = null;
            // SP ok 23JUL2019 RFH
            new CN_PrecioEspecial().PrecioEspecialProductoCliente_Consulta(
                ref precioEspecialPro, Conexion, Id_Emp, Id_Cd, Id_Cte, Id_Prd);
            ...
            else
            {
                ...
                /*
                 *  * MAY11-2020 RFH, RafaGarcia  
                 * Se comenta este segmento ya que es una funciona obsoleta
                 * 
                ClienteProd clienteProd = new ClienteProd();
                clienteProd.Id_Emp = Id_Emp;
                clienteProd.Id_Cd = Id_Cd;
                ...
                precioProductoAceptado = precioAAA;
        ...
        private void LLenarFormValProyecto(int Id_Emp, int Id_Cd, int Id_Vap)
        {
            ...
            new CN_CapValuacionProyecto().ConsultarValuacionProyecto2(ref valuacionProyecto, sesion.Emp_Cnx);

            if (valuacionProyecto.Estatus == 5)
            {
                // Cancelado.
                SoloLectura = 1;
            }
            if (valuacionProyecto.Vap_Estatus == "A" && valuacionProyecto.Vap_Estatus2 == 4)
            {
                // Con ACyS
                SoloLectura = 1;
            }
            if (valuacionProyecto.Vap_Estatus == "C" && valuacionProyecto.Vap_Estatus2 == 4)
            {
                // Con ACyS
                SoloLectura = 1;
            }
            if (valuacionProyecto.Vap_Estatus == "C" && valuacionProyecto.Vap_Estatus2 == 2)
            {
                // Autorizando
                SoloLectura = 1;
            }
            if (valuacionProyecto.Vap_Estatus == "A" && valuacionProyecto.Vap_Estatus2 == 0)
            {
                // Capturado
                SoloLectura = 0;
            }
            txtFolio.Text = valuacionProyecto.Id_Vap.ToString();
            txtMotivoAutorizacion.Text = valuacionProyecto.MotivoParaAutorizacion;
        ...
        //
        // RFH 27 04 2018 
        // Edjecuta el SP spCRM_ObtenerProyectosPorRik_CalcularResultado para 
        // pasar los resultados al string que contiene la valuacion 
        //
        private string GeneraReporteVP_PorSP()
        {
            double CuentasPorCobrar_ = 0;
            double Inventario_ = 0;
            double InversionActivosFijos_ = 0;
            double FinanciamientoProveedores_ = 0;
            double TasaIncrementoCostoCapital_ = 0;
            double VigenciaACYS_ = 0;
            double FleteCD_ = 0;
            double GastosServirCliente_ = 0;
            double ISRyPTU_ = 0;
            double Cetes_ = 0;
            double ManoObraProyectos_ = 0;

            double.TryParse(txtCuentasPorCobrar.Text, out CuentasPorCobrar_);
            double.TryParse(txtInventario.Text, out Inventario_);
            double.TryParse(txtInversionactivosfijos.Text, out InversionActivosFijos_);
            double.TryParse(txtFinanciamientoproveedores.Text, out FinanciamientoProveedores_);
            double.TryParse(txtCostodecapital.Text, out TasaIncrementoCostoCapital_);
            double.TryParse(txtVigencia.Text, out VigenciaACYS_);
            double.TryParse(txtFleteLocales.Text, out FleteCD_);
            double.TryParse(txtGastosServirCliente.Text, out GastosServirCliente_);
            double.TryParse(txtIsr.Text, out ISRyPTU_);
            double.TryParse(txtCetes.Text, out Cetes_);
            double.TryParse(txtManoObra.Text, out ManoObraProyectos_);

            Sesion Sesion = new Sesion();
            Sesion = (Sesion)Session["Sesion" + Session.SessionID];

            eValuacionProyecto Resultado = new eValuacionProyecto();
            int Verificador = 0;

            CN_CrmValuacionOportunidades cnCRMVO = new CN_CrmValuacionOportunidades();
            Resultado = cnCRMVO.ObtenerPorValuacion_Detallado(0, Convert.ToInt32(txtFolio.Text), ref Verificador, Sesion,
                CuentasPorCobrar_, Inventario_, InversionActivosFijos_, FinanciamientoProveedores_, TasaIncrementoCostoCapital_,
                VigenciaACYS_, FleteCD_, GastosServirCliente_, ISRyPTU_, Cetes_,
                ManoObraProyectos_
            );

            String tblF = "";

            tblF += "<table width=\"250px\" border=\"1\" spacing=\"0\">";
            tblF += "<tr>";
            tblF += "<td align=\"center\" style=\"font-family: Verdana; font-size: 10pt;background-color: #A9BCF5\"><b>Año</b></td>";
            tblF += "<td align=\"center\" style=\"font-family: Verdana; font-size: 10pt;background-color: #A9BCF5\"><b>Flujo Anuales</b></td>";
            tblF += "<td align=\"center\" style=\"font-family: Verdana; font-size: 10pt;background-color: #A9BCF5\"><b>VP Flujos</b></td>";
            tblF += "</tr>";


            int F = 0;

            foreach (eResultadoValuacionFlujo e in Resultado.ValuacionFlujo)
            {
                ...
                if (e.Anio == 0 && F == 1)
                {
                    // Final
                    tblF += "<tr>";
                    tblF += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\"></td>";
                    tblF += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\"></td>";
                    tblF += "<td align=\"right\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.VPFlujos + "</td>";
                    tblF += "</tr>";
                    F = 1;
                }
                ...
                if (e.Anio == 0 && F == 0)
                {
                    // Inicio
                    tblF += "<tr>";
                    tblF += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.Anio + "</td>";
                    tblF += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.FlujoAnual + "</td>";
                    tblF += "<td align=\"right\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.VPFlujos + "</td>";
                    tblF += "</tr>";
                    F = 1;
                }
            ...
            tbl1 += "<tr>";
            tbl1 += "<td colspan=\"2\" align=\"center\" style=\"font-family: Verdana; font-size: 12pt;background-color: #A9BCF5\"><b>Determinación de la inversión en activos netos de la operación</b></td>";
            tbl1 += "</tr>";

            String tbl2 = "";
            tbl2 += "<table width=\"100%\">";
            tbl2 += "<tr>";
            tbl2 += "<td colspan=\"3\" align=\"center\" style=\"font-family: Verdana; font-size: 12pt;background-color: #A9BCF5\"><b>Cálculo del Uafir anual después de impuestos</b></td>";
            tbl2 += "</tr>";

            int rc = 0;
            foreach (eResultadosValuacionDetallado e in Resultado.ValuacionProyecto)
            {
                string bb = "";
                // las lineas donde se coloca la linea inferiro en la celda
                if (e.Folio == 4 || e.Folio == 6 || e.Folio == 14 || e.Folio == 18 || e.Folio == 24 || e.Folio == 26)
                {
                ...
                if (rc < 8)
                {
                    ...
                    {
                        tbl1 += "<tr>";
                        tbl1 += "<td align=\"left\" style=\"background-color: #E6E6E6; font-family: Verdana; font-size: 10pt;\"><b>" + e.Titulo + "</b></td>";
                        tbl1 += "<td align=\"right\" style=\"" + bb + "background-color: #E6E6E6; font-family: Verdana; font-size: 10pt;\"><b>" + e.Monto + "</b></td>";
                        tbl1 += "</tr>";
                    }
                    else
                    {
                        tbl1 += "<tr>";
                        tbl1 += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.Titulo + "</td>";
                        tbl1 += "<td align=\"right\" style=\"" + bb + "font-family: Verdana; font-size: 10pt;\">" + e.Monto + "</td>";
                        tbl1 += "</tr>";
                    }
                }
                else
                {
                    // Tabla 2
                    float fMonto = 0;
                    float.TryParse(e.Factor, out fMonto);
                    ...
                    {
                        tbl2 += "<tr>";
                        tbl2 += "<td align=\"left\" style=\"background-color: #E6E6E6; font-family: Verdana; font-size: 10pt;\"><b>" + e.Titulo + "</b></td>";
                        tbl2 += "<td align=\"right\" style=\"" + bb + "background-color: #E6E6E6; font-family: Verdana; font-size: 10pt;\"><b>" + e.Monto + "</b></td>";
                        tbl2 += "<td align=\"right\" style=\"font-family: Verdana; font-size: 10pt;\"><b>" + Monto + "</b></td>";
                        tbl2 += "</tr>";
                    }
                    else
                    {
                        tbl2 += "<tr>";
                        tbl2 += "<td align=\"left\" style=\"font-family: Verdana; font-size: 10pt;\">" + e.Titulo + "</td>";
                        tbl2 += "<td align=\"right\" style=\"" + bb + "font-family: Verdana; font-size: 10pt;\">" + e.Monto + "</td>";
                        tbl2 += "<td align=\"right\" style=\"font-family: Verdana; font-size: 10pt;\">" + Monto + "</td>";
                        tbl2 += "</tr>";
                    }
                }
            }
            ...
            tbl1 += "<tr>";
            tbl1 += "<td align=\"center\" style=\"font-family: Verdana; font-size: 10pt;\">" + tblF + "</td>";
            tbl1 += "</tr>";

            tbl1 += "</table>";
            tbl2 += "</table>";

            string CadenaHtml = "<table width=\"100%\">";
            CadenaHtml += "<tr>";
            CadenaHtml += "<td width=\"50%\" valign=\"top\">" + tbl1 + "</td>";
            CadenaHtml += "<td width=\"50%\" valign=\"top\">" + tbl2 + "</td>";
            CadenaHtml += "</tr>";
            CadenaHtml += "</table>";
        ...
        private string GeneraReporteVP()
        {
            ...
            CadenaHtml = CadenaHtml + "            </tr>";
            CadenaHtml = CadenaHtml + "</table>" + construirContenidoMotivoAutorizacion(txtMotivoAutorizacion.Text) + "</td>";
            CadenaHtml = CadenaHtml + "                <td>&nbsp;</td>";
            CadenaHtml = CadenaHtml + "                <td>&nbsp;</td>";
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        // genera el reporte para poder enviarlos junto con la valuación Alerta validación alerta de precios 
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        private string GeneraReporteVPAlerta()
        {
            ...
            er.VentaNetaMon = VentaNeta;
            er.UtilidadMon = (VentaNeta - CostoMaterial);
            er.UtilidadPorc = ((VentaNeta - CostoMaterial) / VentaNeta) * 100;
            er.UafirMensualMon = UafirMensual;
            er.UafirMensualPorc = ((UafirMensual / VentaNeta) * 100);
            er.UafirAnualMon = UafirMensual * 12;
            er.UafirDespuesImpMon = UafirDespuesImpuestos;
            er.UtilidadRemanenteMon = (UafirDespuesImpuestos - CostoCapital);


            Session["CalculoUafir" + Session.SessionID] = er;
        ...
        private void Guardar()
        {
            ...
            try
            {
                ...
                ValuacionParametrosActual VPA = new ValuacionParametrosActual();
                VPA.Id_Emp = sesion.Id_Emp;
                VPA.Id_Cd = sesion.Id_Cd_Ver;

                //VPA.Id_Vap = Convert.ToInt32(txtFolio.Text);                
                // 16 Mar 2019 RFH
                int iValor = 0;
                Int32.TryParse(txtFolio.Text, out iValor);
                if (iValor <= 0)
                {
                    txtFolio.Text = this.Valor;
                    VPA.Id_Vap = Int32.Parse(txtFolio.Text);
                }
                else
                {
                    VPA.Id_Vap = Int32.Parse(txtFolio.Text);
                ...
                VPA.txtCuentasPorCobrar = Convert.ToDouble(txtCuentasPorCobrar.Text);
                VPA.txtInventario = Convert.ToDouble(txtInventario.Text);
                VPA.txtGastosServirCliente = Convert.ToDouble(txtGastosServirCliente.Text);
                VPA.txtVigencia = Convert.ToDouble(txtVigencia.Text);
                VPA.txtFleteLocales = Convert.ToDouble(txtFleteLocales.Text);
                VPA.txtIsr = Convert.ToDouble(txtIsr.Text);
                VPA.txtCetes = Convert.ToDouble(txtCetes.Text);
                VPA.txtFinanciamientoproveedores = Convert.ToDouble(txtFinanciamientoproveedores.Text);
                VPA.txtInversionactivosfijos = Convert.ToDouble(txtInversionactivosfijos.Text);
                VPA.txtCostodecapital = Convert.ToDouble(txtCostodecapital.Text);
                VPA.txtManoObra = Convert.ToDouble(txtManoObra.Text);
                ...
                if (CRM_Usuario_Rik > 0)
                {
                    valuacionProyecto.Id_Rik = CRM_Usuario_Rik;
                }
                else
                {
                ...
                valuacionProyecto.MotivoParaAutorizacion = txtMotivoAutorizacion.Text;
                ...
                if (this.hiddenId.Value == string.Empty) //nueva valuación de proyecto
                {
                    ...
                    if (valuacionProyecto.ListaProductosValuacionProyecto.Count > 0)
                    {
                        ...
                        var nuevaValuacion = new CN_CapValuacionProyecto().InsertarValuacionProyecto(valuacionProyecto, vp, VPA, sesion, ObtenerProyectosSeleccionados(), _ibt);

                        //Crear notificación para el rik y para el aprobador.
                        /*
                            CN_CapRIKNotificacion cnCapRIKNotificacion = new CN_CapRIKNotificacion();
                            cnCapRIKNotificacion.CrearNotificacionNuevaValuacion(sesion, string.Format("Se ha creado la valuación {0}. A continuación esta será revisada por un evaluador. Se le notificará en su brevedad.", nuevaValuacion.Id_Vap), _ibt);
                            se comenta 2 Oct 2018 RFH 
                         */
                        ...
                        */
                        ...
                        Int32.TryParse(txtFolio.Text, out CrmOp_Id_Val);

                        int iResult = 0;
                        CN_CrmOportunidad Op = new CN_CrmOportunidad();
                        iResult = Op.CrmOportunidades_UpdateEstatus(sesion.Id_Emp, sesion.Id_Cd, CrmOp_Id_Val, CrmOp_Id_Op, 3, sesion.Emp_Cnx);
                        ...
                        throw new HttpException(500, "Ingrese por lo menos un producto en el detalle");
                ...
                else
                {
                    ...
                    if (valuacionProyecto.ListaProductosValuacionProyecto.Count > 0)
                    {
                        //
                        // 17 Sep 2018
                        // Se agrega metodo para actualizar parametros

                        eCapValProyectoParametros eCPP = new eCapValProyectoParametros();
                        eCPP.Id_Emp = VPA.Id_Emp;
                        eCPP.Id_Cd = VPA.Id_Cd;
                        eCPP.Id_Vap = VPA.Id_Vap;
                        eCPP.Cetes = VPA.txtCetes;
                        eCPP.Costodecapital = VPA.txtCostodecapital;
                        eCPP.CuentasPorCobrar = VPA.txtCuentasPorCobrar;
                        eCPP.Financiamientoproveedores = VPA.txtFinanciamientoproveedores;
                        eCPP.FleteLocales = VPA.txtFleteLocales;
                        eCPP.FletesPagadosalCliente = VPA.txtFletesPagadosalCliente;
                        eCPP.GastosServirCliente = VPA.txtGastosServirCliente;
                        eCPP.GastosVarAplTerr = VPA.txtGastosVarAplTerr;
                        eCPP.Inventario = VPA.txtInventario;
                        eCPP.Inversionactivosfijos = VPA.txtInversionactivosfijos;
                        eCPP.Isr = VPA.txtIsr;
                        eCPP.ManoObra = VPA.txtManoObra;
                        eCPP.Vigencia = VPA.txtVigencia;
                        eCPP.SqlResult = 0;

                        CN_CapValuacionProyecto CVP = new CN_CapValuacionProyecto();
                        eCPP = CVP.InsertUpdate_CapValProyectoParametros(eCPP, Sesion.Emp_Cnx);
            ...
            catch (CapturarMotivoParaAutorizarException cmpae)
            {
                ...
                //Alerta("El resultado de la valuación es negativo. Por favor capture el motivo de autorización");
                //Response.StatusCode = 500;
                throw new HttpException(500, "El resultado de la valuación es negativo. Por favor capture el motivo de autorización");
                //Response.StatusDescription = "El resultado de la valuación es negativo. Por favor capture el motivo de autorización";
                //Response.Status = "El resultado de la valuación es negativo. Por favor capture el motivo de autorización";
                //Response.Flush();
                //Response.End();
                //throw cmpae;
            }
        ...
        // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        //
        // REEMPLAZA LA VERSION ARRIBA 
        // ESTA SERA LA FINAL UTILIZANDO SPs
        //
        //
        private void Guardar_Ver2()
        {
            int iFolio = 0;
            Int32.TryParse(txtFolio.Text, out iFolio);

            int Id_Op_ = 0;
            Int32.TryParse(hfId_Op.Value.ToString(), out Id_Op_);

            int TipoValuacion = 0;
            Int32.TryParse(hfTipoValuacion.Value.ToString(), out TipoValuacion);

            int CrmOp_Id_Val = 0;
            int Id_Cte_ = 0;
            Int32.TryParse(hfId_Cte.Value.ToString(), out Id_Cte_);

            Int32.TryParse(hfId_Ter.Value.ToString(), out Id_Ter_);
            ...
            try
            {
                ...
                /*
                 ValuacionProyecto VPro = new ValuacionProyecto();
                 VPro.Id_Rik = CRM_Usuario_Rik > 0 ? CRM_Usuario_Rik : sesion.Id_Rik;
                 VPro.Id_Ter = Id_Ter;
                 VPro.MotivoParaAutorizacion = txtMotivoAutorizacion.Text;
                 VPro.Id_Emp = sesion.Id_Emp;
                 VPro.Id_Cd = sesion.Id_Cd_Ver;
                 VPro.Id_Vap = Convert.ToInt32(txtFolio.Text);
                 VPro.Id_U = sesion.Id_U;
                 VPro.Id_Cte = Convert.ToInt32(txtCliente.Value.HasValue ? txtCliente.Value : -1);
                 Funciones funcion = new Funciones();
                 VPro.Vap_Fecha = Convert.ToDateTime(txtFecha.SelectedDate.Value.ToShortDateString() + funcion.GetLocalDateTime(sesion.Minutos).ToString(" HH:mm:ss"));
                 VPro.Vap_Nota = txtNota.Text;
                 VPro.Vap_Estatus = "C";
                 VPro.Vap_UtilidadRemanente = _resultadosValuacion.UtilidadRemanente;
                 VPro.Vap_ValorPresenteNeto = _resultadosValuacion.ValorPresenteNeto;
                 */
                ...
                //
                VPro.ListaProductosValuacionProyecto = this.ListaProductosValProyecto;


                // 
                //  VALUACION PARAMETROS - ACTUAL 
                // 
                ...
                // 
                //  VALUACION PARAMETROS 
                // 

                ValuacionParametros VParam = new ValuacionParametros();
                VParam.Id_Cd = sesion.Id_Cd_Ver;
                VParam.Id_Emp = sesion.Id_Emp;
                VParam.Vap_Vigencia = txtVigencia.Value.HasValue ? (int)txtVigencia.Value.Value : 0;
                //VParam.Vap_Participacion = txtComision.Value.HasValue ? txtComision.Value.Value : 0;
                VParam.Vap_Mano_Obra = txtManoObra.Value.HasValue ? txtManoObra.Value.Value : 0;
                //VParam.Vap_Amortizacion = txtAmortizacion.Value.HasValue ? txtAmortizacion.Value.Value : 0;
                //VParam.Vap_Numero_Entregas = txtNumEntregas.Value.HasValue ? (int)txtNumEntregas.Value.Value : 0;
                //VParam.Vap_Costo_Entregas = txtCostoEntregas.Value.HasValue ? txtCostoEntregas.Value.Value : 0;
                //VParam.Vap_Comision_Factoraje = txtComisionFactoraje.Value.HasValue ? txtComisionFactoraje.Value.Value : 0;
                //VParam.Vap_Comision_Anden = txtComisionCruce.Value.HasValue ? txtComisionCruce.Value.Value : 0;
                VParam.Vap_Gasto_Flete_Locales = txtFleteLocales.Value.HasValue ? txtFleteLocales.Value.Value : 0;
                //VParam.Vap_IVA = txtIva.Value.HasValue ? txtIva.Value.Value : 0;
                //VParam.Vap_Plazo_Pago_Cliente = txtPlazoPago.Value.HasValue ? txtPlazoPago.Value.Value : 0;
                //VParam.Vap_Inventario_Key = txtInventarioKey.Value.HasValue ? (int)txtInventarioKey.Value.Value : 0;
                //VParam.Vap_Inventario_Consignacion = txtInventarioKeyConsignacion.Value.HasValue ? (int)txtInventarioKeyConsignacion.Value.Value : 0;
                //VParam.Vap_Credito_Key = txtCreditoProveedor.Value.HasValue ? (int)txtCreditoProveedor.Value.Value : 0;
                //VParam.Vap_Credito_Papel = txtCreditoProveedorPapel.Value.HasValue ? (int)txtCreditoProveedorPapel.Value.Value : 0;
                VParam.Vap_ISR = txtIsr.Value.HasValue ? txtIsr.Value.Value : 0;
                //VParam.Vap_Ucs = txtUcs.Value.HasValue ? txtUcs.Value.Value : 0;
                VParam.Vap_Cetes = txtCetes.Value.HasValue ? txtCetes.Value.Value : 0;
                //VParam.Vap_Adicional_Cetes = txtAdicionalCetes.Value.HasValue ? txtAdicionalCetes.Value.Value : 0;
                //VParam.Vap_Costos_Fijos_No_Papel = txtCostosFijosNoPapel.Value.HasValue ? txtCostosFijosNoPapel.Value.Value : 0;
                //VParam.Vap_Costos_Fijos_Papel = txtCostosFijosPapel.Value.HasValue ? txtCostosFijosPapel.Value.Value : 0;
                //VParam.Vap_Gastos_Admin = txtGAdmitivos.Value.HasValue ? txtGAdmitivos.Value.Value : 0;
                //VParam.Vap_Inversion_Activos = txtInversionActivosFijos.Value.HasValue ? (int)txtInversionActivosFijos.Value.Value : 0;                
                ...
                if (CRM_Usuario_Rik > 0)
                {
                    VPro.Id_Rik = CRM_Usuario_Rik;
                }
                else
                {
                    VPro.Id_Rik = sesion.Id_Rik;
                }
                ...
                List<string> ProductosAlertaa = new List<string>();
                ProductosAlertaa = (List<string>)Session["ProdsAutorizacion" + Session.SessionID];
                // INSERT CAPVALOPROYECTO
                //JFCV estatus capturado
                if (ProductosAlertaa.Count() > 0)
                {
                    VPro.Vap_Estatus = "A";
                    VPro.Vap_Estatus2 = 0;
                }
                ...
                CN_CapValuacionProyecto CN_CapValP = new CN_CapValuacionProyecto();
                var agrupadosPorIdOp = VPro.ListaProductosValuacionProyecto.GroupBy(x => x.Id_Op);
                ValuacionProyecto VProReesult = new ValuacionProyecto();
                VProReesult = VPro;
                if (agrupadosPorIdOp.Any() && Id_Op_ == 0)
                {
                    foreach (var grupo in agrupadosPorIdOp)
                    {
                        foreach (var producto in grupo)
                        {

                            VPro.Id_Ter = producto.Id_Ter;
                            VProReesult = CN_CapValP.InsertarValuacionProyecto_Ver2(
                               Sesion.Id_Emp, Sesion.Id_Cd, grupo.Key, Id_Cte_, iFolio, TipoValuacion,
                               VPro, VParam, VPA, sesion.Emp_Cnx);
                            VPA.Id_Vap = Int32.Parse(this.Valor);
                        }
                ...
                {
                    VProReesult = CN_CapValP.InsertarValuacionProyecto_Ver2(
                     Sesion.Id_Emp, Sesion.Id_Cd, Id_Op_, Id_Cte_, iFolio, TipoValuacion,
                     VPro, VParam, VPA, sesion.Emp_Cnx);
                }
                ...
                if (0 == 1)
                {
                    ...
                    if (this.hiddenId.Value == string.Empty) //nueva valuación de proyecto
                    {
                        ...
                        if (VPro.ListaProductosValuacionProyecto.Count > 0)
                        {
                            //new CN_CapValuacionProyecto().InsertarValuacionProyecto(ref valuacionProyecto, vp, sesion.Emp_Cnx, ref verificador);                        
                            //var nuevaValuacion = new CN_CapValuacionProyecto().InsertarValuacionProyecto(ref VPro, VParam, sesion, ref verificador, ObtenerProyectosSeleccionados(), _ibt);
                    ...
                    else
                    {
                        ...
                        if (VPro.ListaProductosValuacionProyecto.Count > 0)
                        {
                            ...
                            //new CN_CapValuacionProyecto().ModificarValuacionProyecto(ref valuacionProyecto, vp, sesion.Emp_Cnx, ref verificador);
                            //new CN_CapValuacionProyecto().ModificarValuacionProyecto(ref VPro, VPA, sesion, ref verificador, ObtenerProyectosSeleccionados(), _ibt);
                            _ibt.Commit();
                            _ibt.Begin();
                            //_validarVisibilidadComandoPropuestaEconomica(valuacionProyecto.Id_Vap);
                            EnviaEmail(Convert.ToInt32(txtFolio.Text));
        ...
        //JFCV 16 Jun 2022 LISTADO DE PRODUCTOS  Validar que tengan precio mayor a Precio minimo Rik 
        private bool Guardar_Valida()
        {
            int iFolio = 0;
            Int32.TryParse(txtFolio.Text, out iFolio);
            ...
            try
            {
                ...
                //JFCV 16 Jun 2022 LISTADO DE PRODUCTOS  Validar que tengan precio mayor a Precio minimo Rik 
                //
                //JFCV validar precios minimos para alertas
                #region inicio validar precios alertas 
                string ConexionCentral = ConfigurationManager.AppSettings["strConnectionCentral"].ToString();
                //JFCV 26 enero 2021 Validar si tiene precios inferiores a los precios minimos
                ...
                //JFCV 24 ene inicializar en nulo los datos de la sesión. tenga o no que validarlo
                Session["ProdsAutorizacion" + Session.SessionID] = ProductosAlerta;
                Session["lAurizacionPrecios" + Session.SessionID] = lalertasautdet;
                Session["Id_FacPrec" + Session.SessionID] = txtFolio.Text;


                if (ListaProductosValProyecto.Count > 0)
                {
                    ...
                    int GLOBAL_CRM_ValidaPrecioMinimoRik = 0;
                    CapaEntidad.eSysConfiguracion SysC = new CapaEntidad.eSysConfiguracion();
                    try
                    {
                        CN_SysConfigruacion CN = new CN_SysConfigruacion();
                    ...
                    if (GLOBAL_CRM_ValidaPrecioMinimoRik == 1)
                    {
                        foreach (ValuacionProyectoDetalle productoavalidar in ListaProductosValProyecto)
                        {

                            double PrecioIngresado = Convert.ToDouble(productoavalidar.Vap_Precio);

                            if (Convert.ToInt64(productoavalidar.Id_Prd) <= 999999999999)
                            {
                                //JFCV 20 Junio 2021 Valido si el precio de venta es menor al minimo
                                ...
                                alertaaut.Id_Cte = Convert.ToInt32(txtCliente.Value.HasValue ? txtCliente.Value.Value : -1);
                                alertaaut.Cte_NomComercial = txtClienteNombre.Text.Trim() != "" ? txtClienteNombre.Text.Trim().Trim() : "";
                                alertaaut.Id_Prd = Convert.ToInt64(productoavalidar.Id_Prd);
                                alertaaut.IdRepresentante = Convert.ToInt32(VPro.Id_Rik);
                                if (VPro.Id_Ter != 0)
                                {
                                    alertaaut.Id_Ter = Convert.ToInt32(VPro.Id_Ter);
                                }
                                else
                                {
                                    alertaaut.Id_Ter = Convert.ToInt32(productoavalidar.Id_Ter);
                                }

                                double Preciodefactura = Convert.ToDouble(productoavalidar.Vap_Precio);
                                double Preciodeesp = Convert.ToDouble(productoavalidar.Vap_PrecioEspecial);

                                if (Preciodefactura < Preciodeesp)
                                {
                                    ...
                                    if (alertaautdet != null && alertaautdet.Precio_MinimoRik > 0)
                                    {
                                        ...
                                        if (Math.Round(Preciodefactura, 3) < alertaautdet.PrecioObjetivo)
                                        {
                                            ...
                                            pasoalertaautdet.Id_Cte = Convert.ToInt32(txtCliente.Value.HasValue ? txtCliente.Value.Value : -1);
                                            pasoalertaautdet.IdRepresentante = Convert.ToInt32(VPro.Id_Rik);
                                            if (VPro.Id_Ter != 0)
                                            {
                                                pasoalertaautdet.Id_Ter = Convert.ToInt32(VPro.Id_Ter);
                                            }
                                            else
                                            {
                                                pasoalertaautdet.Id_Ter = Convert.ToInt32(productoavalidar.Id_Ter); //Convert.ToInt32(alertaautdet.Id_Ter);
                                            }
                                            ...
                                            pasoalertaautdet.PrecioLista = alertaautdet.PrecioLista;
                                            pasoalertaautdet.Cantidad = Convert.ToInt32(productoavalidar.Vap_Cantidad);
                                            ...
                                            pasoalertaautdet.Prd_Descripcion = productoavalidar.Producto.Prd_Descripcion;
                                            pasoalertaautdet.Cte_NomComercial = txtClienteNombre.Text;
                                            pasoalertaautdet.TipoAutorizacion = 1; //valuación de proyecto
                                ...
                                //JFCV 24 Abril 2022 fin Valido si el precio de venta es menor al minimo
                            }// si prod es menor a 999999999999
                        }  //Termina ciclo  convenios  

                        //JFCV 17 feb 2021 alerta de precios autorización
                        //JFCV 26 enero 2021 Validar si tiene precios inferiores a los precios minimos


                        if (ProductosAlerta.Count > 0 && lalertasautdet.Count > 0)
                        {
                            string procesar = GeneraReporteVPAlerta();
                            //inserto en variable de sesion la rentabilidad 
                            //Session["CalculoUafir" + Session.SessionID] = er; EstadisticaRentabilidad

                            Session["ProdsAutorizacion" + Session.SessionID] = ProductosAlerta;
                            Session["lAurizacionPrecios" + Session.SessionID] = lalertasautdet;
                            Session["Id_FacPrec" + Session.SessionID] = txtFolio.Text;
                            //RAM1.ResponseScripts.Add("AbrirVentana_AlertaAutorizacionPrecios(); return false;");

                            ScriptManager.RegisterStartupScript(this, this.GetType(), "AbrirVentana_AlertaPrecios", "AbrirVentana_AlertaPrecios()", true);
                            bool continuar = false;

                            //Alerta("No se actualizo la información de la valuación, ya que hay precios no autorizados.");
                            return continuar;
                        }
                        //JFCV 2022 FIn Validar si tiene precios inferiores a los precios minimos

                        //JFCV convenios valida precios de alertas 
                    }
                ...
                #endregion inicio validar precios alertas 
        ...
        /// </summary>
        // 24JUN-2019 RFG - se descarta funcion
        //
            ...
            try
            {
                ...
                if (this.hiddenId.Value == string.Empty) //nueva valuación de proyecto
                {
                    ...
                    if (valuacionProyecto.ListaProductosValuacionProyecto.Count > 0)
                    {
                        ...
                        //Crear notificación para el rik y para el aprobador.
                        /*
                        //Se retira por lo pronto 2 Oct 2018
                        CN_CapRIKNotificacion cnCapRIKNotificacion = new CN_CapRIKNotificacion();
        ...
        private void EnviaEmail(int solicitud)
        {
            try
            {
                ...
                sb_mail.Append("<div align='center'>");
                sb_mail.Append("<table style='font-family: verdana; font-size:9pt'>");
                sb_mail.Append("<tr>");
                sb_mail.Append("<td>");
                ...
                sb_mail.Append("<tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("<IMG SRC=\"https://key.com.mx/templates/theme-org/images/presets/preset1/logo.png\" ALIGN='left'>");
                sb_mail.Append("</td>");
                sb_mail.Append("</tr><tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("<h1>CRM</h1>");
                sb_mail.Append("<h4>Key Química</h4>");
                sb_mail.Append("</td>");
                sb_mail.Append("</tr>");
                sb_mail.Append("</table>");
                ...
                sb_mail.Append("<tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("<b>Solicitud de autorización de valuación de proyectos</b>");
                sb_mail.Append("</td>");
                sb_mail.Append("</tr><tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("Folio:" + solicitud.ToString() + "</b>");
                sb_mail.Append("</td>");
                sb_mail.Append("</tr><tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("Centro de distribución:" + session.Id_Cd_Ver + " - " + session.Cd_Nombre);
                sb_mail.Append("</td>");
                sb_mail.Append("</tr><tr>");
                sb_mail.Append("<td>");
                sb_mail.Append("Solicitó:" + session.Id_U + " - " + session.U_Nombre);
                sb_mail.Append("</td>");
                sb_mail.Append("</tr>");
                sb_mail.Append("</table>");
                ...
                string[] URL = HttpContext.Current.Request.Url.AbsoluteUri.ToString().Split(new char[] { '/' });

                sb_mail.Append("<table style=\"width:250px;\">");
                sb_mail.Append("<tr>");

                sb_mail.Append("<td ");
                sb_mail.Append("style=\"font-family:'Helvetica Neue',Helvetica,sans-serif;");
                sb_mail.Append("color:white; width:100%; max-width:280px; text-decoration:none; text-align:center; ");
                sb_mail.Append("display:inline-block!important; font-size:16px; line-height:40px; -webkit-border-radius:2px;");
                sb_mail.Append("-moz-border-radius:2px; border-radius:2px; font-weight:bold; margin-top:0; margin-right:0; ");
                sb_mail.Append("margin-bottom:0; margin-left:0; padding-top:0; padding-right:0; padding-bottom:0; padding-left:0; background-color:#49b1ba\"");
                sb_mail.Append(">");

                sb_mail.Append("<a href=\"http://" + URL[2] + "/" + URL[3] + "/PortalRik/GestionPromocion/Fulldashboard.aspx\"");
                //sb_mail.Append("<a href=\"http://" + URL[2] + "/PortalRik/GestionPromocion/Fulldashboard.aspx\"");

                sb_mail.Append("target=\"_blank\" ");
                sb_mail.Append("rel=\"noopener noreferrer\" ");
                sb_mail.Append("data-auth=\"NoApplicable\" ");
                sb_mail.Append("style=\"color:#FFFFFF; display:inline-block; text-decoration:none; magin:10px;\">");
                sb_mail.Append("Abrir SIANWEB");
                sb_mail.Append("</a>");
                ...
                //sb_mail.Append("<center>");
                //sb_mail.Append("<br>");

                //sb_mail.Append("<a href='" + Request.Url.ToString().Replace(url[url.Length - 1], "") + "CapValProyectos_Autorizacion.aspx?Id1=" + solicitud.ToString() + "&id2=" + session.Id_Emp + "&id3=" + session.Id_Cd_Ver + "'>Solicitud de autorización de valuación de proyectos</a></font></center>");
                //sb_mail.Append("<a href=\"http://" + URL[2] + "/" + URL[3] + "/GestionPromocion/Fulldashboard.aspx\"");

                // sb_mail.Append("<center><br><a href='http://" + url + "/CapValProyectos_Autorizacion.aspx?id1=" + solicitud.ToString() + "&id2=" + session.Id_Emp + "&id3=" + session.Id_Cd_Ver + "'>Solicitud de autorización de valuación de proyectos</a></font></center>");
                // sb_mail.Append("<center><br><a href='" + Request.Url.ToString().Replace((new FileInfo(Request.Url.AbsolutePath)).Name, "") + "CapValProyectos.aspx?id1=" + solicitud.ToString() + "&id2=" + session.Id_Emp + "&id3=" + session.Id_Cd_Ver + "'>Solicitud de autorización de valuación de proyectos</a></font></center>");
                sb_mail.Append("</td>");
                sb_mail.Append("</tr>");
                sb_mail.Append("</table>");
                sb_mail.Append("</div>");
                ...
                m.Subject = "Solicitud de Autorización de Valuación";
                m.IsBodyHtml = true;
                string body = sb_mail.ToString();
        ...
        protected void CargarListadoProyectos(int idVal)
        {
            ...
            int idCte = Convert.ToInt32(txtCliente.Value);
            var proyectos = cnCrmValuacionOportunidades.ObtenerPorValuacion(Sesion, idCte, idVal);
            foreach (GridDataItem item in rgProyectos.MasterTableView.Items)
            {
                CheckBox chk = item["chkColSeleccion"].Controls[0] as CheckBox;
                if (chk != null)
                {
                    TableCell cell = (TableCell)item["Id"];
                    int idOp = Convert.ToInt32(cell.Text);
                    var proyecto = (from p in proyectos
                                    where p.Id_Op == idOp
                                    select p).ToList();
                    if (proyecto.Count > 0)
                    {
        ...
        protected void VisualizarPropuestaEconomica()
        {
            ...
            if (CRM_Usuario_Rik > 0)
            {
                ALValorParametrosInternos.Add(CRM_Usuario_Rik);
            }
            else
            {
        ...
        */
        ...
        // 24JUL-2019
        // Se cambia clase de talle 
        private List<ValuacionProyectoDetalle> _proyectosSeleccionados
        {
            get
            {
                object sessionParam = ViewState["_proyectosSeleccionados"];
                if (sessionParam == null)
                {
                    sessionParam = new List<ValuacionProyectoDetalle>();
                    ViewState.Add("_proyectosSeleccionados", sessionParam);
                }
                return sessionParam as List<ValuacionProyectoDetalle>;
            }
            set
            {
                Session["_proyectosSeleccionados"] = value;
