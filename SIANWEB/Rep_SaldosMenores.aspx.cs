...
namespace SIANWEB
{
    public partial class Rep_SaldosMenores : System.Web.UI.Page
    {
        #region Variables
        ...
        {
            if (Session["RepSaldoMenor"] != null)
            {
                List<SaldoMenor> Lista = (List<SaldoMenor>)Session["RepSaldoMenor"];
                GrdReporteSaldo.DataSource = Lista;
                GrdReporteSaldo.DataBind();
            }
        }
        ...
        private void CargarGrid()
        {
            try
            {
                List<SaldoMenor> List = new List<SaldoMenor>();
                CN_SaldoMenor CN = new CN_SaldoMenor();
                Sesion session2 = new Sesion();
                session2 = (Sesion)Session["Sesion" + Session.SessionID];
                SaldoMenor Datos = new SaldoMenor();
                ...
                Datos.Doc_tipo = ddlDoc.Value.ToString().ToUpper();
                CN.ConsultarSaldo(Datos, ref List, session2.Emp_Cnx);

                Session["RepSaldoMenor"] = List;
                GrdReporteSaldo.DataSource = List;
                GrdReporteSaldo.DataBind();
            }
            catch (Exception ex)
            {
        ...
        {
            GrdReporteSaldo.ExportXlsToResponse(new XlsExportOptionsEx { ExportType = ExportType.WYSIWYG });
        }
