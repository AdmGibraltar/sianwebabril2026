...
namespace SIANWEB
{
    public partial class ProOrdenCompra_Archivo_S_OP_Excel : System.Web.UI.Page
    {
        ...
        protected void btnImportar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // string path = "C:\\BK_KeyWeb" + "\\" + NombreArchivo;
        ...
        private void GuardarDeExcel(DataSet ds)
        {
            try
            {
                ...
                if (ds.Tables != null)
                {
                    ...
                    ordCompra.Ord_Fecha_Entrega = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, fechaServidor.Hour, fechaServidor.Minute, fechaServidor.Second);
                    ordCompra.Id_Mon = 2;
                    ...
                    {
                        if (row[1] != DBNull.Value && float.TryParse(string.IsNullOrEmpty(row[1].ToString()) ? "0" : row[1].ToString(), out datoOrig))
                        {

                            int prd = 0;//!string.IsNullOrEmpty(row[0].ToString()) ? int.TryParse(row[0].ToString(), out prd) : 0;

                            if (!int.TryParse(row[0].ToString(), out prd))
                                throw new Exception(String.Format("En el renglon {0} del archivo hay una partida con un dato inválido", i + 1));


                            ordenado = Convert.ToInt32(Math.Ceiling(datoOrig));
