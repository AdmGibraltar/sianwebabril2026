...
namespace SIANWEB
{
    public partial class ProAuxiliarV2 : Page
    {
        public static Sesion MySesion { get; set; }
        public bool PermisoGuardar
        {
            get { return GetPermisoValor("PermisoGuardar"); }
            set { SetPermisoValor("PermisoGuardar", value); }
        }

        public bool PermisoModificar
        {
            get { return GetPermisoValor("PermisoModificar"); }
            set { SetPermisoValor("PermisoModificar", value); }
        }

        public bool PermisoEliminar
        {
            get { return GetPermisoValor("PermisoEliminar"); }
            set { SetPermisoValor("PermisoEliminar", value); }
        }

        public bool PermisoImprimir
        {
            get { return GetPermisoValor("PermisoImprimir"); }
            set { SetPermisoValor("PermisoImprimir", value); }
        }

        public List<Comun> Rutas { get; set; }
        public List<Comun> Auxiliar { get; set; }
        public List<PedidoDet> AsignacionPorProducto { get; set; }
        public List<Pedido> AsignacionPorPedido { get; set; }
        public List<Pedido> PedidosSinIdentificar { get; set; }

        public string AsignacionPorProductoJson { get; set; }
        public List<Comun> Clientes { get; set; }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (ConfigurationManager.AppSettings["ValidaPickingApp"].ToString() == "S")
                {
                    if (Session["p"].ToString() != "1")
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "Picking", "sweetAlertMessageRegresa('Solo permite operar el picking por medio de la tablet');", true);
                        return;
                    }
                }
                CargarRutas();
                CargarAuxiliares();
                ValidarPermisos();
                //TxtFechaEntregaInicial.Text = DateTime.Parse("2021-11-01").ToString("yyyy-MM-dd");
                //TxtFechaEntregaFinal.Text = DateTime.Parse("2021-11-10").ToString("yyyy-MM-dd");
                TxtFechaEntregaInicial.Text = DateTime.Now.ToString("yyyy-MM-dd");
                TxtFechaEntregaFinal.Text = MySesion.CalendarioFin.ToString("yyyy-MM-dd");
                AsignacionPorProducto = GetListPorProdcto();
                AsignacionPorPedido = GetListPorPedido();
                PedidosSinIdentificar = GetListPorPedidoSinIdentificar();
                CargarClientes();
            }
        }
        ...
        {
            var result = (List<PedidoDet>)Session[Session.SessionID + "ListaResultadosTemporal"];
            var listaResultados = new List<PedidoDet>();
            var listaResultadosRepetidos = new List<PedidoDet>();
            Session[Session.SessionID + "ListaResultados"] = new List<PedidoDet>();

            thTipo.Style.Add("display", "table-cell");
            thCredito.Style.Add("display", "table-cell");
            thParcialidades.Style.Add("display", "table-cell");
            if (chAgruparProducto2.Checked)
            {
                thTipo.Style.Add("display", "none");
                thCredito.Style.Add("display", "none");
                thParcialidades.Style.Add("display", "none");
                foreach (var item in result)
                {
                    // Verificar si ya existe un elemento en la lista con el mismo producto y ruta
                    var elementoExistente = listaResultados.FirstOrDefault(x => x.Id_Prd == item.Id_Prd && x.Ruta == item.Ruta);

                    if (elementoExistente != null)
                    {
                        // Si el elemento existe, sumar las cantidades
                        elementoExistente.Ped_Cantidad += item.Ped_Cantidad;
                        elementoExistente.Ped_Pendiente += item.Ped_Pendiente;
                        elementoExistente.Ped_Asignado += item.Ped_Asignado;
                        elementoExistente.Ped_Picking += item.Ped_Picking;
                        elementoExistente.Ped_Facturado += item.Ped_Facturado;
                        elementoExistente.Ped_Remisionado += item.Ped_Remisionado;
                        elementoExistente.Prd_Disponible += item.Prd_Disponible;
                        elementoExistente.Ped_CantidadDisponible += item.Ped_CantidadDisponible;
                        elementoExistente.Agrupado = true;
                        // Puedes seguir sumando las demás propiedades según sea necesario
                    }
                    else
                    {
                        // Si el elemento no existe, crear uno nuevo y agregarlo a la lista
                        PedidoDet oPedidoDet = new PedidoDet();
                        oPedidoDet.Id_Prd = item.Id_Prd;
                        oPedidoDet.Ruta = item.Ruta;
                        oPedidoDet.CreditoStr = item.CreditoStr;
                        oPedidoDet.TipoPedido = item.TipoPedido;
                        oPedidoDet.Prd_Descripcion = item.Prd_Descripcion;
                        oPedidoDet.Prd_Presentacion = item.Prd_Presentacion;
                        oPedidoDet.Prd_UniNe = item.Prd_UniNe;

                        oPedidoDet.Ped_Cantidad = item.Ped_Cantidad;
                        oPedidoDet.Ped_Pendiente = item.Ped_Pendiente;
                        oPedidoDet.Ped_Asignado = item.Ped_Asignado;
                        oPedidoDet.Ped_Picking = item.Ped_Picking;
                        oPedidoDet.Ped_Facturado = item.Ped_Facturado;
                        oPedidoDet.Ped_Remisionado = item.Ped_Remisionado;
                        oPedidoDet.Prd_Disponible = item.Prd_Disponible;
                        oPedidoDet.Ped_CantidadDisponible = item.Ped_CantidadDisponible;
                        oPedidoDet.Ped_PermiteParcialidades = item.Ped_PermiteParcialidades;
                        oPedidoDet.Agrupado = true;
                        listaResultados.Add(oPedidoDet);

                    }


                    PedidoDet oPedidoDetRep = new PedidoDet();
                    oPedidoDetRep.Id_Prd = item.Id_Prd;
                    oPedidoDetRep.Ruta = item.Ruta;
                    oPedidoDetRep.CreditoStr = item.CreditoStr;
                    oPedidoDetRep.Ped_PermiteParcialidades = item.Ped_PermiteParcialidades;
                    oPedidoDetRep.TipoPedido = item.TipoPedido;
                    listaResultadosRepetidos.Add(oPedidoDetRep);
                }
                Session[Session.SessionID + "ListaResultados"] = listaResultadosRepetidos;
            }
            else
            {
                foreach (var item in result)
            ...
            AsignacionPorProductoJson = new JavaScriptSerializer().Serialize(listaResultados);
            AsignacionPorProducto = listaResultados;
        ...
        void CargarClientes()
        {
            try
            {
                ...
                var clientes = new List<Comun>();
                cd.LlenaCombo(1, MySesion.Id_Emp, MySesion.Id_Cd_Ver, 1, "spCatClientes_Combo", MySesion.Emp_Cnx, ref clientes);
        ...
        void CargarRutas()
        {
            try
            {
                ...
                cd.LlenaCombo(1, MySesion.Id_Emp, MySesion.Id_Cd_Ver, 1, "spCatRutasEntregas_Combo", MySesion.Emp_Cnx, ref rutas);
                Rutas = rutas;
            }
            catch { }
        }
        ...
        List<PedidoDet> GetListPorProdcto()
        {
            var result = new List<PedidoDet>();
            try
            {
                var pedido = new Pedido
                {
                    ...
                    Filtro_CteIni = "",
                    Filtro_CteFin = "",
                    Filtro_FecEntregaIni = DateTime.Parse(TxtFechaEntregaInicial.Text),
                    Filtro_FecEntregaFin = DateTime.Parse(TxtFechaEntregaFinal.Text),
                    Filtro_Estatus = "1",
                    Filtro_Credito = 0,
                    Ruta = cmbRuta.Value,
                    Id_Aux = string.IsNullOrEmpty(cmbAuxiliar.Value) ? 0 : Convert.ToInt32(cmbAuxiliar.Value),
                    Id_Cte = int.Parse(id_cliente2.Value),
                };

                new CN_CapPedido().ConsultaPedidoAsig_Admin_FechaEntregaPorProducto_Picking(pedido, MySesion.Emp_Cnx, ref result);
            }
            catch { }
            var listaResultados = new List<PedidoDet>();
            var listaResultadosRepetidos = new List<PedidoDet>();
            Session[Session.SessionID + "ListaResultados"] = new List<PedidoDet>();
            Session[Session.SessionID + "ListaResultadosTemporal"] = new List<PedidoDet>();
            ...
            Session[Session.SessionID + "ListaResultadosTemporal"] = listaResultados;
            AsignacionPorProductoJson = new JavaScriptSerializer().Serialize(listaResultados);

            //foreach (Control control in Page.Controls)
            //{
            //    // Verificar si el control es un td y tiene el atributo "name" igual a "miTd"
            //    if (control is HtmlTableCell && ((HtmlTableCell)control).Attributes["name"] == "tdParcialidades")
            //    {
            //        // Ocultar el td
        ...
        List<Pedido> GetListPorPedido()
        {
            ...
            try
            {
                ...
                new CN_CapPedido().ConsultaPedidoAsig_Admin_FechaEntrega_Picking(pedido, MySesion.Emp_Cnx, ref result);
            }
            catch { }
            return result;
        }
        ...
        List<Pedido> GetListPorPedidoSinIdentificar()
        {
            var result = new List<Pedido>();
            try
            {
                ...
                new CN_CapPedido().ConsultaPedidoAsig_Admin_FechaEntrega_Picking_SinIdentificar(pedido, MySesion.Emp_Cnx, ref result);
            }
            catch { }
            return result;
        }
        ...
        void ValidarPermisos()
        {
            try
            {
                ...
                pagina.Url = pag.Length > 1
                    ? (new FileInfo(Page.Request.Url.AbsolutePath)).Name + "?" + pag[1]
                    : (new FileInfo(Page.Request.Url.AbsolutePath)).Name;

                var cn = new CN_Pagina();
                cn.PaginaConsultar(ref pagina, MySesion.Emp_Cnx);
                ...
                var cd = new CD_PermisosU();
                cd.ValidaPermisosUsuario(ref permiso, MySesion.Emp_Cnx);
                if (permiso.PAccesar == true)
                {
                    PermisoGuardar = permiso.PGrabar;
                    PermisoModificar = permiso.PModificar;
                    PermisoEliminar = permiso.PEliminar;
        ...
        bool GetPermisoValor(string permiso)
        {
            if (Session["Sesion" + Session.SessionID] == null) return false;
            //return (bool)Session[permiso + Session.SessionID + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]];
            return true;
        }

        void SetPermisoValor(string permiso, bool value)
        {
            Session[permiso + Session.SessionID + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]] = value;
        }
        ...
        [WebMethod]
        public static ResponseJson ConfirmarPedido(int id_Prd, string ruta, string credito, string parcialidades, string tipoPedido, bool chAgrupar)
        {
            ...
            try
            {
                ...
                if (chAgrupar)
                {
                    List<PedidoDet> datosAgrupados = (List<PedidoDet>)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + "ListaResultados"];
                    var elementosCoincidentes = datosAgrupados.Where(x => x.Id_Prd == id_Prd && x.Ruta == ruta).ToList();
        ...
        [WebMethod]
        public static ResponseJson CancelarPedido(int id_Prd, string ruta, string credito, string parcialidades, string tipoPedido, bool chAgrupar)
        {
            var response = new ResponseJson
            {
                Status = false,
        ...
        protected void chAgruparProducto_CheckedChanged(object sender, EventArgs e)
        {
            this.GetData();
        }
        ...
        //                TxtFechaEntregaInicial.DbSelectedDate = DateTime.Parse("01/11/2021");
        //                TxtFechaEntregaFinal.DbSelectedDate = DateTime.Parse("10/11/2021");
        //                rgAsignacionporproducto.Rebind();
        //                rgAsignacion.Rebind();
        //            }
        //            catch (Exception ex)
