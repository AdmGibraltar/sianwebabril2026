...
namespace SIANWEB
{
    public partial class CapPagoSubrogacion : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void Timbrar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                {
                    hdnValor.Value = txtFecha1.SelectedDate.ToString().Substring(0, 10);

                    string script = "radconfirm('¿Es correcto la fecha elegida?', confirmCallBackFn, 350, 150);";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            timbrarsubrogacion(listaPago, Emp_Cnx, ConexionCfdII);
        }


        private void timbrarsubrogacion(Pago pago, string conexion, string ConexionCfdII)
        {
            try
            {
        ...
        private void Timbrar(List<PagoDet> List_DetComplemento, int Ids_Emp, int Ids_Cd, string conexion, string ConexionCfdII)
        {
            try
            {
                ...
                string serie;
                string rfc;
                DateTime? fecha = txtFecha1.SelectedDate;

                int pagdet = 0;
                //Prmt 2 cuando se escoge la factura predeterminada de detalle timbre
                //Prmt 3 cuando se se escoge la factura desde el monitor
                ...
                for (int cont = 0; cont < List_DetComplemento.Count; cont++)
                {
                    ...
                    {
                        new CN_CapFactura().ConsultaFactura(ref Factura, ref ListaFacturaDet, conexion);
                    }
                ...
                {
                    this.ImprimirTimbres(Timbres, id_Pag, Ids_Emp, Ids_Cd, conexion, ConexionCfdII, fecha);
                }
        ...
        private void ImprimirTimbres(List<Timbrado> Timbres, int Id_pag, int Id_Emp, int Id_CD, string conexion, string ConexionCfdII, DateTime? fecha)
        {
                ...
                foreach (var Timbre in Timbres)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var Factura in Timbre.Facturas)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    saldoant = PagoFactura.Importe;
                                    saldo = 0;
                                }
                                else
                                {
                                    saldoant = PagoFactura.Importe - Pagado.Total_Pagado_Timbrado;
                        ...
                        XML_Enviar.Append(" fecha=\"" + string.Format("{0:s}", fecha) + "\"");
                    ...
                    catch (Exception ex)
                    {
                        foreach (var Factura in Timbre.Facturas)
                        {
                            int ver = 0;
                            PagoDet PagoDet = list_pagos.Where(at => at.Ref.Replace(" ", string.Empty) == Factura.Id_Fac.ToString()).Select(row => row).FirstOrDefault();
                            new CN_CapPago().BorrarComplementoPago(sesion.Id_Emp, sesion.Id_Cd_Ver, PagoDet.Id_Pag, PagoDet.Id_PagDet, folio, sesion.Emp_Cnx, ref ver);
                        }
