...
{
    public partial class FullDashboard : BaseServerPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            CurrentPath = new List<string>() { "Gestion de la Promoción", "Dashboard" }.ToArray();
            ...
            try
            {
                if (ValidarSesion())
                {
                    //this.hfIdRik = session.Id_Rik.ToString();
        ...
        //ABR30-2020 RFH - Ya no cargara Metas del Rik
            ...
            try
            {
                if (session != null)
                {
                    if (this.ddlMeses.SelectedIndex == -1 || this.ddlMeses.SelectedIndex == 0)
                    {
                        ...
                        {
                            case 3:
                                // Si es Gerente utiliza el parametros para para generar la grafica.
                                graficas.GraficaDistribucion(session.Id_Emp, session.Id_Cd_Ver, "1,2,3,4,5", Parametro_IdRik, (int?)null, (int?)null, intDdl, ref dsGraficaDistribucion, session.Emp_Cnx);
                                break;
                            case 5:
                                break;
                            case 6:
                        ...
                        if (dsGraficaDistribucion != null && dsGraficaDistribucion.Tables.Count > 0)
                        {
                            if (intDdl == 1)
                            {
