...
namespace SIANWEB.WebService.PortalRIK.GestionPromocion
{
    ...
    {
        ///     AjustaPME?IdEmp=' + IdEmp + '&IdCDI=' + IdCDI + '&IdCte=' + IdCte + '&IdCteDet=' + IdCteDet + '&IdTer=' + IdTer + '&IdSeg=' + IdSeg + '&valor
        [HttpGet]
        public eResponse<int> AjustaPME(int IdEmp, int IdCDI, int IdCte, int IdOp, int IdTer, int IdSeg, float valor)
        {
            eResponse<int> result = new eResponse<int>();
            result.Estado = 0;
            try
            {
                CN_CrmOportunidad CN = new CN_CrmOportunidad();
                result.Datos = CN.AjustaPME(IdEmp, IdCDI, IdCte, IdOp, IdTer, IdSeg, valor, Sesion);
                result.Estado = 1;
            }
            catch (Exception ex)
            {
        ...
        // 1 Maroz 2019
        [HttpGet]
        public eResponse<int> EliminarFisicamente(int Id_Op, int Eliminar, string ClaveConfirmacion)
        {
            eResponse<int> result = new eResponse<int>();
            result.Estado = 0;
            try
            {
                CN_CrmOportunidad CN = new CN_CrmOportunidad();
                result.Datos = CN.EliminarFisicamente(Sesion.Id_Emp, Sesion.Id_Cd, Id_Op, ClaveConfirmacion, Sesion);
                result.Estado = 1;
            }
            catch (Exception ex)
            {
