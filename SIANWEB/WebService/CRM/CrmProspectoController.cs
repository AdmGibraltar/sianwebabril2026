        ...
        : BaseWebAPIController
    {
        ...
        public IEnumerable<CrmProspecto> Get(int idEmp, int idCd, int idRik)
        {
            ...
            try
            {
                ...
                resultado = cnCrmProspecto.spObtenerProspectos(idEmp, idCd, idRik, Sesion.Emp_Cnx, Sesion.VersionTerritorio);
                if (resultado == null)
                {
        ...
        [HttpGet]
        public IEnumerable<eProspectos> VerificarNombreProspecto(string nombre)
        {
            var response = new CN_CrmProspecto().VerificarNombre(Sesion, nombre);
            return response;
        }
