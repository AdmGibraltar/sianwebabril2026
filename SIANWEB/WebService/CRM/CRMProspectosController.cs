...
{
    public class CRMProspectosController : ApiController
    {
        [HttpGet]
        public eResponse<List<eProspectos>> Get(
            string TextoBuscar, int PageSize, int PageIndex)
        {
            eResponse<List<eProspectos>> result = new eResponse<List<eProspectos>>();
            result.Estado = 0;
            ...
            try
            {
                CN_CrmProspecto CN = new CN_CrmProspecto();
                lst = CN.ObtenerProspecto_Page(Sesion.Id_Emp, Sesion.Id_Cd, TextoBuscar, PageSize, PageIndex, Sesion.Emp_Cnx);
                result.Datos = lst;
            }
            catch (Exception ex)
            {
