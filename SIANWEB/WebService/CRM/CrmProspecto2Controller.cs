...
namespace SIANWEB.WebService.PortalRIK.GestionPromocion
{
    ...
    public class CrmProspecto2Controller : BaseWebAPIController
    {

        [HttpGet]
        public eResponse<List<CrmProspecto>> Get(int idEmp, int idCd, int idRik)
        {
            CN_CrmProspecto cnCrmProspecto = new CN_CrmProspecto();
            eResponse<List<CrmProspecto>> result = new eResponse<List<CrmProspecto>>();
            List<CrmProspecto> lst = new List<CrmProspecto>();
            result.Estado = 0;
            try
            {
                lst = cnCrmProspecto.spObtenerProspectos(idEmp, idCd, idRik, Sesion.Emp_Cnx, Sesion.VersionTerritorio);
                result.Datos = lst;
                result.Estado = 1;
            }
            catch (Exception ex)
        ...
        [HttpGet]
        public eResponse<CrmProspecto> Get(int Id_CrmProspecto)
        {
            CN_CrmProspecto cnCrmProspecto = new CN_CrmProspecto();
            eResponse<CrmProspecto> result = new eResponse<CrmProspecto>();
            CrmProspecto Obj = new CrmProspecto();
            result.Estado = 0;
            try
            {
                Obj = cnCrmProspecto.spObtenerProspecto(Sesion.Id_Emp, Sesion.Id_Cd, Id_CrmProspecto, Sesion.Emp_Cnx);
                result.Datos = Obj;
                result.Estado = 1;
            }
            catch (Exception ex)
        ...
        public IEnumerable<CrmProspecto> Get(int idEmp, int idCd, int idRik)
        {
            ...
            try
            {
                ...
                resultado = cnCrmProspecto.spObtenerProspectos(idEmp, idCd, idRik, Sesion.Emp_Cnx);
                if (resultado == null)
                {
