...
namespace SIANWEB.WebService.PortalRIK.GestionPromocion.Proyectos
{
        ...
        : BaseWebAPIController
    {
                ...
                var t = Task<HttpResponseMessage>.Factory.StartNew(() =>
                {
                    ...
                    try
                    {
                        using (IBusinessTransaction ibt = CN_FabricaTransaccionNegocios.Default(Sesion))
                        {
                            ...
                            var proyectos = cnCrmOportunidad.ObtenerSoloCRMIIPorRik_Ver2(Sesion, ibt).Select(p => {

                                CrmPromociones promocion = new CrmPromociones();

                                promocion.Ids = p.Id_Op;
                                promocion.Id = p.Id_Op;
                                promocion.Id_Cte = p.Id_Cte.Value;
                                promocion.Cds = p.Id_Cd;
                                promocion.Id_Apl = p.Id_Apl.Value; // RFH 
                                promocion.Id_Area = p.ID_Area.Value;
                                promocion.Id_Sol = p.Id_Sol.Value;
                                promocion.Id_Seg = p.Id_Seg.Value;
                                promocion.IdUen = p.Id_Uen.Value;
                                promocion.Representante = p.Id_Usu.Value;
                                promocion.NombreCte = p.CatCliente.Cte_NomComercial;
                                promocion.Id_Ter = p.Id_Ter.Value;

                                try
                                {
                                    promocion.Segmento = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.Id_Seg.Value : p.Id_Seg.Value;
                                }
                                catch (Exception e)
                                {
                                    promocion.Segmento = 0;
                                }

                                promocion.Cli_VPObservado = Convert.ToDouble(p.MontoProyecto.Value);
                                promocion.Descripcion = p.CrmOportunidadesAplicacion != null ? string.Format("{0}/{1}/{2}/{3}/{4}", p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.CatUEN.Uen_Descripcion, p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.Seg_Descripcion, p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.Area_Descripcion, p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.Sol_Descripcion, p.CrmOportunidadesAplicacion.CatAplicacion.Apl_Descripcion) : string.Empty;
                                promocion.Analisis = p.Analisis != null ? p.Analisis.Value.ToShortDateString() : string.Empty;
                                promocion.Presentacion = p.Presentacion != null ? p.Presentacion.Value.ToShortDateString() : string.Empty;
                                promocion.Negociacion = p.Negociacion != null ? p.Negociacion.Value.ToShortDateString() : string.Empty;
                                promocion.Cierre = p.Cierre != null ? p.Cierre.Value.ToShortDateString() : string.Empty;
                                promocion.Cancelacion = p.Cancelacion;
                                promocion.FechaCancelacion = p.FechaCancelacion != null ? p.FechaCancelacion.Value.ToShortDateString() : string.Empty;
                                promocion.Avances = p.Avances != null ? p.Avances.Value : 0;
                                promocion.Estatus = p.Estatus != null ? p.Estatus.Value : 0;
                                promocion.VentaMensual = p.VentaMensual != null ? Convert.ToDouble(p.VentaMensual.Value) : 0.0D;
                                promocion.Id_CrmProspecto = p.Id_CrmProspecto != null ? p.Id_CrmProspecto.Value : 0;
                                promocion.VentaNoRepetitiva = p.VentaNoRepetitiva;
                                promocion.Id_Uen = p.Id_Uen.Value;
                                promocion.EnValuacion = p.CrmOp_EnValuacion;
                                promocion.Cliente = p.Id_Cte.Value;
                                promocion.Area = p.ID_Area.Value;
                                promocion.Solucion = p.Id_Sol.Value;
                                promocion.Seg_Descripcion = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.Seg_Descripcion : string.Empty;
                                promocion.Area_Descripcion = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.Area_Descripcion : string.Empty;
                                promocion.Sol_Descripcion = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.Sol_Descripcion : string.Empty;
                                promocion.Dim_Id_Uen = p.Dim_Id_Uen;
                                promocion.Dim_Cantidad = p.Dim_Cantidad;
                                promocion.Dim_Descripcion = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.Seg_Unidades : string.Empty;
                                promocion.ValorPotencialTeorico = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.Seg_ValUniDim.Value * Convert.ToDouble(p.Dim_Cantidad != null ? p.Dim_Cantidad.Value : 0) : 0;
                                promocion.VentaPromedioMensualEsperada = p.CrmOp_VPM != null ? p.CrmOp_VPM.Value : 0;
                                promocion.Uen_Descrip = p.CrmOportunidadesAplicacion != null ? p.CrmOportunidadesAplicacion.CatAplicacion.CatSolucion.CatArea.CatSegmento.CatUEN.Uen_Descripcion : string.Empty;
                                promocion.Ter_Nombre = p.CatTerritorio != null ? p.CatTerritorio.Ter_Nombre : string.Empty;
                                promocion.CrmValuacionOportunidades = cnCrmValuacionOportunidades.ObtenerPorProyecto(Sesion, p.Id_Cte.Value, p.Id_Op, ibt);
                                ...
                                switch (0)
                                {
                                    case 0:
                                        ...
                                        var proyectoValuaciones = cdCrmValuacionOportunidades.ConsultarPorProyecto(
                                            Sesion.Id_Emp, Sesion.Id_Cd, promocion.Id_Cte, promocion.Id, ibt.DataContext);
                                        ...
                                        else
                                        {
                                            ...
                                            Obj = CD_co.Consulta_ResultadoValuacion_Ver2(
                                            promocion.Id_Cte, promocion.Id, proyectoValuaciones.Id_Val, Sesion, ibt);
        ...
        [HttpGet]
        public eResponse<List<eCrmProyecto>> spCRM_SelProyectos(int Id_Rik, int Id_Cte, int Agrupado)
        {
            eResponse<List<eCrmProyecto>> result = new eResponse<List<eCrmProyecto>>();
            result.Estado = 0;

            List<eCrmProyecto> lst = new List<eCrmProyecto>();
            try
            {
                CN_CrmCapValProyecto CN = new CN_CrmCapValProyecto();
                lst = CN.GetCrmProyecto(Sesion.Id_Emp, Sesion.Id_Cd, Id_Rik, Id_Cte, Agrupado, Sesion.Emp_Cnx);
        ...
        [HttpPost]
        public HttpResponseMessage UpdateTerritorioOportunidad([FromBody] WebAPI.Models.Post.UpdateTerritorioCrm data)
        {
            try
            {
                string msgError = "";
                ...
                int Id_CrmProspectoNew = CN.ActualizarTerritorioOpo(Sesion.Id_Cd, data.Id_Cte, data.Id_Terr, 714, Sesion.Emp_Cnx);

                if (Id_CrmProspectoNew <= 0)
                {
                    msgError = "No se pudo actualizar el territorio";
                }

                return Request.CreateResponse(System.Net.HttpStatusCode.OK, msgError);
