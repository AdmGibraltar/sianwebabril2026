...
namespace SIANWEB
{
    public partial class CapRemisiones : System.Web.UI.Page
    {
        ...
        private string uniqueKey
        {
            get
            {
                // Si sessionKey.Value no está inicializado, inicializarlo
                if (sessionKey == null || string.IsNullOrEmpty(sessionKey.Value))
                {
                    sessionKey.Value = Guid.NewGuid().ToString();
                }
        ...
        private bool _PermisoGuardar { get { if (Session["Sesion" + Session.SessionID] == null) { return false; } return (bool)Session["PermisoGuardar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]]; } set { Session["PermisoGuardar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]] = value; } }
        private bool _PermisoModificar { get { if (Session["Sesion" + Session.SessionID] == null) { return false; } return (bool)Session["PermisoModificar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]]; } set { Session["PermisoModificar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]] = value; } }
        private bool _PermisoEliminar { get { if (Session["Sesion" + Session.SessionID] == null) { return false; } return (bool)Session["PermisoEliminar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]]; } set { Session["PermisoEliminar" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]] = value; } }
        private bool _PermisoImprimir { get { if (Session["Sesion" + Session.SessionID] == null) { return false; } return (bool)Session["PermisoImprimir" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]]; } set { Session["PermisoImprimir" + Session.SessionID + uniqueKey + Page.Request.Url.ToString().Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries)[0]] = value; } }
        private int _Accion
        {
            get
            {
        ...
        {
            set { Session["Tm_ReqSpoREM" + Session.SessionID + uniqueKey] = value; }
            get { return (bool)Session["Tm_ReqSpoREM" + Session.SessionID + uniqueKey]; }
        }
        ...
        public DataTable dt_detalles
        {
            get
            {
                return Session["dt_DetallesRem" + Session.SessionID + uniqueKey] as DataTable;
            }
            set
            {
                Session["dt_DetallesRem" + Session.SessionID + uniqueKey] = value;
            }
        }
        public DataTable dt_cuenta
        {
            get
            {
                return Session["dt_cuentaRem" + Session.SessionID + uniqueKey] as DataTable;
            }
            set
            {
                Session["dt_cuentaRem" + Session.SessionID + uniqueKey] = value;
            }
        }
        public DataTable dt_cuentaOriginal
        {
            get
            {
                return Session["dt_cuentaOriginalRem" + Session.SessionID + uniqueKey] as DataTable;
            }
            set
            {
                Session["dt_cuentaOriginalRem" + Session.SessionID + uniqueKey] = value;
            }
        }
        public DataTable dt_cuentaPedido
        {
            get
            {
                return Session["dt_cuentaPedidoRem" + Session.SessionID + uniqueKey] as DataTable;
            }
            set
            {
                Session["dt_cuentaPedidoRem" + Session.SessionID + uniqueKey] = value;
            }
        }
        ...
        {
            set { Session["id_detalleREM" + Session.SessionID + uniqueKey] = value; }
            get { int? st = (int?)Session["id_detalleREM" + Session.SessionID + uniqueKey]; return st == null ? 0 : (int)st; }
        }
        //static int Id_RemDet_A = -1; //id de la partida que se va actualizar
        private int Id_RemDet_A
        {
            set { Session["Id_RemDet_AREM" + Session.SessionID + uniqueKey] = value; }
            get { int? st = (int?)Session["Id_RemDet_AREM" + Session.SessionID + uniqueKey]; return st == null ? -1 : (int)st; }
        }
        //static int cantidad_A = 0; //cantidad de la partida que se va actualizar
        private int cantidad_A
        {
            set { Session["cantidad_AREM" + Session.SessionID + uniqueKey] = value; }
            get { int? st = (int?)Session["cantidad_AREM" + Session.SessionID + uniqueKey]; return st == null ? 0 : (int)st; }
        }
        //static double costo_A = -1; //costo de la partida que se va actualizar
        private double costo_A
        {
            set { Session["costo_AREM" + Session.SessionID + uniqueKey] = value; }
            get { double? st = (double?)Session["costo_AREM" + Session.SessionID + uniqueKey]; return st == null ? -1 : (double)st; }
        }
        //static long Id_Prd_A;
        private long Id_Prd_A
        {
            set { Session["Id_Prd_AREM" + Session.SessionID + uniqueKey] = value; }
            get { return (int)Session["Id_Prd_AREM" + Session.SessionID + uniqueKey]; }
        }
        //static bool edicionRemisionDePedido;
        private bool edicionRemisionDePedido
        {
            set { Session["edicionRemisionDePedidoRem" + Session.SessionID + uniqueKey] = value; }
            get { return (bool)Session["edicionRemisionDePedidoRem" + Session.SessionID + uniqueKey]; }
        }
        /// <summary>
        ...
        {
            set { Session["tipoDeMovimientoREM" + Session.SessionID + uniqueKey] = value; }
            get { int? st = (int?)Session["tipoDeMovimientoREM" + Session.SessionID + uniqueKey]; return st == null ? 0 : (int)st; }
        }
        //static bool remisionDePedido;
        //static bool hayProductosNoSpo;
        private bool hayProductosNoSpo
        {
            set { Session["hayProductosNoSpoREM" + Session.SessionID + uniqueKey] = value; }
            get { return (bool)Session["hayProductosNoSpoREM" + Session.SessionID + uniqueKey]; }
        }
        //static int Id_Rem_Actualiza;
        private int Id_Rem_Actualiza
        {
            set { Session["Id_Rem_ActualizaREM" + Session.SessionID + uniqueKey] = value; }
            get { return (int)Session["Id_Rem_ActualizaREM" + Session.SessionID + uniqueKey]; }
        }


        private int Id_CuentaNacional
        {
            set { Session["Id_CuentaNacional" + Session.SessionID + uniqueKey] = value; }
            get { return (int)Session["Id_CuentaNacional" + Session.SessionID + uniqueKey]; }
        }
        ...
        {
            set { Session["NumCuentaContNacional" + Session.SessionID + uniqueKey] = value; }
            get { return (int)Session["NumCuentaContNacional" + Session.SessionID + uniqueKey]; }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (sesion == null)
                {
                    ...
                    Session["dir" + Session.SessionID + uniqueKey] = pag[pag.Length - 1];
                    Response.Redirect("login.aspx", false);
                }*/
                //construimos una clave unica para evitar que se guarden remisiones duplicadas  
                //esta sesion se mantiene sin el guild ya que es la general 
        ...
        protected void cmbTipoMovimiento_SelectedIndexChanged(object sender, Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                if (tipo == 61)
                {
                    ...
                    prv.Id_Emp = sesion.Id_Emp;
                    prv.Id_Cd = sesion.Id_Cd_Ver;
                    prv.Id_Terr = Request.QueryString["ter"] == null ? (int?)null : Convert.ToInt32(Request.QueryString["ter"]);
                    clsCatProveedores.ConsultaClientes(prv, sesion.Emp_Cnx, ref List, prv.Id_Cd, null);


                    txtClienteId.Text = List[0].IdStr.ToString();
                    txtCliente.Text = List[0].Descripcion;
                    cmbCliente_indiceCambia("");
                }

                if ((tipo == 60 || tipo == 82) && txtTotal.Value >= Convert.ToDouble(HiddenField2.Value))
                {
        ...
        protected bool ValidaClientesDirectos()
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            bool ret = true;
            ...
            new CN_CatCliente().ConsultaClientesDirectosSP(ref cliente, ref TieneClientesDirectosSP, sesion.Emp_Cnx);
            if (int.Parse(cmbTipoMovimiento.SelectedValue) == 60)
            {

                if (TieneClientesDirectosSP == 1 && cliente.Id_Cte != -1)
                {
                    LimpiarCampos1();
                    txtClienteId.Text = "";
                    txtCliente.Text = "";
                    AlertaFocus("Este cliente solo puede hacer mercancia en comodatos con el tipo de movimiento 82", txtClienteId.ClientID);
                    ret = false;
                    return false;
                }
            ...
            if (int.Parse(cmbTipoMovimiento.SelectedValue) == 82)
            {

                if (TieneClientesDirectosSP == 0 && cliente.Id_Cte != -1)
                {
                    LimpiarCampos1();
                    txtClienteId.Text = "";
                    txtCliente.Text = "";
                    AlertaFocus("Este cliente solo puede hacer mercancia en comodatos con el tipo de movimiento 60", txtClienteId.ClientID);
                    ret = false;
                    return false;
                }
        ...
        protected bool ValidaClienteCuentaNacional()
        {
            ...
            if (int.Parse(cmbTipoMovimiento.SelectedValue) == 80)
            {
                ...
                var idTdm = Request.QueryString["tdm"];
                CN_CatCNac_OrdenCompra cnOC = new CN_CatCNac_OrdenCompra();
                var res = cnOC.ValidaClienteOC80(cliente.Id_Emp, cliente.Id_Cd, cliente.Id_Cte.Value);

                if (res == null && cliente.Id_Cte != -1 && idTdm == "1")
                {
                    LimpiarCampos1();
                    txtClienteId.Text = "";
                    txtCliente.Text = "";
                    AlertaFocus("Este cliente no se le puede realizar la rem 80", txtClienteId.ClientID);
                    ret = false;
                    return false;
                }
        ...
        protected bool ValidaSucursal()
        {
            ...
            if (cliente.Id_Cte != -1)
            {
                ...
                if (int.Parse(cmbTipoMovimiento.SelectedValue) != 54 && int.Parse(cmbTipoMovimiento.SelectedValue) != 65 && int.Parse(cmbTipoMovimiento.SelectedValue) != 61)
                {
        ...
        protected void txtClienteId_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                ErrorManager();
                if (txtTipoId.Text == "61")
                {
                    Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
                    Alerta("Este movimiento solo se puede realizar directamente a la sucursal que la procesa.");
                    txtClienteId.Text = sesion.Id_Cd_Ver.ToString();
                    return;
                }
        ...
        void ValidarCliente_TipoMovimiento82()
        {
        ...
        protected void rgDetalles_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "InitInsert")
            {
                ...
                if (cmbTipoMovimiento.SelectedValue.ToString() == "80")
                {
                    e.Canceled = true;
                    return;
                }
            ...
            if (e.CommandName == "Edit")
            {
                ...
                Id_Prd_A = Int64.Parse((rgDetalles.MasterTableView.Items[e.Item.ItemIndex]["Id_Prd"].FindControl("ProdLabel") as Label).Text);
        ...
        protected void rgDetalles_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                try
                {
                    prdActivo = Convert.ToInt32(dataItem["Prd_Activo"].Text);
                }
                catch (Exception ex) { }
                ...
                if (prdActivo == 2)
                {
                    ...
                    Alerta("Hay productos inactivos en el detalle de la remisión");
                }
            }
        ...
        protected void rgDetalles_InsertCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            try
            {
                ...
                long Id_Prd = Convert.ToInt64(TxtProducto.Value);
                ...
                //cmbTipoMovimiento
                if (ConfigurationManager.AppSettings["PermitePrecios0Remision"].ToString() != "S")
                {
                ...
                //cuando se carga un pedido y una de las partidas tiene 0 y no la carga en el detalle 
                //si agregaba un producto nuevo no ponia el id detalle correcto ya que se repetia 
                //JFCV modifique ahora saca un max de los ids de las partidas para que no marque error 
                // 18 junio 2019 
                int maxrenglon = 0;
                foreach (DataRow dr in dt_detalles.Rows)
                {
                    int accountLevel = Convert.ToInt32(dr["Id_RemDet"].ToString());
                    maxrenglon = Math.Max(maxrenglon, accountLevel);
                }
                id_detalle = maxrenglon;
                ...
                CalcularTotales();
                if (txtTipoId.Value == 60 || txtTipoId.Value == 82)
                {
        ...
        protected void rgDetalles_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            try
            {
                ...
                DataRow[] editable_dr;
                long Id_Prd = Int64.Parse((rgDetalles.MasterTableView.Items[e.Item.ItemIndex]["Id_Prd"].FindControl("ProdLabel") as Label).Text);
                if (dt_cuenta.Select("Id_Prd='" + Id_Prd + "'").Length > 0)
                {
                    editable_dr = dt_cuenta.Select("Id_Prd='" + Id_Prd + "'");
                    editable_dr[0].BeginEdit();
        ...
        protected void cmbProductoDet_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                clienteProd.Id_Prd = Convert.ToInt64(estecombo.Value.HasValue ? estecombo.Value : -1);
                ...
                try
                {
                    new CN_CatProducto().ConsultaProducto(ref prd, session.Emp_Cnx, session.Id_Emp, session.Id_Cd_Ver, Convert.ToInt64(estecombo.Value.HasValue ? estecombo.Value : -1), 2);
                }
                catch (Exception ex)
                {
                    (j.FindControl("DescripcionTextBox") as RadTextBox).Text = "";
                ...
                if (prd.Prd_Activo == 2 && prd.Prd_InvFinal <= 0)
                {
                    Alerta("Producto inactivo, reemplazalo con otra alternativa/consulta con el area operativa/CEDIS");
                    (j.FindControl("DescripcionTextBox") as RadTextBox).Text = "";
                    (j.FindControl("RadNumericTextBoxPrecio") as RadNumericTextBox).Text = "";
                    (j.FindControl("RadNumericTextBoxCantidad") as RadNumericTextBox).Text = "";
                    return;
        ...
        protected void txtCantidad_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                Int64 prd = Convert.ToInt64((Txtcantidad.Parent.Parent.FindControl("RadNumericTextBox1") as RadNumericTextBox).Value);
                ...
                if (tipoDeMovimiento == 2 && (txtTipoId.Value == 60 || txtTipoId.Value == 82))
                {
        ...
        protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            try
            {
                ...
                {
                    case "cliente":
                        txtClienteId.Text = Session["Id_Buscar" + Session.SessionID + uniqueKey].ToString();
                        txtClienteId_TextChanged(null, null);
                        break;
                    case "precio":
                        (producto as RadNumericTextBox).DbValue = Session["Id_Buscar" + Session.SessionID + uniqueKey];
                    ...
                    case "direccion":
                        ...
                        string[] id = Session["Id_Buscar" + Session.SessionID + uniqueKey].ToString().Split('-');


                        cliente.Id_CteDirEntrega = Int32.Parse(id[0]) - 1;
                        cliente.Id_Cte = Int32.Parse(Session["Descripcion_Buscar" + Session.SessionID + uniqueKey].ToString());
        ...
        protected void ImgBuscarDireccionEntrega_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                RadAjaxManager1.ResponseScripts.Add("AbrirBuscarDireccionEntrega('" + sessionKey.Value + "');");
            }
            catch (Exception ex)
            {
                ErrorManager(ex, new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        protected void imgAceptar_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                ErrorManager();
                RadAjaxManager1.ResponseScripts.Add("popup('" + sessionKey.Value + "');");
            }
            catch (Exception ex)
            {
                ErrorManager(ex, new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        private void Imprimir(Remision Rem)
        {
            try
            {
                ...
                GridItem gi = Session["Pedido" + Session.SessionID + uniqueKey] as GridItem;
                ...
                if (bRemisionValida)
                {
                    ...
                    if (remision.Id_Tm == 60 || remision.Id_Tm == 82)
                    {
                        instance = typeof(LibreriaReportes.RemisionImpresion);
                    }
                    else
                    {
                        instance = typeof(LibreriaReportes.RemisionImpresionSinContrato);
                    }
                    Session["InternParameter_Values" + Session.SessionID + uniqueKey + HF_ClvPag.Value] = ALValorParametrosInternos;
                    Session["assembly" + Session.SessionID + uniqueKey + HF_ClvPag.Value] = instance.AssemblyQualifiedName;
            ...
            if (RadUpload1.UploadedFiles.Count > 0)
            {
                foreach (Telerik.Web.UI.UploadedFile postedFile in RadUpload1.UploadedFiles)
                {
                    ...
                    if (!Object.Equals(postedFile, null))
                    {
                        ...
                        {
                            if (postedFile.GetExtension() != ".pdf")
                            {
                                UploadedFileLog.InnerHtml += string.Format("<strong/><b>NO ES UN PDF</strong>");
                                Alerta("EL ARCHIVO DEBE SER FORMATO PDF");
                                return;
                            }
                            UploadedFileLog.InnerHtml += string.Format("<br /><b>Filename</b>: {0}", txtFolio.Text + ".pdf");
                            UploadedFileLog.InnerHtml += string.Format("<br /><b>File Size</b>: {0} bytes", postedFile.ContentLength);
                            string fn = System.IO.Path.GetFileName(txtFolio.Text + ".pdf");
                            string SaveLocation = Server.MapPath("AdjuntosOrdenesDeCompra") + "\\" + fn;

                            try
                            {
                                postedFile.SaveAs(SaveLocation);
                                UploadedFileLog.InnerHtml += string.Format("<br /><b>ARCHIVO GUARDADO</b>");

                            }
                            catch (Exception ex)
                            {
                                UploadedFileLog.InnerHtml += string.Format("<br /><b>ERROR </b>" + ex.Message);
                                Alerta(ex.Message);
        ...
        private void cmbCliente_indiceCambia(string pedrem)
        {
            try
            {
                LimpiarCampos1();
                ...
                if (pedrem.ToString() != "")
                {
                    ...
                    Ped.Id_Ped = Convert.ToInt32(pedrem.ToString());

                    CN.ConsultaPedidosAutorizados(Ped, sesion.Emp_Cnx, ref Verificador);
                }
        ...
        private void LimpiarCampos1()
        {
            ...
            txtRepresentanteStr.Text = "";
            //jfcv 22sep22
            Session["lAurizacionPrecios" + Session.SessionID + uniqueKey] = null;
        }
        private void crearDT()
        {
            ...
            dt_detalles.Columns.Add("Prd_Activo");
        }
        ...
        private void Guardar(bool gen_contrato)
        {
            try
            {
                ...
                if (txtTipoId.Text == "80" && txtPedido.Text.Trim().Length == 0)
                {
                    Alerta("No se Permite Remisionar, Se requiere que se Capture un Pedido de Orden de Compra Centralizado");
                    return;
                }
                if (txtTipoId.Text == "62" || txtTipoId.Text == "63")
                {
                    if (txtPedido.Text == "" && ConfigurationManager.AppSettings["ValidaPedidos"].ToString() == "S")
                    {

                        String SiValida = "S";

                        if (ConfigurationManager.AppSettings["ValidaPedidosClientes"].ToString() == "S")
                        {
                            new CN_CatCliente().ConsultaClienteValidoFacturaSinPedido(Convert.ToInt32(txtClienteId.Value.HasValue ? txtClienteId.Value.Value : -1), sesion, ref SiValida);
                        }
                        ...
                        if (SiValida == "S")
                        {
                            ...
                            if (txtTerritorioId.Text.Trim().Length >= 8)
                            {

                                if (txtTerritorioId.Text.Trim().Substring(1, 2) == "06")
                                {

                                }
                                else
                                {
                                    Alerta("No se permite remisionar sin pedido o solo que el territorio sea tipo mostrador");
                                }
                            }
                            else
                            {
                                if (txtTerritorioId.Text.Trim().Substring(0, 1) == "6")
                                {
                                }
                                else
                                {
                ...
                { //solo se checa inventario si NO es Pedido
                       ...
                       )
                    {
                        ...
                        int invFinal = -1;
                        int asignado = -1;
                        new CN_CapEntradaSalida().ConsultarDisponible(sesion, Int64.Parse(row["Id_Prd"].ToString()), ref disponible, ref invFinal, ref asignado);
                ...
                if ((txtTipoId.Value == 60 || txtTipoId.Value == 82) && valuacion.Visible)
                {
                ...
                //RBM Dic 2020
                //Se agrega validación para el movimiento 61 solo puede ser creado para la sucursal que lo realiza
                //Inicio
                if (txtTipoId.Text == "61" && txtTipoId.Text != sesion.Id_Cd_Ver.ToString())
                {
                    Alerta("Se cambió el número de cliente debido a que no coincidía el Id con la descripción y esta remisión es exclusiva para la sucursal que la crea, ella misma es el cliente..");
                    txtClienteId.Text = sesion.Id_Cd_Ver.ToString();
                }
                //Fin
                ...
                if (Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey] != null)
                {
                    //if (Session["RemEspecialGuardada" + Session.SessionID + uniqueKey].ToString() == "1")
                    //{
                    double totalEspecial = 0;
                    foreach (RemisionDet ncd in (List<RemisionDet>)Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey])
                    {
                    ...
                    foreach (RemisionDet f in (List<RemisionDet>)Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey])
                    {
                        bEncontrados = false;
                ...
                // configuración 954 conf de remisiones  
                ...
                try
                {
                    ...
                    SysC = CN.spSysConfiguracionById(sesion.Id_Emp, sesion.Id_Cd, 954, sesion.Emp_Cnx);
                    int iTmp = 0;
                ...
                if (GLOBAL_ValidaPrecioMinimoRik == 1)
                {
                    ...
                    int procesar = 1;
                    if (Session["Id_FacPrec" + Session.SessionID + uniqueKey] != null)
                    {
                        string folior = Session["Id_FacPrec" + Session.SessionID + uniqueKey].ToString();
                    ...
                    if (procesar == 1)
                    {
                        ...
                        {
                            for (int x = 0; x < dt_detalles.Rows.Count; x++)
                            {


                                double PrecioIngresado = Convert.ToDouble(dt_detalles.Rows[x]["Precio"]);
                                //double impore =  dt.Rows[x]["Prd_Precio"].ToString().Trim()!= DBNull.Value  ? (double)(dt.Rows[x]["Prd_Precio"]) : 0;

                                if (Convert.ToInt64(dt_detalles.Rows[x]["Id_Prd"]) <= 999999999999)
                                {
                                    //JFCV 26 enero 2021 Valido si el precio de venta es menor al minimo
                                    ...
                                    alertaaut.Id_Cte = Convert.ToInt32(txtClienteId.Value.ToString().Trim() != "" ? txtClienteId.Value.ToString().Trim() : "-1");
                                    alertaaut.Cte_NomComercial = txtCliente.Text.Trim() != "" ? txtCliente.Text.Trim() : "";
                                    alertaaut.Id_Prd = Convert.ToInt64(dt_detalles.Rows[x]["Id_Prd"]);
                                    alertaaut.IdRepresentante = Convert.ToInt32(txtRepresentante.Value.HasValue ? txtRepresentante.Value.Value : -1);
                                    alertaaut.Id_Ter = Convert.ToInt32(txtTerritorioId.Value.HasValue ? txtTerritorioId.Value.Value : -1);
                                    double Preciodefactura = Convert.ToDouble(dt_detalles.Rows[x]["Precio"]);
                                    ...
                                    if (alertaautdet != null && alertaautdet.Precio_MinimoRik > 0)
                                    {
                                        ...
                                        if (Math.Round(Preciodefactura, 3) < alertaautdet.PrecioObjetivo)
                                        {
                                            ...
                                            pasoalertaautdet.Id_Cte = Convert.ToInt32(txtClienteId.Value.ToString().Trim() != "" ? txtClienteId.Value.ToString().Trim() : "-1");
                                            alertaaut.Cte_NomComercial = txtCliente.Text.Trim() != "" ? txtCliente.Text.Trim().Trim() : "";
                                            pasoalertaautdet.IdRepresentante = Convert.ToInt32(txtRepresentante.Value.HasValue ? txtRepresentante.Value.Value : -1);
                                            pasoalertaautdet.Id_Ter = Convert.ToInt32(txtTerritorioId.Value.HasValue ? txtTerritorioId.Value.Value : -1);
                                            ...
                                            pasoalertaautdet.PrecioLista = alertaautdet.PrecioLista;
                                            pasoalertaautdet.Cantidad = Convert.ToInt32(dt_detalles.Rows[x]["Cantidad"]);
                                            ...
                                            pasoalertaautdet.Prd_Descripcion = Convert.ToString(dt_detalles.Rows[x]["Descripcion"]);
                                            pasoalertaautdet.Cte_NomComercial = txtCliente.Text.Trim() != "" ? txtCliente.Text.Trim().Trim() : "";
                                            pasoalertaautdet.TipoAutorizacion = 3; // remisión
                                    ...
                                    //JFCV 26 enero 2021 fin Valido si el precio de venta es menor al minimo
                        ...
                        Session["ProdsAutorizacion" + Session.SessionID + uniqueKey] = null;
                        Session["Id_FacPrec" + Session.SessionID + uniqueKey] = null;
                        Session["lAurizacionPrecios" + Session.SessionID + uniqueKey] = null;


                        if (ProductosAlerta.Count > 0 && lalertasautdet.Count > 0)
                        {
                            Session["ProdsAutorizacion" + Session.SessionID + uniqueKey] = ProductosAlerta;
                            Session["lAurizacionPrecios" + Session.SessionID + uniqueKey] = lalertasautdet;
                            Session["Id_FacPrec" + Session.SessionID + uniqueKey] = txtFolio.Value.ToString().Trim();

                            RadAjaxManager1.ResponseScripts.Add("AbrirVentana_AlertaAutorizacionPrecios(); return false;");
                ...
                #endregion fin validar precios conevnio 

                //JFCV convenios valida precios de alertas 10 Feb 2021 Fin
                int verificador = -1;
                int Id_Rem = 0;
                ...
                if (txtTipoId.Value == 54 && txtClienteId.Value == 100)
                {
                    try
                    {
                        new CN_CapRemision().GuardarRemision(remision, detalles, sesion, ref verificador, tipoDeMovimiento == 2, gen_contrato, ref Id_Rem, ref tipoMovimento, ref mensaje, Convert.ToString(Request.QueryString["strpicking"]), ConfigurationManager.AppSettings["PermitePrecios0Remision"].ToString());
                        EnviarCorreo(Id_Rem);
                ...
                {//SI GUARDÓ BIEN LA REMISION:
                    ...
                    if (Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey] != null)
                    {
                        if (Session["RemEspecialGuardada" + Session.SessionID + uniqueKey].ToString() == "1") //guarda solo si hizo clic en guardar en pantalla de RemisionEspecial.
                        {
                            ...
                            List<RemisionDet> listaProductosRemisionEspecial = (List<RemisionDet>)Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey];
        ...
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey] = null;
            Session["RemEspecialGuardada" + Session.SessionID + uniqueKey] = "0";
            ...
            switch (Request.QueryString["tdm"])
            {
                case "1":
                    ...
                    RadComboBoxItem items = cmbTipoMovimiento.FindItemByValue("80");
                    items.Remove();
                    break;
                ...
                case "2":
                    ...
                    if (remision.Id_Tm != 80)
                    {
                        RadComboBoxItem item = cmbTipoMovimiento.FindItemByValue("80");
                        item.Remove();
                    }
                    ...
                    foreach (RemisionDet detalle in detalles)
                    {
                        meterPedido_aDTs(detalle.Id_RemDet, detalle.Id_Ter == null ? -1 : int.Parse(detalle.Id_Ter.ToString()),
                            detalle.Id_Prd, detalle.Prd_Descripcion, detalle.Rem_Cant, detalle.Rem_Precio == null ? 0 : double.Parse(detalle.Rem_Precio.ToString()),
                                        detalle.Rem_Cant * detalle.Rem_Precio, detalle.Ter_Nombre, detalle.Producto.Prd_Activo);
                    }
                    CalcularTotales();
                ...
                case "3":
                    ...
                    else
                    {
                        ...
                        if (pedido.Ped_Tipo > 0)
                        {


                            this.txtTipoId.Value = 63;
                            this.txtTipoId.Enabled = false;
                            this.cmbTipoMovimiento.SelectedValue = "63";
                            this.cmbTipoMovimiento.Text = "PENDIENTE POR FACTURAR";
                            this.cmbTipoMovimiento.Enabled = false;
                        }
                        ...
                        if (bTieneAsignado)
                        {
                            ...
                            cmbCliente_indiceCambia(PedRem.ToString());
                            ...
                            foreach (DataRow row in tabla.Rows)
                            {
                                meterPedido_aDTs(int.Parse(row["Id_PedDet"].ToString()), int.Parse(row["Id_Ter"].ToString()), Int64.Parse(row["Id_Prd"].ToString()),
                                                    row["Prd_Descripcion"].ToString(), int.Parse(row["Prd_Cantidad"].ToString()), double.Parse(row["Prd_Precio"].ToString()),
                                                    double.Parse(row["Prd_Importe"].ToString()), row["Ter_Nombre"].ToString(), 1);
                            }
                            //Sacar copia de los DT originales para compararlo y no pasar de esa cantidad                        
                            dt_cuentaOriginal = dt_cuenta.Clone();
                            dt_cuentaOriginal.Merge(dt_cuenta);
                    ...
                    //Edsg 05072019 valida si es un pedido de mov 80

                    CN_CatCNac_OrdenCompra cnOC = new CN_CatCNac_OrdenCompra();
                    var res = cnOC.ValidaRemMov80(PedRem);

                    if (res != null)
                    {
                        this.txtTipoId.Value = 80;
                        this.txtTipoId.Enabled = false;
                        this.cmbTipoMovimiento.SelectedValue = "80";
                        this.cmbTipoMovimiento.Text = "TRANSFERENCIA DE ALMACEN POR REMISION CUENTAS NACIONALES";
                        this.cmbTipoMovimiento.Enabled = false;
        ...
        private void meterPedido_aDTs(int Id_Detalle, int territorio, long Id_Prd, string descripcion, int cantidad, double precio, double importe, string Ter_Nombre, int Prd_Activo)
        {
            try
            {
                Sesion session = (Sesion)Session["Sesion" + Session.SessionID];
                ...
                new CN_CatProducto().ConsultaProducto(ref producto, session.Emp_Cnx, session.Id_Emp, session.Id_Cd_Ver, Id_Prd, 2);
                ...
                if (cantidad > 0)
                {
                    double iva_cd = 0;
                    new CN_CatCentroDistribucion().ConsultarIva(session.Id_Emp, session.Id_Cd_Ver, ref iva_cd, session.Emp_Cnx);
                    dt_detalles.Rows.Add(new object[] { Id_Detalle, territorio, Id_Prd, descripcion, cantidad, precio, importe, Ter_Nombre, "", "", Prd_Activo });
        ...
        private void CalcularTotales()
        {
            try
            {
                ...
                txtIva.Text = IVA.ToString();
                txtTotal.Text = total.ToString();
                Session["fTotal" + Session.SessionID + uniqueKey] = total.ToString();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void meterPedido_aDT_cuentaPedido(int Id_Detalle, int territorio, long Id_Prd, string descripcion, int cantidad, double precio, double importe)
        {
            try
            { //agregar al dt
                DataRow[] editable_dr;
        ...
        private void CargarEspecial(int Id_Ncr, Sesion sesion, int Id_Cte)
        {
            ...
            if (listaProdRemisionEspecialFinal.Count > 0)
            {
                ...
                {
                    Session["ListaProductosRemisionEspecial" + Session.SessionID + uniqueKey] = listaProdRemisionEspecialFinal;
                    Session["RemEspecialGuardada" + Session.SessionID + uniqueKey] = 1;
                }
            }
            //-
        }

        public bool GuardarAdjuntos()
        {
            ...
            if (RadUpload1.UploadedFiles.Count > 0)
            {
                foreach (Telerik.Web.UI.UploadedFile postedFile in RadUpload1.UploadedFiles)
                {
                    ...
                    if (!Object.Equals(postedFile, null))
                    {
                        if (postedFile.ContentLength > 0)
                        {
                            ...
                            string fn = (txtFolio.Text + postedFile.GetExtension());


                            try
                            {
                                if (!Directory.Exists(Server.MapPath("AdjuntosOrdenesDeCompra" + "\\" + txtFolio.Text)))
                                {

                                    Directory.CreateDirectory(Server.MapPath("AdjuntosOrdenesDeCompra" + "\\" + txtFolio.Text));
                                }

                                string SaveLocation = Server.MapPath("AdjuntosOrdenesDeCompra") + "\\" + txtFolio.Text + "\\" + postedFile.FileName;
                ...
                try
                {
                    string pathname = Server.MapPath("~/AdjuntosOrdenesDeCompra/");

                    string[] filename = Directory.GetFiles(Server.MapPath("AdjuntosOrdenesDeCompra") + "\\" + txtFolio.Text + "");
                    ...
                    using (ZipFile zip = new ZipFile())
                    {
                        ...
                        zip.Save(pathname + txtFolio.Text + ".zip");
                        // zip.Save(Server.MapPath("~/zipa.zip"));

                        Directory.Delete(Server.MapPath("AdjuntosOrdenesDeCompra") + "\\" + txtFolio.Text, true);
