...
namespace SIANWEB
{
    public partial class CapFacturaAlmacenCobro : System.Web.UI.Page 
    {
        ...
        public List<FacturaAlmacenCobroDet> ListaFacturaAlmacenCobro
        {
            get
            {
                if (!string.IsNullOrEmpty(Page.Request.QueryString["IdPag"]))
                {
                    return (List<FacturaAlmacenCobroDet>)Session[Session.SessionID + "ListaFacturaAlmacenCobroDet" + Page.Request.QueryString["IdPag"].ToString()];
                }
                else
                {
                    return (List<FacturaAlmacenCobroDet>)Session[Session.SessionID + "ListaFacturaAlmacenCobroDet"];
                }
            }
            set
            {
                if (!string.IsNullOrEmpty(Page.Request.QueryString["IdPag"]))
                {
                    Session[Session.SessionID + "ListaFacturaAlmacenCobroDet" + Page.Request.QueryString["IdPag"].ToString()] = value;
                }
                else
                {
                    Session[Session.SessionID + "ListaFacturaAlmacenCobroDet"] = value;
                }
            }
        }
            ...
            try
            {
                ...
                if (sesion != null)
                {
                    if (!Page.IsPostBack)
                    {
                        ...
                        int IdPag = Convert.ToInt32(Page.Request.QueryString["IdPag"]);
                        ...
                        this.Inicializar(Id_Emp, Id_Cd, Id_Fac, IdPag);
        ...
        private void Inicializar(int Id_Emp, int Id_Cd, int Id_Fac, int IdPag)
        {
            Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
            Session["ListaFacturaAlmacenCobroDet"+ IdPag.ToString()] = new List<FacturaAlmacenCobroDet>();
