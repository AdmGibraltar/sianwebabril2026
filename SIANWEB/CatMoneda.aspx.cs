...
namespace SIANWEB
{
    public partial class CatMoneda : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (e.CommandName.ToString() == "Modificar")
                {
                    ...
                    if (item >= 0)
                    {
                        ...
                        txtCodigo.Text = rgTipoMoneda.Items[item]["Mon_Codigo"].Text;
        ...
        protected void rgTipoMoneda_NeedDataSource(object source, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            try
            {
                if (e.RebindReason == GridRebindReason.ExplicitRebind)
                {
                    rgTipoMoneda.DataSource = GetList();
                }
            }
            catch (Exception ex)
            {
                DisplayMensajeAlerta(string.Concat(ex.Message, "rgTipoMoneda_NeedDataSource"));
            }
        }

        protected void rgTipoMoneda_PageIndexChanged(object source, GridPageChangedEventArgs e)
        {
            try
            {
                ErrorManager();
        ...
        protected void RadToolBar1_ButtonClick(object sender, RadToolBarEventArgs e)
        {
            ...
            {
                string mensaje = string.Concat(ex.Message, hiddenActualiza.Value == string.Empty ? "CatTipoMoneda_insert_error" : "CatTipoMoneda_update_error");
        ...
        private string MaximoId()
        {
            try
            {
                ...
                return CN_Comun.Maximo(sesion.Id_Emp, "CatTMoneda", "Id_Mon", sesion.Emp_Cnx, "spCatCentral_Maximo");
        ...
        private bool Deshabilitar()
        {
            try
            {
                ...
                if (hiddenActualiza.Value != "") //Hidden Field BANDERA
                {
                    ...
                    ct.Tabla = "CatTMoneda"; //Nombre de la tabla del catalogo
                    ct.Columna = "Id_Mon"; //Nombre de la columna del ID del catalogo
        ...
        private void Guardar()
        {
            try
            {
                ...
                tipoMoneda.Mon_codigo = txtCodigo.Text;
        ...
        private void DisplayMensajeAlerta(string mensaje)
        {
            ...
            else if (mensaje.Contains("rgTipoMoneda_NeedDataSource"))
                Alerta("Error al cargar el Grid de tipos de moneda");
            else if (mensaje.Contains("rgTipoMoneda_ItemCommand"))
                Alerta("Error al ejecutar un evento (rgTipoMoneda_ItemCommand) al cargar el Grid de tipo de moneda");
            else if (mensaje.Contains("Cmb_CentroDistribucion_IndexChanging_error"))
                Alerta("Error al cambiar de centro de distribución");
            else if (mensaje.Contains("radGrid_PageIndexChanged"))
                Alerta("Error al cambiar de página");
            else if (mensaje.Contains("PermisoGuardarNo"))
                Alerta("No tiene permisos para grabar");
            else if (mensaje.Contains("CatTipoMonedaIdRepetida_error"))
                Alerta("La clave ya existe");
            else if (mensaje.Contains("CatTipoMonedaDescripcionRepetida_error"))
                Alerta("La descripción ya existe");
            else if (mensaje.Contains("PermisoModificarNo"))
                Alerta("No tiene permisos para actualizar");
            else if (mensaje.Contains("TipoMoneda_insert_ok"))
                Alerta("Los datos se guardaron correctamente");
            else if (mensaje.Contains("TipoMoneda_insert_error"))
                Alerta("No se pudo guardar los datos del tipo de moneda");
            else if (mensaje.Contains("TipoMoneda_update_ok"))
                Alerta("Los datos se modificaron correctamente");
            else if (mensaje.Contains("TipoMoneda_update_error"))
                Alerta("No se pudo actualizar los datos del tipo de moneda");
            else
