    ...
    public partial class CapValProyCtasMarginales : System.Web.UI.Page
    {
        ...
        protected void cmbProductoDet_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    new CN_CatProducto().ConsultaProducto(ref prd, session.Emp_Cnx, session.Id_Emp, session.Id_Cd, Convert.ToInt32(txtProducto.Value.HasValue ? (sender as RadNumericTextBox).Value : -1),1);
                }
                catch (Exception ex)
                {
