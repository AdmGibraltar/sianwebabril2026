...
namespace SIANWEB
{
    public partial class MasterPage01 : System.Web.UI.MasterPage
    {
        ...
        private void VerMonitoreoComplementosPago()
        {
            CN_CapPago CPago = new CN_CapPago(); 

            Sesion session = new Sesion();
            session = (Sesion)Session["Sesion" + Session.SessionID];
            int NumDocNTimbrados = 0;
            CPago.VerMonitorComplementosNoTimbrados(session.Id_Emp, session.Id_Cd, session.Id_U, session.Emp_Cnx, ref NumDocNTimbrados);
            if (NumDocNTimbrados == -1)
            {
                lblMSgDocNoTimb.Text = "";
                this.DivDoc.Visible = false;
                this.ImageDocNoTimbrado.Visible = false;
            }
            else
            {
                if (NumDocNTimbrados < 10)
                {
                    lblMSgDocNoTimb.Text = NumDocNTimbrados.ToString();
                }
                else
                {
                    lblMSgDocNoTimb.Text = "9+";
                }
                this.DivDoc.Visible = true;
                this.ImageDocNoTimbrado.Visible = true;
            }
