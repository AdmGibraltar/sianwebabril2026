            ...
            try
            {
                ...
                if (prd != -1)
                {
                    ...
                    try
                    {
                        compras.ConsultarPrdCompraLocal(Sesion, prd, ref local);
                        if (local.Count > 0)
                            hddAAA.Value = local[0].AAA.ToString();
                        else
                            hddAAA.Value = "0";
                    }
                    catch (Exception ex)
                    {
                        hddAAA.Value = "0";
                        Alerta("No Se Encontraron Datos del Costo AAA del Padre");
                        return false;
                        //  AlertaFocus(ex.Message, txtProducto.ClientID);
                    }
                }
                ...
                double nCosto = Convert.ToDouble(txtCosto.Text);
                if (nCosto > Convert.ToDouble(hddAAA.Value))
                {
                    Alerta("El Costo del Producto Local No Puede Ser Mayor al Costo del Padre");
                    return false;
                }
                else
                    return true;
