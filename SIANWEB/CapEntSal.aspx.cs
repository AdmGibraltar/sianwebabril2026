    ...
    public partial class CapEntSal : System.Web.UI.Page
    {
        ...
        private int GrupoActual
        {
            get
            {
                ...
                string[] grupo2 = new string[] { "6", "15", "16", "27", "28" };
            ...
            try
            {
                ...
                switch (btn.CommandName)
                {
                    case "new":
                        ...
                        break;
                    case "save":
                        if (!string.IsNullOrEmpty(txtOCId.Text))
                        {
                            var listOrdenCompra = new List<OrdenCompra>();
                            var ordCompra = new OrdenCompra
                            {
                                Id_Emp = sesion.Id_Emp,
                                Id_Cd = sesion.Id_Cd_Ver,
                                Id_U = sesion.Propia ? sesion.Id_U : -1
                            };
                            new CN_CapOrdenCompra().ConsultaOrdenCompra_Lista(ordCompra, sesion.Emp_Cnx, ref listOrdenCompra, Convert.ToInt32(txtOCId.Text), Convert.ToInt32(txtOCId.Text), DateTime.MinValue, DateTime.MinValue, "");
                            if (listOrdenCompra.Count > 0 && listOrdenCompra[0].Ord_Estatus.ToLower() != "i")
                            {
                                Alerta("No se puede guardar porque la OC se encuentra en estatus " + listOrdenCompra[0].Ord_Estatus);
                                return;
                            }
                        }
                        Guardar();
                    ...
                    if (Convert.ToInt32(txtOCId.Text) > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (listOrdenCompra.Count != 0)
                            {
                                ...
                                if (txtProveedorId.Text != "" && listOrdenCompra[0].Id_Pvd != int.Parse(txtProveedorId.Text))
                                {
                                    Alerta("El cliente " + txtProveedorId.Text + " no pertenece a la OC " + txtOCId.Text);
                                    txtOCId.Text = "";
                                    return;
                                }
                                ...
                                for (int i = 0; i < listaOrdCompraDet.Count; i++)
                                {
                                    ...
                                    if (EntSalDetalle.Es_Cantidad > 0) detalles.Add(EntSalDetalle);
        ...
        {
            if (e.Item is GridDataItem i && !string.IsNullOrEmpty(txtOCId.Text))
            {
                var listOrdenCompra = new List<OrdenCompra>();
                var ordCompra = new OrdenCompra
                {
                ...
                if (listOrdenCompra.Count > 0 && listOrdenCompra[0].Ord_Estatus.ToLower() != "i")
                {
                    (i["EditCommandColumn"].Controls[0] as ImageButton).Visible = false;
                    (i["DeleteColumn"].Controls[0] as ImageButton).Visible = false;
                }
            }
        ...
        protected void rgEntradaSalida_ItemCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    int cantidadEdicion = int.Parse((rgEntradaSalida.MasterTableView.Items[e.Item.ItemIndex]["cantidad"].FindControl("RadNumericTextBoxCantidad") as RadNumericTextBox).Value.ToString());
                    int cantidadActual = int.Parse(Session["CantidadEdicion" + Session.SessionID + HF_ClvPag.Value].ToString());
                    Int64 prd_ = Int64.Parse((rgEntradaSalida.MasterTableView.Items[e.Item.ItemIndex]["Id_Prd"].FindControl("txtId_Prd") as RadNumericTextBox).Value.ToString());
                    int ter_ = int.Parse((rgEntradaSalida.MasterTableView.Items[e.Item.ItemIndex]["Id_Ter"].FindControl("txtTerritorio") as RadNumericTextBox).Value.ToString());
                    int can_ = 0;
                    ...
                    var listaOrdCompraDet = new List<OrdenCompraDet>();
                    var ordenCompraDet = new OrdenCompraDet
                    {
                        Id_Emp = sesion.Id_Emp,
                        Id_Cd = sesion.Id_Cd_Ver,
                        Id_Ord = Convert.ToInt32(txtOCId.Text)
                    };
                    new CN_CapOrdenCompraDet().spCapOrdCompraDetalle_Consulta_Entradas(ordenCompraDet, sesion.Emp_Cnx, ref listaOrdCompraDet);
                    ...
                    int maxCantidad = (int)listaOrdCompraDet.First(x => x.Id_Prd == prd_).Ord_Cantidad;
                    if (cantidadEdicion > maxCantidad)
                    {
                        Alerta("La cantidad no puede ser mayor a los " + maxCantidad + " permitido");
                        (rgEntradaSalida.MasterTableView.Items[e.Item.ItemIndex]["cantidad"].FindControl("RadNumericTextBoxCantidad") as RadNumericTextBox).Value = cantidadActual;
                        return;
                    }

                    Session["CantidadEdicion" + Session.SessionID + HF_ClvPag.Value] = cantidadEdicion;
                    ...
                    valor_retorno = Producto_Cantidad(sesion, valor_retorno.ToString(), nat_ == "" ? "-1" : nat_, _gpo.ToString() == "" ? "-1" : _gpo.ToString(), Convert.ToInt64(prd_), ref_ == "" ? "-1" : ref_, Convert.ToInt32(es_), Convert.ToInt32(ter_), can_, mov_ == "" ? "-1" : mov_, cte_ == "" ? "-1" : cte_, producto);
        ...
        protected void rgEntradaSalida_InsertCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                //  Si el codigo es de Compra Local, valida a variacion del costo, si es mayor al 20%, envia msg de error y no deja pasar
                //          Es_Det.Id_Cd, Es_Det.Id_Prd, Es_Det.Es_Costo
                if (!(bool)EvaluaCambioDeCosto(Es_Det.Id_Cd, Es_Det.Id_Prd, Es_Det.Es_Costo))
                {
                    //Es_Det.Id_EsDetStr = (editedItem["Id_EsDetStr"].FindControl("lblDet_Item") as Label).Text;
                    Alerta("<b>La diferencia entre el costo anterior y el capturado es mayor al permitido.</b><br> Si cuenta con el nuevo modulo de Compras Locales, debe generar una nueva solitud para este codigo-costo.<br> En caso de tener el modulo anterior, utilizar un nuevo codigo.");
                    e.Canceled = true;
                    list_Es.Remove(Es_Det);
        ...
        protected void cmbiva_SelectedIndexChanged(object sender, Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                CalcularTotales();
        ...
        private void showDetalles()
        {
            ...
            if (list_mov.Count > 0)
            {
                ...
                //rgEntradaSalida.Columns[rgEntradaSalida.Columns.FindByUniqueName("RadNumericTextBoxCosto").OrderIndex - 2].
        ...
        private bool EvaluaCambioDeCosto(int Id_Cd, long Id_Prd, double Es_Costo)
        {
            try
            {
                int iOk = 0;
                CapaNegocios.CN_ComprasLocales CNCompra = new CapaNegocios.CN_ComprasLocales();
                CNCompra.EvaluaCambioDeCosto(Id_Cd, Id_Prd, Es_Costo, sesion.Emp_Cnx, ref iOk);
        ...
        private bool AplicaCambioDeCostoCompraLocal(int Id_Cd, long Id_Prd, double Es_Costo)
        {
            try
            {
                int iOk = 0;
                CapaNegocios.CN_ComprasLocales CNCompra = new CapaNegocios.CN_ComprasLocales();
                CNCompra.AplicaCambioDeCostoCompraLocal(Id_Cd, Id_Prd, Es_Costo, sesion.Emp_Cnx, ref iOk);
        ...
        private void cargarMovimientoEntSal(bool nuevo)
        {
            ...
            try
            {
                ...
                txtOCId.Text = entradaSalida.Id_Ord.ToString();
        ...
        private void Guardar()
        {
            try
            {
                ...
                if (Request.QueryString["id"] == "-1" || Request.QueryString["id"] == null)
                {
                    ...
                    //  AplicaCambioDeCostoCompraLocal(entsal.Id_Cd, entsal
                    verificadorStr = "";
                }
                else
                {
        ...
        private void CargarTipoMovimiento(int tipo_movimiento, int Tm_Afecta = -1, bool nuevo = false) //Central
        {
            try
            {
                ...
                {
                    RemoverItem(new int[] { 18, 51, 25, 78, 81, 82 });
                }
                else
                {
        ...
        {
            cmbiva.Items.Insert(0, new RadComboBoxItem("16.00", "16"));
            cmbiva.Items.Insert(1, new RadComboBoxItem("8.00", "8"));
        ...
        private void CalcularTotales()
        {
            try
            {
                ...
                double iva = Math.Round(subtotal * Convert.ToInt32(cmbiva.SelectedValue.ToString()) / 100, 2);
        ...
        private EntradaSalidaDetalle GenerarDetalle(GridEditableItem editedItem, ref EntradaSalidaDetalle Es_Det)
        {
            try
            {
                ...
                Es_Det.Id_Prd = (Int64)(editedItem.FindControl("txtId_Prd") as RadNumericTextBox).Value;
        ...
        private string Producto_Cantidad(Sesion sesion, string valor_retorno, string nat_, string gpo_, Int64 id_prd, string ref_, int es_, int ter_, int can_, string mov_, string cte_, Producto producto)
        {
            try
            {
                if (nat_ == "1")
                ...
                else if (gpo_ == "1")
                {
                    if (actualizacionDocumento)
                    {
                        ...
                        cn_catproducto.ConsultaProducto(ref cp, sesion.Emp_Cnx, sesion.Id_Emp, sesion.Id_Cd_Ver, id_prd, 0);
