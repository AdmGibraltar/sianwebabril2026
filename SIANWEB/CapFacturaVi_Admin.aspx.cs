    ...
    public partial class CapFacturaVi_Admin : System.Web.UI.Page
    {
        ...
        protected void RAM1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
            try
            {
                ...
                GridDataItem dataItem = (GridDataItem)gi;
                string strIdPed = string.Empty;
                string strIdCte = string.Empty;
                string strCteNom = string.Empty;
                if (dataItem != null)
                {
                    strIdPed = dataItem["Id_Ped"].Text;
                    strIdCte = dataItem["Id_Cte"].Text;
                    strCteNom = dataItem["Cte_Nom"].Text;
                }
                ...
                switch (cmd)
                {
                    ...
                    case "ok":
                        ...
                        if (Session["PreguntarImpresionVI" + Session.SessionID] != null)
                        {
                            ...
                            string Num_ped = Convert.ToString(Session["PreguntarNum_ped" + Session.SessionID]);
                            Session["PreguntarImpresionVI" + Session.SessionID] = null;
                            ImprimirFactura(session.Id_Emp, session.Id_Cd_Ver, vi, Num_ped, "FACTURA", "");
                        }
                        break;
                    case "okRem":
                        int viRem = -1;
                    ...
                    case "no":
                        ...
                        break;
                    case "AsManual":
                        RAM1.ResponseScripts.Add("return AbrirVentana_AsigPrdxPed('" + strIdPed + "','" + strIdCte + "','" + strCteNom + "')");
                        break;
                    case "DesManual":
                        RAM1.ResponseScripts.Add("return AbrirVentana_AsigPrdxPed('" + strIdPed + "','" + strIdCte + "','" + strCteNom + "')");
                        break;
                    case "AsAuto":
                        cn_prodesasigna.AsignacionPedido_Aut(session.Id_Emp, session.Id_Cd_Ver, funcion.GetLocalDateTime(session.Minutos), session.Id_U, strIdPed, ref verificador, session.Emp_Cnx);
                        if (verificador > 0)
                        {
                        ...
                        break;
                    case "DesAuto":
                        cn_prodesasigna.DesasignacionPedido_Aut(session.Id_Emp, session.Id_Cd_Ver, strIdPed, ref verificador, session.Emp_Cnx);
                        if (verificador == 1)
                        {
        ...
        protected void rgPedido_ItemCommand(object source, GridCommandEventArgs e)
        {
            try
            {
                ...
                GridItem gi = e.Item;
                GridDataItem dataItem = (GridDataItem)gi;
                string strIdPed = dataItem["Id_Ped"].Text;
                ...
                var pedidoDetList = new List<PedidoDet>();
                CN_CapPedido cn_pedido = null;
                Pedido pedido = null;
                switch (e.CommandName)
                {
                    case "Asignar":
                        cn_pedido = new CN_CapPedido();
                        pedido = new Pedido
                        {
                            Id_Emp = session.Id_Emp,
                            Id_Cd = session.Id_Cd,
                            Id_Ped = Convert.ToInt32(strIdPed)
                        };
                        cn_pedido.ConsultaPedidoAsig_Picking_Pedido(pedido, session.Emp_Cnx, ref pedidoDetList);
                        if (pedidoDetList.Count > 0 && pedidoDetList.Any(x => x.Ped_Picking > 0))
                        {
                            string[] prds = pedidoDetList
                                .Where(x => x.Ped_Picking > 0)
                                .Select(x => x.Id_Prd.ToString()).ToArray();

                            string msjError = $"El pedido en el producto {string.Join(",", prds)} tiene picking capturado, favor de retirar el picking para poder asignar el producto en el pedido.";
                    ...
                    case "Desasignar":
                        ...
                        if (pedidoDetList.Count > 0 && pedidoDetList.Any(x => x.Ped_Picking > 0))
                        {
                            ...
                            string msjError = $"El pedido en el producto {string.Join(",", prds)} tiene picking capturado, favor de retirar el picking para poder desasignar el producto en el pedido.";
                    ...
                    case "Facturar":
                        ...
                        Session["PedidoFacturacion" + Session.SessionID] = strIdPed;
                        Session["PreguntarNum_ped" + Session.SessionID] = strIdPed;
                        ...
                        string vstrpcking = cmbpicking.SelectedValue == "-1" || cmbpicking.SelectedValue == "" ? "" : cmbpicking.SelectedValue;
                        RAM1.ResponseScripts.Add("return AbrirVentana_Factura('0','0','0','" + PermisoGuardar + "', '" + PermisoModificar + "', '" + PermisoEliminar + "','" + PermisoImprimir + "','" + vstrpcking + "')");
                        break;
                    case "Remisionar":
                        Session["PedidoVI" + Session.SessionID] = true;
                        Session["PedidoRemision" + Session.SessionID] = strIdPed;
                        string vstrpckingR = cmbpicking.SelectedValue == "-1" || cmbpicking.SelectedValue == "" ? "" : cmbpicking.SelectedValue;
                        RAM1.ResponseScripts.Add("return AbrirVentana_Remision(" + strIdPed + "," + _PermisoGuardar.ToString().ToLower() + ",'" + vstrpckingR + "')");
                        break;
                    case "Baja":
                        if (_PermisoEliminar)
                        {
                            ...
                            ped.Id_Ped = Convert.ToInt32(strIdPed);
                            ped.Id_U = session.Id_U;
                            ped.Ped_Fecha = DateTime.Now;
                            ped.Modulo = "Facturación de VI";
                            int verificador = -1;
                            cn_cappedidovtainst.Baja(ped, session.Emp_Cnx, ref verificador);
                            if (verificador == 1)
                            {
                                int verificador2 = 0;
                                cn_cappedidovtainst.logCancelarPedido(ped, session.Emp_Cnx, ref verificador2);
                        ...
                        {
                            Alerta("no se tiene permiso para dar de baja el pedido");
                        }
                        break;
                }
            }
        ...
        protected void Sel_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                GridDataItem dataItem = (chk.Parent.Parent as GridDataItem);
                if (dataItem["Ped_Total"].Text == "N")
                {
                ...
                if (dataItem["Facturar"].Text == "true")
                {
        ...
        private void CargarAsignar()
        {
            try
            {
                ...
                {*/
                cmbpicking.Items.Clear();
                cmbpicking.Items.Add(new RadComboBoxItem("-- Todos --", ""));
                cmbpicking.Items.Add(new RadComboBoxItem("Si", "K"));
                cmbpicking.Items.Add(new RadComboBoxItem("No", "N"));
                /* } else
                 {
                     cmbpicking.Items.Clear();
                     cmbpicking.Items.Add(new RadComboBoxItem("Si", "K"));
        ...
        private List<PedidoVtaInst> GetList()
        {
            try
            {
                ...
                pedido.Filtro_Tipo = cmbpicking.SelectedValue == "-1" || cmbpicking.SelectedValue == "" ? (string)null : cmbpicking.SelectedValue;
        ...
        private void GuardarRemision(int? pedidoPrevio)
        {
            try
            {
                ...
                string mensaje = "";
                cn_capRemision.GuardarRemision(remision, listaRemisionDet, session, ref verificador, false, true, ref Id_Rem, ref tipoMovimento, ref mensaje, "", ConfigurationManager.AppSettings["PermitePrecios0Remision"].ToString());
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void GuardarFactura(int? pedidoPrevio)
        {
            try
            {
                ...
                new CN_CapFactura().InsertarFactura(session, ref factura
                    ...
                    , ref verificador
                    , ref pedidoPrevio
                    , ref listaEntSalRemisiones, listAdenda, "", "", ref es, ref listes, ref facturaEsp, ref listaProductosFacturaEspecial, false, cmbpicking.SelectedValue == "-1" || cmbpicking.SelectedValue == "" ? "" : cmbpicking.SelectedValue);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void ImprimirRemision(int Id_Rem)
        {
            try
            {
                ...
                if (bRemisionValida)
                {
                    ...
                    Session["idRem" + Session.SessionID + HF_ClvPag.Value] = remision.Id_Rem;
        ...
        private void ImprimirFactura(int Id_Emp, int Id_Cd, int Id_Fac, string Num_ped, string movimiento, string agregado_nota_cancelacion, bool tienePDF = false)
        {
            ...
            try
            {
                ...
                if (bDocumentoValido)
                {
                    ...
                    cabecera.Attributes["FormaRevision"].Value = cliente.RevPago.ToString();
                    ...
                    if (facturaNacional != null)
                    {
                        ...
                        cabeceraCN.Attributes["Agente"].Value = factura.Rik_Nombre == null ? string.Empty : factura.Rik_Nombre.ToString();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            int verificador2 = 0;
                            factura = new Factura();
                            CN_Eccommerce Eccomerce = new CN_Eccommerce();
                            factura.Id_Emp = Sesion.Id_Emp;
                            factura.Id_Cd = Sesion.Id_Cd_Ver;
                            factura.Id_Fac = Id_Fac;

                            Eccomerce.BuscarNumeroPedidoEcommerce(factura, Sesion.Emp_Cnx, ref verificador2);
                            ...
                            if (facturaNacional != null)
                            {
                                ...
                                Eccomerce.ValidarPedidoFacturaEcommerce(factura, Sesion.Emp_Cnx, ref lista);
                                string estatus = "";
                                string mensaje = "";
                                if (lista.Count > 0)
                                {
