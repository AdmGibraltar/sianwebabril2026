    ...
    public partial class CapDevParcial_Lista : System.Web.UI.Page
    {
        ...
        private void baja(ref GridCommandEventArgs e, ref List<string> statusPosibles, ref GridItem item)
        {
            try
            {
                ...
                {
                    Telerik.Web.UI.RadComboBox cmdNCrSerie = new RadComboBox();
                    CapaNegocios.CN__Comun CN_Comun = new CapaNegocios.CN__Comun();
                    CN_Comun.LlenaCombo(1, Sesion.Id_Emp, Sesion.Id_Cd, 3, Sesion.Emp_Cnx, "spCatConsFactEle_Combo", ref cmdNCrSerie);
                    var serieNC = cmdNCrSerie.FindItemByValue("1").Text + int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Id_Nca2"].Text);

                    int numCliente = int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Num_Cliente"].Text);
                    int Id_Nca = int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Id_Nca"].Text);
                    string statusDev = rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Es_Estatus"].Text;
                    string fechaDev = rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Fecha"].Text;
                    int TipoDev = int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Tipo"].Text);
                    int factura_dev = int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Factura"].Text);
                    double Nca_Subtotal = double.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Nca_Subtotal"].Text);
                    double Nca_Iva = double.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Nca_Iva"].Text);
                    double Nca_Total = double.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Nca_Total"].Text);
                    string serieDev = cmdNCrSerie.FindItemByValue("1").Text;
                    RAM1.ResponseScripts.Add(string.Concat(@"AbrirVentana_CancelarNotaCreditoVencido('", Sesion.Id_Emp, "','", Sesion.Id_Cd, "','", int.Parse(rgDevolucion.MasterTableView.Items[e.Item.ItemIndex]["Id_Nca2"].Text), "','", serieNC, "','", numCliente, "','", Id_Nca, "','", statusDev, "','", fechaDev, "','", TipoDev, "','", factura_dev, "','", Nca_Subtotal, "','", Nca_Iva, "','", Nca_Total, "','", serieDev, "')"));
                    return;
                }
                ...
                if (verificador == 1)
                {
                    ...
                    notaCredito.Id_Cd = Sesion.Id_Cd;
                    notaCredito.Id_Ncr = devParcial.Id_Nca2;
                    notaCredito.Id_NcrSerie = cmdNCrSerie.FindItemByValue("1").Text + devParcial.Id_Nca2;

                    CapNotaCredito_Lista RefCapNotaCredito = new CapNotaCredito_Lista();
                    RefCapNotaCredito.CancelarNotaCredito(notaCredito.Id_Emp, notaCredito.Id_Cd, notaCredito.Id_Ncr, notaCredito.Id_NcrSerie, ref strMsjError, ref strScript);
