    ...
    public partial class RepAnalisisRemisiones : System.Web.UI.Page
    {
        ...
        private void Abrir_Reporte(bool a_pantalla)
        {
            try
            {
                ...
                if (sesion.Id_Cd == 0)
                {
                    ALValorParametrosInternos.Add(sesion.Id_Cd_Ver.ToString());
                }
                else
                {
        ...
        private void ImprimirXLS(ArrayList ALValorParametrosInternos, Type instance, ArrayList ALValorParametrosConsignacion, Type instance2)
        {
            try
            {
                ...
                Telerik.Reporting.Report NEVIG = (Telerik.Reporting.Report)Activator.CreateInstance(instance);
                for (int i = 0; i <= ALValorParametrosInternos.Count - 1; i++)
                {

                    NEVIG.ReportParameters[i].AllowNull = true;
                    NEVIG.ReportParameters[i].Value = ALValorParametrosInternos[i];
                }

                NEVIG.ReportParameters[16].Value = 61;
                NEVIG.ReportParameters[1].Value = "Producto no encontrado vigente";
                NEVIG.ReportParameters[23].Value = 0;  //vigente

                //Reporte de No Encontrado Vencidas
                Telerik.Reporting.Report NEVEN = (Telerik.Reporting.Report)Activator.CreateInstance(instance);
                for (int i = 0; i <= ALValorParametrosInternos.Count - 1; i++)
                {
                    NEVEN.ReportParameters[i].AllowNull = true;
                    NEVEN.ReportParameters[i].Value = ALValorParametrosInternos[i];
                }
                NEVEN.ReportParameters[16].Value = 61;
                NEVEN.ReportParameters[1].Value = "Producto no encontrado vencidas";
                NEVEN.ReportParameters[23].Value = 1;  //Vencida
                ...
                reportBook.Reports.Add(NEVIG);
                reportBook.Reports.Add(NEVEN);
                ...
                reportBook.Reports[7].DocumentName = "NO_ENCONTRADO_VIG";
                reportBook.Reports[8].DocumentName = "NO_ENCONTRADO_VEN";
