...
namespace SIANWEB
{
    public partial class CatProductos : System.Web.UI.Page
    {
        ...
        {
            get
            {   // si hay un producto seleccionado toma la variable de sesion del producto, 
                if (!string.IsNullOrEmpty(cmbProductosLista.SelectedValue))
                {
                    return (List<ProductoPrecios>)Session["listSource" + cmbProductosLista.SelectedValue];
                }
                else
                {
                    // variable de sesion para producto nuevo
                    return (List<ProductoPrecios>)Session["listSource"];
                }
            }
            set
            {
                ...
                if (!string.IsNullOrEmpty(cmbProductosLista.SelectedValue))
                {
                    Session["listSource" + cmbProductosLista.SelectedValue] = value;
                }
                else
                {
                    // variable de sesion para producto nuevo
                    Session["listSource"] = value;
                }
            }
        }
        ...
        {
            get { return Convert.ToInt64(Session["_IdProducto"]); }
        ...
        protected void cmbProductosLista_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Value != string.Empty && e.Value != "-1")
                {
                    ...
                    // Crea e inicializa la variable de sesion relacionada al producto seleccionado
                    Session["listSource" + e.Value] = null;

                    this.LlenarFormularioProducto(Convert.ToInt64(e.Value), id_Cd_Prod);
        ...
        private void LlenarFormularioProducto(Int64 id_Producto, int id_Cd_Prod)
        {
            try
            {
                ...
                chkActivo.Checked = producto.Prd_Activo == 1 ? true : false;
                chkActivo.Text = producto.Prd_Activo == 1 ? "Activo" : producto.Prd_Activo == 2 ? "Inactivo" : "Obsoleto";
                lblEstatusPrd.Text = producto.Prd_Activo == 1 ? "Activo" : producto.Prd_Activo == 2 ? "Inactivo" : "Obsoleto";
                ...
                string BotonGuardar = ConfigurationManager.AppSettings["BotonGuardarProducto"].ToString();

                if (producto.Prd_SGCUP == "S" || BotonGuardar == "0")
                {
                    this.RadToolBar1.Items[5].Visible = false;
                }
                else
                ...
                try
                {
                    ...
                    if (cmbClaveProductoServicio.FindItemIndexByValue(producto.Prd_ClaveProdServ.ToString()) == -1)
                    {
                    ...
                    if (cmbClaveUnidad.FindItemIndexByValue(producto.Prd_ClaveUnidad.ToString()) == -1)
                    {
                        CargarUnidades();
                    }
                    cmbClaveUnidad.SelectedIndex = cmbClaveUnidad.FindItemIndexByValue(producto.Prd_ClaveUnidad.ToString());
        ...
        private Producto LlenatObjetoProducto()
        {
            ...
            producto.Prd_Activo = chkActivo.Checked ? 1 : 2;
        ...
        private Producto ConsultarPorducto(Int64 id_Producto, int id_Cd_Prod)
        {
            try
            {
                Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
        ...
        private List<ProductoPrecios> ConsultarPorductoPrecios(Int64 id_Producto)
        {
            try
            {
                Sesion sesion = (Sesion)Session["Sesion" + Session.SessionID];
        ...
        private void CargarProductos()//Local
        {
            try
            {
                ...
                Sesion = (Sesion)Session["Sesion" + Session.SessionID];
                CapaNegocios.CN__Comun CN_Comun = new CapaNegocios.CN__Comun();
                CN_Comun.LlenaCombo64(0, Sesion.Id_Emp, Sesion.Id_Cd_Ver, Sesion.Emp_Cnx, "spCatProducto_Combo", ref cmbProductosLista);
            }
            catch (Exception ex)
            {
                throw ex;
