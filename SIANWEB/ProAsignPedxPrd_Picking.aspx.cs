...
namespace SIANWEB
{
    public partial class ProAsignPedxPrd_Picking : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void ImbConfirmar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ConfirmarPedido(Convert.ToInt64(txtProducto.Text), RadTextBoxRuta.Text, RadTextCredito.Text, RadTextParcialidades.Text, RadTextTipoPedido.Text);
            }
            catch (Exception ex)
            {
                ErrorManager(ex, new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        protected void ImbCancelar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                CancelarPedido(Convert.ToInt64(txtProducto.Text), RadTextBoxRuta.Text, RadTextCredito.Text, RadTextParcialidades.Text, RadTextTipoPedido.Text);
            }
            catch (Exception ex)
            {
                ErrorManager(ex, new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        private void Inicializar()
        {
            ...
            RadTextCredito.Text = Request.QueryString["Credito"].ToString();
            RadTextParcialidades.Text = Request.QueryString["Parcialidades"].ToString();
            RadTextTipoPedido.Text = Request.QueryString["TipoPedido"].ToString();
            ...
            try
            {
                if (Convert.ToBoolean(HF_Guardar.Value))
                {
                    ...
                    for (int x = 0; x < rgPedido.Items.Count; x++)
                    {
                        ...
                        Ped_Det.Id_Prd = Convert.ToInt64(txtProducto.Text);
                        ...
                        asignable += Ped_Det.Prd_Asig;
                        Ped_Det.Ped_Picking = Convert.ToInt32((rgPedido.Items[x]["Ped_Picking"].FindControl("txtPicking") as RadNumericTextBox).Text);
                        list.Add(Ped_Det);
                    }

                    /*int verificador = 0;
                    CN_CatProducto cn_catproducto = new CN_CatProducto();
                    cn_catproducto.AsignarPedXPrd(pedido, list, session.Emp_Cnx, ref verificador, asignable, Convert.ToInt64(txtProducto.Text));
                    */

                    int verificador = 0;
                    CN_CapPedido cn_cappedido = new CN_CapPedido();

                    cn_cappedido.AsignarPrdXPed_Picking(pedido, list, session.Emp_Cnx, ref verificador);
                    ...
                    {
                        Alerta("Picking confirmado correctamente");
                        rgPedido.Rebind();
                        return;
                    }
                    else
        ...
        private List<ProductoDet> GetList()
        {
            try
            {
                try
                {
                    ...
                    PedidoDet prdDet = new PedidoDet();
                    prdDet.Id_Emp = session2.Id_Emp;
                    prdDet.Id_Cd = session2.Id_Cd_Ver;
                    prdDet.Id_Prd = Convert.ToInt64(HF_Ped.Value);
                    prdDet.Ruta = RadTextBoxRuta.Text;
                    prdDet.CreditoStr = RadTextCredito.Text;
                    prdDet.Ped_PermiteParcialidades = RadTextParcialidades.Text;
                    prdDet.TipoPedido = RadTextTipoPedido.Text;


                    CN_CapPedido.ConsultaAsignPedxPrd_Picking_Producto(prdDet, session2.Emp_Cnx, ref List);
                    return List;
                }
                catch (Exception ex)
                {
