...
namespace SIANWEB
{
    public partial class ProValidaCierre : System.Web.UI.Page
    {
        #region Varibles
        ...
        protected void imgAceptar_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                    ...
                    CN_ProCierreMes cn_ProValidaCierremes = new CN_ProCierreMes();
                    cn_ProValidaCierremes.ValidaCierre(sesion.Id_Emp, sesion.Id_Cd_Ver, sesion.Emp_Cnx, ref MensajeAlerta);

                    if (MensajeAlerta.Trim() != "") { 
                        Alerta(MensajeAlerta);
                    } else
                    {
                        Alerta("No tiene documentos pendientes de imprimir, Está listo para hacer el cierre");
                    }
